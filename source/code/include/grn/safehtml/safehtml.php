...
define('UNICODE_X0000', html_entity_decode('&#x0000;', ENT_NOQUOTES, 'UTF-8'));
define('UNICODE_X0299',
    html_entity_decode('&#x0299;', ENT_NOQUOTES, 'UTF-8'));  // ʙ
define('UNICODE_X0262',
    html_entity_decode('&#x0262;', ENT_NOQUOTES, 'UTF-8'));  // ɢ
define('UNICODE_X029C',
    html_entity_decode('&#x029C;', ENT_NOQUOTES, 'UTF-8'));  // ʜ
define('UNICODE_X026A',
    html_entity_decode('&#x026A;', ENT_NOQUOTES, 'UTF-8'));  // ɪ
define('UNICODE_X029F',
    html_entity_decode('&#x029F;', ENT_NOQUOTES, 'UTF-8'));  // ʟ
define('UNICODE_X0274',
    html_entity_decode('&#x0274;', ENT_NOQUOTES, 'UTF-8'));  // ɴ
define('UNICODE_X207F',
    html_entity_decode('&#x207F;', ENT_NOQUOTES, 'UTF-8'));  // ⁿ
define('UNICODE_X0280',
    html_entity_decode('&#x0280;', ENT_NOQUOTES, 'UTF-8'));  // ʀ
define('UNICODE_X028F',
    html_entity_decode('&#x028F;', ENT_NOQUOTES, 'UTF-8'));  // ʏ
...
class SafeHTML
{
     ...
     * @access public
     */
    var $singleTags = ['area', 'br', 'img', 'input', 'hr', 'wbr',];
     ...
     * @access public
     */
    var $deleteTagsContent = ['script', 'style', 'title', 'xml',];
     ...
     * @access public
     */
    var $listTags = ['dir', 'menu', 'ol', 'ul', 'dl',];
     ...
     * @access private
     */
    function _writeAttrs($tagName, $attrs)
    {
        if (is_array($attrs)) {
            $attrs = $this->beforeWriteAttrs($tagName, $attrs);
            foreach ($attrs as $name => $value) {
                ...
                if ($name == 'style') {
                    ...
                    // Start(GRN2-7264)
                    foreach (
                        $this->normalizeChars as $searchChar => $replaceChar
                    ) {
                        $value = str_replace($searchChar, $replaceChar, $value);
                    }
                    // End(GRN2-7264)
                ...
                // replace a decimal format html entity to a character
                $tempval = preg_replace_callback(
                    '/&#(\d+);?/m', function ($matches) {
                    return chr($matches[1]);
                }, $value
                );
                // replace a hex format html entity to a character
                $tempval = preg_replace_callback(
                    '/&#x([0-9a-f]+);?/mi', function ($matches) {
    ...
    function _openHandler($parser, $name, $attrs)
    {
        ...
        $this->_counter[$name] = isset($this->_counter[$name])
            ? $this->_counter[$name] + 1 : 1;
