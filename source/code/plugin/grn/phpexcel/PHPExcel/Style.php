    ...
    implements PHPExcel_IComparable
{
    ...
    public function __construct($isSupervisor = false, $isConditional = false)
    {
        ...
        $this->_alignment = new PHPExcel_Style_Alignment($isSupervisor,
            $isConditional);
        $this->_numberFormat
            = new PHPExcel_Style_NumberFormat($isSupervisor,
            $isConditional);
    ...
    public function applyFromArray($pStyles = null, $pAdvanced = true)
    {
        if (is_array($pStyles)) {
            if ($this->_isSupervisor) {
                ...
                $rangeStart[0]
                    = PHPExcel_Cell::columnIndexFromString($rangeStart[0]) - 1;
                $rangeEnd[0]
                    = PHPExcel_Cell::columnIndexFromString($rangeEnd[0]) - 1;
                ...
                if ($pAdvanced && isset($pStyles['borders'])) {
                    ...
                    if (isset($pStyles['borders']['outline'])) {
                        foreach (
                            ['top', 'right', 'bottom', 'left'] as $component
                        ) {
                            if ( ! isset($pStyles['borders'][$component])) {
                                $pStyles['borders'][$component]
                    ...
                    for ($x = 1; $x <= $xMax; ++$x) {
                        ...
                        for ($y = 1; $y <= $yMax; ++$y) {
                            ...
                            $range = $colStart . $rowStart . ':' . $colEnd
                                     . $rowEnd;
                ...
                switch ($selectionType) {
                    case 'COLUMN':
                        $oldXfIndexes = [];
                        for (
                            $col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col
                        ) {
                    ...
                    case 'ROW':
                        $oldXfIndexes = [];
                        for (
                            $row = $rangeStart[1]; $row <= $rangeEnd[1]; ++$row
                        ) {
                            if ($this->getActiveSheet()->getRowDimension($row)
                                     ->getXfIndex() == null
                            ) {
                                $oldXfIndexes[0]
                    ...
                    case 'CELL':
                        ...
                        for (
                            ...
                            for (
                                $row = $rangeStart[1]; $row <= $rangeEnd[1];
                                ++$row
                            ) {
                                $oldXfIndexes[$this->getActiveSheet()
                                                   ->getCellByColumnAndRow($col,
                                                       $row)->getXfIndex()]
                                    = true;
                            }
                        }
