...
class PHPExcel_Calculation
{
    ...
    private function __construct(PHPExcel $workbook = null)
    {
        ...
        $this->_cyclicReferenceStack
            = new PHPExcel_CalcEngine_CyclicReferenceStack();
        $this->_debugLog
            = new PHPExcel_CalcEngine_Logger($this->_cyclicReferenceStack);
    ...
    public function renameCalculationCacheForWorksheet(
        ...
        if (isset($this->_calculationCache[$fromWorksheetName])) {
            $this->_calculationCache[$toWorksheetName]
                = &$this->_calculationCache[$fromWorksheetName];
    ...
    public function setLocale($locale = 'en_us')
    {
        ...
        if (in_array($language, self::$_validLocaleLanguages)) {
            ...
            if ($locale != 'en_us') {
                ...
                $functionNamesFile = PHPEXCEL_ROOT . 'PHPExcel'
                ...
                foreach ($localeFunctions as $localeFunction) {
                    list($localeFunction) = explode('##',
                        $localeFunction);    //	Strip out comments
                    if (strpos($localeFunction, '=') !== false) {
                        list($fName, $lfName) = explode('=', $localeFunction);
                        $fName = trim($fName);
                        $lfName = trim($lfName);
                        if ((isset(self::$_PHPExcelFunctions[$fName]))
                            && ($lfName != '')
                            && ($fName != $lfName)
                        ) {
                ...
                if (isset(self::$_localeFunctions['TRUE'])) {
                    self::$_localeBoolean['TRUE']
                        = self::$_localeFunctions['TRUE'];
                }
                if (isset(self::$_localeFunctions['FALSE'])) {
                    self::$_localeBoolean['FALSE']
                        = self::$_localeFunctions['FALSE'];
                }

                $configFile = PHPEXCEL_ROOT . 'PHPExcel' . DIRECTORY_SEPARATOR
                              . 'locale' . DIRECTORY_SEPARATOR
                              . str_replace('_', DIRECTORY_SEPARATOR, $locale)
                              . DIRECTORY_SEPARATOR . 'config';
                if ( ! file_exists($configFile)) {
                    $configFile = PHPEXCEL_ROOT . 'PHPExcel'
                                  . DIRECTORY_SEPARATOR . 'locale'
                                  . DIRECTORY_SEPARATOR . $language
                                  . DIRECTORY_SEPARATOR . 'config';
                }
            ...
            self::$functionReplaceFromExcel
                = self::$functionReplaceToExcel = self::$functionReplaceFromLocale
    ...
    public static function _translateSeparator(
        $fromSeparator,
        $toSeparator,
        $formula,
        &$inBraces
    ...
    private static function _translateFormula(
        $from,
        $to,
        $formula,
        $fromSeparator,
            ...
            if (strpos($formula, '"') !== false) {
                ...
                foreach ($temp as &$value) {
                    ...
                    if ($i = ! $i) {
                        ...
                        $value = self::_translateSeparator($fromSeparator,
                ...
                $formula = self::_translateSeparator($fromSeparator,
    ...
    public function _translateFormulaToLocale($formula)
    {
        ...
        if (self::$functionReplaceToLocale === null) {
            ...
            foreach (
                array_values(self::$_localeFunctions) as $localeFunctionName
            ) {
                self::$functionReplaceToLocale[] = '$1'
                                                   . trim($localeFunctionName)
                                                   . '$2';
            }
            foreach (array_values(self::$_localeBoolean) as $localeBoolean) {
                self::$functionReplaceToLocale[] = '$1' . trim($localeBoolean)
                                                   . '$2';
            }
        }

        return self::_translateFormula(self::$functionReplaceFromExcel,
    ...
    public function _translateFormulaToEnglish($formula)
    {
        if (self::$functionReplaceFromLocale === null) {
            ...
            foreach (
                ...
                self::$functionReplaceFromLocale[] = '/(@?[^\w\.])'
        ...
        if (self::$functionReplaceToExcel === null) {
            ...
            foreach (array_keys(self::$_localeFunctions) as $excelFunctionName) {
                self::$functionReplaceToExcel[] = '$1'
                                                  . trim($excelFunctionName)
                                                  . '$2';
            }
            foreach (array_keys(self::$_localeBoolean) as $excelBoolean) {
                self::$functionReplaceToExcel[] = '$1' . trim($excelBoolean)
                                                  . '$2';
            }
        }

        return self::_translateFormula(self::$functionReplaceFromLocale,
            self::$functionReplaceToExcel, $formula,
            self::$_localeArgumentSeparator, ',');
    ...
    public static function _wrapResult($value)
    {
        if (is_string($value)) {
            ...
            if (preg_match('/^' . self::CALCULATION_REGEXP_ERROR . '$/i',
                $value, $match)
            ) {
    ...
    public function calculateCellValue(
        ...
        try {
            $result
                = self::_unwrapResult($this->_calculateFormulaValue($pCell->getValue(),
                $pCell->getCoordinate(), $pCell));
            $cellAddress = array_pop($this->_cellStack);
            $this->_workbook->getSheetByName($cellAddress['sheet'])
        ...
        if ((is_array($result))
            ...
            $testResult
                = PHPExcel_Calculation_Functions::flattenArray($result);
    ...
    public function calculateFormula(
        ...
        try {
            $result
                = self::_unwrapResult($this->_calculateFormulaValue($formula,
                $cellID, $pCell));
        } catch (PHPExcel_Exception $e) {
            throw new PHPExcel_Calculation_Exception($e->getMessage());
        }
    ...
    public function getValueFromCache($cellReference, &$cellValue)
    {
        ...
        $this->_debugLog->writeDebugLog('Testing cache value for cell ',
            $cellReference);
        if (($this->_calculationCacheEnabled)
            && (isset($this->_calculationCache[$cellReference]))
        ) {
            $this->_debugLog->writeDebugLog('Retrieving value for cell ',
                $cellReference, ' from cache');
    ...
    public function _calculateFormulaValue(
        ...
        if (($wsTitle{0} !== "\x00")
            && ($this->_cyclicReferenceStack->onStack($wsCellReference))
        ) {
        ...
        $cellValue = $this->_processTokenStack($this->_parseFormula($formula,
            $pCell), $cellID, $pCell);
     ...
     *                                            2 = extend to fit
     */
    private static function _checkMatrixOperands(
        &$operand1,
        &$operand2,
        $resize = 1
    ) {
        ...
        if ( ! is_array($operand1)) {
            list($matrixRows, $matrixColumns)
                = self::_getMatrixDimensions($operand2);
            $operand1 = array_fill(0, $matrixRows,
                array_fill(0, $matrixColumns, $operand1));
            $resize = 0;
        } elseif ( ! is_array($operand2)) {
            list($matrixRows, $matrixColumns)
                = self::_getMatrixDimensions($operand1);
            $operand2 = array_fill(0, $matrixRows,
                array_fill(0, $matrixColumns, $operand2));
            $resize = 0;
        }

        list($matrix1Rows, $matrix1Columns)
            = self::_getMatrixDimensions($operand1);
        list($matrix2Rows, $matrix2Columns)
            = self::_getMatrixDimensions($operand2);
        if (($matrix1Rows == $matrix2Columns)
        ...
        if ($resize == 2) {
            ...
            self::_resizeMatricesExtend($operand1, $operand2, $matrix1Rows,
                $matrix1Columns, $matrix2Rows, $matrix2Columns);
        } elseif ($resize == 1) {
            //	Given two matrices of (potentially) unequal size, convert the larger in each dimension to match the smaller
            self::_resizeMatricesShrink($operand1, $operand2, $matrix1Rows,
                $matrix1Columns, $matrix2Rows, $matrix2Columns);
        }
        ...
        if ($this->_debugLog->getWriteDebugLog()) {
            ...
            if (is_array($value)) {
                ...
                foreach ($value as $row) {
                    if (is_array($row)) {
                        $returnMatrix[] = implode($pad,
                            array_map([$this, '_showValue'], $row));
                        $rpad = '; ';
                    } else {
                ...
                return ($value) ? self::$_localeBoolean['TRUE']
                    : self::$_localeBoolean['FALSE'];
            }
        }
    ...
    private function _convertMatrixReferences($formula)
    {
        ...
        if (strpos($formula, '{') !== false) {
            ...
            if (strpos($formula, '"') !== false) {
                ...
                $formula = str_replace($matrixReplaceFrom, $matrixReplaceTo,
                    $formula);
            }
    ...
    {
        if (($formula = $this->_convertMatrixReferences(trim($formula)))
            === false
        ) {
            return false;
        }
...
//echo 'Assessing Expression '.substr($formula, $index),PHP_EOL;
            $opCharacter
                = $formula{$index};    //	Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter,PHP_EOL;
            if ((isset(self::$_comparisonOperators[$opCharacter]))
...
//echo 'Element is a Positive number, not Plus operator',PHP_EOL;
            ...
            } elseif ((isset(self::$_operators[$opCharacter])
                       or $isOperandOrFunction)
                      && $expectingOperator
...
//echo 'Element with value '.$opCharacter.' is an Operator',PHP_EOL;
                ...
                $stack->push('Binary Operator',
                    $opCharacter);    //	Finally put our current operator onto the stack
                ++$index;
...
//echo 'Element is a Closing bracket',PHP_EOL;
                ...
                $d = $stack->last(2);
                if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i',
                    $d['value'], $matches)
                ) {    //	Did this parenthesis just close a function?
                    $functionName
                        = $matches[1];                                        //	Get the function name
...
//}
                    $output[]
                        = $d;                        //	Dump the argument count on the output
                    $output[]
...
//echo 'Built-in function '.$functionName,PHP_EOL;
                        $expectedArgumentCount
                            = self::$_controlFunctions[$functionName]['argumentCount'];
                        $functionCall
                            = self::$_controlFunctions[$functionName]['functionCall'];
                    } elseif (isset(self::$_PHPExcelFunctions[$functionName])) {
//echo 'PHPExcel function '.$functionName,PHP_EOL;
                        $expectedArgumentCount
                            = self::$_PHPExcelFunctions[$functionName]['argumentCount'];
                        $functionCall
                            = self::$_PHPExcelFunctions[$functionName]['functionCall'];
...
//echo '$expectedArgumentCount is between 0 and '.abs($expectedArgumentCount),PHP_EOL;
                            if ($argumentCount > abs($expectedArgumentCount)) {
                                ...
                                $expectedArgumentCountString = 'no more than '
...
//echo '$expectedArgumentCount is numeric '.$expectedArgumentCount,PHP_EOL;
                        ...
                        $isOperandOrFunction = preg_match('/(\d*)([-+,])(\d*)/',
                            $expectedArgumentCount, $argMatch);
//print_r($argMatch);
//echo PHP_EOL;
                        switch ($argMatch[2]) {
                            case '+' :
                                if ($argumentCount < $argMatch[1]) {
                                    $argumentCountError = true;
                                    $expectedArgumentCountString = $argMatch[1]
                                                                   . ' or more ';
                                }
                                break;
                            case '-' :
                                if (($argumentCount < $argMatch[1])
                                    ...
                                    $argumentCountError = true;
                                    $expectedArgumentCountString = 'between '
                                                                   . $argMatch[1]
                                                                   . ' and '
                                                                   . $argMatch[3];
                                }
                            ...
                            case ',' :
                                if (($argumentCount != $argMatch[1])
                                    ...
                                    $argumentCountError = true;
                                    $expectedArgumentCountString = 'either '
                                                                   . $argMatch[1]
                                                                   . ' or '
                                                                   . $argMatch[3];
                                }
...
//echo 'Element is a Function argument separator',PHP_EOL;
                ...
                $d = $stack->pop();
                $stack->push($d['type'], ++$d['value'],
                    $d['reference']);    // increment the argument count
                $stack->push('Brace',
...
//					echo 'Element '.$val.' is a Function<br />';
                    if (isset(self::$_PHPExcelFunctions[strtoupper($matches[1])])
                        ...
                        if ($ax) {
                            $stack->push('Operand Count for Function '
                                         . strtoupper($val) . ')', 0);
                            $expectingOperator = true;
                        } else {
                            $stack->push('Operand Count for Function '
                ...
                } elseif (preg_match('/^' . self::CALCULATION_REGEXP_CELLREF
                                     . '$/i', $val, $matches)
                ) {
...
//						echo 'Element is a String<br />';
                        ...
                        $val = self::_wrapResult(str_replace('""', '"',
...
//						echo 'Element '.$excelConstant.' is an Excel Constant<br />';
                        $val = self::$_ExcelConstants[$excelConstant];
                            ...
                            = array_search(trim(strtoupper($val)),
                            self::$_localeBoolean)) !== false
                    ) {
//						echo 'Element '.$localeConstant.' is an Excel Constant<br />';
            ...
            if ($index == strlen($formula)) {
                ...
                if ((isset(self::$_operators[$opCharacter]))
                    && ($opCharacter != '%')
                ) {
...
//					echo 'Element is an Intersect Operator<br />';
        ...
        while (($op = $stack->pop())
               !== null) {    // pop everything off the stack and push onto output
    ...
    private function _processTokenStack(
        $tokens,
        $cellID = null,
        PHPExcel_Cell $pCell = null
    ) {
...
//				echo 'Token is a binary operator<br />';
                ...
                if ($token == ':') {
                    ...
                    $this->_debugLog->writeDebugLog('Evaluating ',
                        $this->_showValue($operand1), ' ', $token, ' ',
                ...
                switch ($token) {
                    ...
                    case '='    :            //	Equality
                    case '<>'    :            //	Inequality
                        $this->_executeBinaryComparisonOperation($cellID,
                            $operand1, $operand2, $token, $stack);
                        break;
                    //	Binary Operators
                    case ':'    :            //	Range
                        $sheet1 = $sheet2 = '';
                        ...
                        if ($sheet1 == $sheet2) {
                            ...
                            foreach ($oData as $oDatum) {
                                $oCR
                                    = PHPExcel_Cell::coordinateFromString($oDatum);
                                $oCol[]
                                    = PHPExcel_Cell::columnIndexFromString($oCR[0])
                                      - 1;
                                $oRow[] = $oCR[1];
                            }
                            $cellRef
                            ...
                            $stack->push('Cell Reference', $cellValue,
                                $cellRef);
                        } else {
                            $stack->push('Error',
                                PHPExcel_Calculation_Functions::REF(), null);
                        }

                        break;
                    case '+'    :            //	Addition
                        $this->_executeNumericBinaryOperation($cellID,
                            $operand1, $operand2, $token, 'plusEquals', $stack);
                        break;
                    case '-'    :            //	Subtraction
                        $this->_executeNumericBinaryOperation($cellID,
                            $operand1, $operand2, $token, 'minusEquals',
                            $stack);
                        break;
                    case '*'    :            //	Multiplication
                        $this->_executeNumericBinaryOperation($cellID,
                            $operand1, $operand2, $token, 'arrayTimesEquals',
                            $stack);
                        break;
                    case '/'    :            //	Division
                        $this->_executeNumericBinaryOperation($cellID,
                            $operand1, $operand2, $token, 'arrayRightDivide',
                            $stack);
                        break;
                    case '^'    :            //	Exponential
                        $this->_executeNumericBinaryOperation($cellID,
                            $operand1, $operand2, $token, 'power', $stack);
                        break;
                    case '&'    :            //	Concatenation
                        ...
                        if ((is_array($operand1)) || (is_array($operand2))) {
                            ...
                            try {
                                ...
                                $matrix
                                    = new PHPExcel_Shared_JAMA_Matrix($operand1);
                                //	Perform the required operation against the operand 1 matrix, passing in operand 2
                                $matrixResult = $matrix->concat($operand2);
                                $result = $matrixResult->getArray();
                            } catch (PHPExcel_Exception $ex) {
                                $this->_debugLog->writeDebugLog('JAMA Matrix Exception: ',
                                    $ex->getMessage());
                                $result = '#VALUE!';
                            }
                        } else {
                            ...
                            foreach ($rowIntersect[$row] as $col => $data) {
                                $oCol[]
                                    = PHPExcel_Cell::columnIndexFromString($col)
                                      - 1;
                                $cellIntersect[$row]
                                    = array_intersect_key($operand1[$row],
                                    $operand2[$row]);
                            }
                        }
                        $cellRef
...
//					echo 'Token is a negation operator<br />';
                    $this->_debugLog->writeDebugLog('Evaluating Negation of ',
                        $this->_showValue($arg));
                    $multiplier = -1;
                } else {
//					echo 'Token is a percentile operator<br />';
                    $this->_debugLog->writeDebugLog('Evaluating Percentile of ',
                        $this->_showValue($arg));
                    $multiplier = 0.01;
                }
                if (is_array($arg)) {
                    ...
                    $this->_executeNumericBinaryOperation($cellID, $multiplier,
...
//						We can't access the range, so return a REF error
                        ...
                        $cellRef = $matches[6] . $matches[7] . ':' . $matches[9]
                                   . $matches[10];
                        if ($matches[2] > '') {
...
//							echo '$cellRef='.$cellRef.' in worksheet '.$matches[2].'<br />';
                            $this->_debugLog->writeDebugLog('Evaluating Cell Range ',
                            ...
                            $this->_debugLog->writeDebugLog('Evaluation Result for cells ',
...
//							echo '$cellRef='.$cellRef.' in worksheet '.$matches[2].'<br />';
                            $this->_debugLog->writeDebugLog('Evaluating Cell ',
                                $cellRef, ' in worksheet ', $matches[2]);
                            if ($pCellParent !== null) {
                                $cellSheet
                                    = $this->_workbook->getSheetByName($matches[2]);
                            ...
                            $this->_debugLog->writeDebugLog('Evaluation Result for cell ',
                                $cellRef, ' in worksheet ', $matches[2], ' is ',
                                $this->_showTypeDetails($cellValue));
//							$cellRef = $matches[2].'!'.$cellRef;
                        } else {
//							echo '$cellRef='.$cellRef.' in current worksheet<br />';
            ...
            } elseif (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION
                                 . '$/i', $token, $matches)
            ) {
//				echo 'Token is a function<br />';
                ...
                if ($functionName != 'MKMATRIX') {
                    $this->_debugLog->writeDebugLog('Evaluating Function ',
                        self::_localeFunc($functionName), '() with ',
                ...
                if ((isset(self::$_PHPExcelFunctions[$functionName]))
                    ...
                    if (isset(self::$_PHPExcelFunctions[$functionName])) {
                        $functionCall
                            = self::$_PHPExcelFunctions[$functionName]['functionCall'];
                        $passByReference
                            = isset(self::$_PHPExcelFunctions[$functionName]['passByReference']);
                        $passCellReference
                            = isset(self::$_PHPExcelFunctions[$functionName]['passCellReference']);
                    } elseif (isset(self::$_controlFunctions[$functionName])) {
                        $functionCall
                            = self::$_controlFunctions[$functionName]['functionCall'];
                        $passByReference
                            = isset(self::$_controlFunctions[$functionName]['passByReference']);
                        $passCellReference
                            = isset(self::$_controlFunctions[$functionName]['passCellReference']);
                    }
                    ...
                    for ($i = 0; $i < $argCount; ++$i) {
                        ...
                        if (($passByReference)
                            ...
                            if ($arg['reference'] === null) {
                                ...
                                if ($functionName != 'MKMATRIX') {
                                    $argArrayVals[]
                                        = $this->_showValue($cellID);
                                }
                            } else {
                                $args[] = $arg['reference'];
                                if ($functionName != 'MKMATRIX') {
                                    $argArrayVals[]
                                        = $this->_showValue($arg['reference']);
                                }
                            }
                        } else {
...
//					} else {
                    ...
                    if (strpos($functionCall, '::') !== false) {
                        $result = call_user_func_array(explode('::',
                            $functionCall), $args);
                    } else {
                        foreach ($args as &$arg) {
                            $arg
                                = PHPExcel_Calculation_Functions::flattenSingleValue($arg);
                        }
                        unset($arg);
...
//					}
                    if ($functionName != 'MKMATRIX') {
                        $this->_debugLog->writeDebugLog('Evaluation Result for ',
                            self::_localeFunc($functionName),
                            '() function call is ',
                            $this->_showTypeDetails($result));
                    }
...
//					echo 'Token is a PHPExcel constant: '.$excelConstant.'<br />';
                    $stack->push('Constant Value',
                        self::$_ExcelConstants[$excelConstant]);
                    $this->_debugLog->writeDebugLog('Evaluating Constant ',
                        $excelConstant, ' as ',
...
//					echo 'Token is a number, boolean, string, null or an Excel error<br />';
                ...
                } elseif (preg_match('/^' . self::CALCULATION_REGEXP_NAMEDRANGE
                                     . '$/i', $token, $matches)
                ) {
//					echo 'Token is a named range<br />';
                    $namedRange = $matches[6];
//					echo 'Named Range is '.$namedRange.'<br />';
                    $this->_debugLog->writeDebugLog('Evaluating Named Range ',
                        $namedRange);
                    $cellValue = $this->extractNamedRange($namedRange,
                        ((null !== $pCell) ? $pCellWorksheet : null), false);
                    $pCell->attach($pCellParent);
                    $this->_debugLog->writeDebugLog('Evaluation Result for named range ',
                        $namedRange, ' is ',
                        $this->_showTypeDetails($cellValue));
        ...
        if (is_string($operand)) {
            ...
            if ( ! is_numeric($operand)) {
                ...
                if ($operand > '' && $operand{0} == '#') {
                    ...
                    $this->_debugLog->writeDebugLog('Evaluation Result is ',
                        $this->_showTypeDetails($operand));

                    return false;
                } elseif ( ! PHPExcel_Shared_String::convertToNumberIfFraction($operand)) {
                    //	If not a numeric or a fraction, then it's a text string, and so can't be used in mathematical binary operations
                    $stack->push('Value', '#VALUE!');
                    $this->_debugLog->writeDebugLog('Evaluation Result is a ',
                        $this->_showTypeDetails('#VALUE!'));
        ...
        if ((is_array($operand1)) || (is_array($operand2))) {
            ...
            if ((is_array($operand1)) && ( ! is_array($operand2))) {
                foreach ($operand1 as $x => $operandData) {
                    $this->_debugLog->writeDebugLog('Evaluating Comparison ',
                        $this->_showValue($operandData), ' ', $operation, ' ',
                        $this->_showValue($operand2));
                    $this->_executeBinaryComparisonOperation($cellID,
                ...
                foreach ($operand2 as $x => $operandData) {
                    $this->_debugLog->writeDebugLog('Evaluating Comparison ',
                        $this->_showValue($operand1), ' ', $operation, ' ',
                        $this->_showValue($operandData));
                    $this->_executeBinaryComparisonOperation($cellID, $operand1,
                        $operandData, $operation, $stack);
                    $r = $stack->pop();
                    $result[$x] = $r['value'];
                }
            } else {
            ...
            $this->_debugLog->writeDebugLog('Comparison Evaluation Result is ',
                $this->_showTypeDetails($result));
        ...
        switch ($operation) {
            ...
            case '>':
                if ($useLowercaseFirstComparison) {
                    $result = $this->strcmpLowercaseFirst($operand1, $operand2)
                              > 0;
                } else {
...
//				echo 'Adjusted Range reference is '.$pRange,PHP_EOL;
            ...
            // Extract range
            $aReferences
                = PHPExcel_Cell::extractAllCellReferencesInRange($pRange);
...
//				echo 'Named Range '.$pRange.' (';
                ...
                if (ctype_alpha($splitRange[0][0])) {
                    $pRange = $splitRange[0][0] . '1:' . $splitRange[0][1]
        ...
        foreach (self::$_PHPExcelFunctions as $functionName => $function) {
            if ($function['functionCall']
                ...
                $returnValue[$functionName]
                    = new PHPExcel_Calculation_Function($function['category'],
                    $functionName,
