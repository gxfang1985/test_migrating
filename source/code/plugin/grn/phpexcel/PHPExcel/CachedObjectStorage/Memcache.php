    ...
    implements PHPExcel_CachedObjectStorage_ICache
{
    ...
    protected function _storeData()
    {
        if ($this->_currentCellIsDirty && ! empty($this->_currentObjectID)) {
            ...
            if ( ! $this->_memcache->replace($this->_cachePrefix
                                             . $this->_currentObjectID
                                             . '.cache', $obj, null,
                $this->_cacheTime)
            ) {
                if ( ! $this->_memcache->add($this->_cachePrefix
                                             . $this->_currentObjectID
                                             . '.cache', $obj, null,
                    $this->_cacheTime)
                ) {
    ...
    public function isDataSet($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            $success = $this->_memcache->get($this->_cachePrefix . $pCoord
                                             . '.cache');
            if ($success === false) {
                //	Entry no longer exists in Memcache, so clear it from the cache array
                parent::deleteCacheData($pCoord);
                throw new PHPExcel_Exception('Cell entry ' . $pCoord
    ...
    public function getCacheData($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            $obj = $this->_memcache->get($this->_cachePrefix . $pCoord
                                         . '.cache');
            if ($obj === false) {
                //	Entry no longer exists in Memcache, so clear it from the cache array
                parent::deleteCacheData($pCoord);
    ...
    public function copyCellCollection(PHPExcel_Worksheet $parent)
    {
        ...
        foreach ($cacheList as $cellID) {
            if ($cellID != $this->_currentObjectID) {
                $obj = $this->_memcache->get($this->_cachePrefix . $cellID
                                             . '.cache');
                if ($obj === false) {
                    //	Entry no longer exists in Memcache, so clear it from the cache array
                    parent::deleteCacheData($cellID);
                    throw new PHPExcel_Exception('Cell entry ' . $cellID
                                                 . ' no longer exists in MemCache');
                }
                if ( ! $this->_memcache->add($newCachePrefix . $cellID
                                             . '.cache', $obj, null,
                    $this->_cacheTime)
                ) {
                    $this->__destruct();
    ...
    public function __construct(PHPExcel_Worksheet $parent, $arguments)
    {
        ...
        if (is_null($this->_cachePrefix)) {
            ...
            if ( ! $this->_memcache->addServer($memcacheServer, $memcachePort,
                false, 50, 5, 5, true, [$this, 'failureCallback'])
            ) {
                throw new PHPExcel_Exception('Could not connect to MemCache server at '
                                             . $memcacheServer . ':'
                                             . $memcachePort);
            }
