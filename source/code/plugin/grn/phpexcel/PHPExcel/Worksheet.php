...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    public function __construct(PHPExcel $pParent = null, $pTitle = 'Worksheet')
    {
        ...
        $this->_cellCollection
            = PHPExcel_CachedObjectStorageFactory::getInstance($this);
        ...
        // Default column dimension
        $this->_defaultColumnDimension
            = new PHPExcel_Worksheet_ColumnDimension(null);
    ...
    public function calculateColumnWidths($calculateMergeCells = false)
    {
        ...
        if ( ! empty($autoSizes)) {
            ...
            foreach ($this->getMergeCells() as $cells) {
                foreach (
                    PHPExcel_Cell::extractAllCellReferencesInRange($cells) as
                    $cellReference
                ) {
                ...
                while ($this->_parent->sheetNameExists($pValue . ' ' . $i)) {
                    ...
                    if ($i == 10) {
                        if (PHPExcel_Shared_String::CountCharacters($pValue)
                            ...
                            $pValue = PHPExcel_Shared_String::Substring($pValue,
                                0, 28);
                        }
                    } elseif ($i == 100) {
                        if (PHPExcel_Shared_String::CountCharacters($pValue)
        ...
        if ($this->_parent) {
            ...
            if ($updateFormulaCellReferences) {
                PHPExcel_ReferenceHelper::getInstance()
                                        ->updateNamedFormulas($this->_parent,
                                            $oldTitle, $newTitle);
            }
        }
    ...
    public function setCellValueExplicit(
        ...
        // Set value
        $cell = $this->getCell(strtoupper($pCoordinate))
    ...
    public function getCell($pCoordinate = 'A1')
    {
        ...
        if (strpos($pCoordinate, '!') !== false) {
            $worksheetReference
                = PHPExcel_Worksheet::extractSheetTitle($pCoordinate, true);
        ...
        // Named range?
        if (( ! preg_match('/^'
                           . PHPExcel_Calculation::CALCULATION_REGEXP_CELLREF
                           . '$/i', $pCoordinate, $matches))
            && (preg_match('/^'
                           . PHPExcel_Calculation::CALCULATION_REGEXP_NAMEDRANGE
                           . '$/i', $pCoordinate, $matches))
        ) {
            $namedRange = PHPExcel_NamedRange::resolveRange($pCoordinate,
                $this);
            if ($namedRange !== null) {
    ...
    private function _createNewCell($pCoordinate)
    {
        ...
        $this->_cachedHighestRow = max($this->_cachedHighestRow,
            $aCoordinates[1]);
            ...
            if ($namedRange !== null) {
                ...
                if ($this->getHashCode() != $namedRange->getWorksheet()
                    ...
                    if ( ! $namedRange->getLocalOnly()) {
                        ...
                        throw new PHPExcel_Exception('Named range '
                                                     . $namedRange->getName()
    ...
    {
        return $this->cellExists(PHPExcel_Cell::stringFromColumnIndex($pColumn)
                                 . $pRow);
    }
    ...
    public function getRowDimension($pRow = 1, $create = true)
    {
        ...
        if ( ! isset($this->_rowDimensions[$pRow])) {
            ...
            $this->_rowDimensions[$pRow]
                = new PHPExcel_Worksheet_RowDimension($pRow);
    ...
    public function getColumnDimension($pColumn = 'A', $create = true)
    {
        ...
        if ( ! isset($this->_columnDimensions[$pColumn])) {
            ...
            $this->_columnDimensions[$pColumn]
    ...
    public function getStyleByColumnAndRow(
        ...
        return $this->getStyle(PHPExcel_Cell::stringFromColumnIndex($pColumn)
                               . $pRow);
    }
    ...
    public function duplicateStyle(
        ...
        $style = $pCellStyle->getIsSupervisor()
            ? $pCellStyle->getSharedComponent() : $pCellStyle;

        // Add the style to the workbook if necessary
        $workbook = $this->_parent;
        if ($existingStyle
            = $this->_parent->getCellXfByHashCode($pCellStyle->getHashCode())
        ) {
        ...
        for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col) {
            for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++$row) {
                $this->getCell(PHPExcel_Cell::stringFromColumnIndex($col - 1)
    ...
    public function duplicateConditionalStyle(
        ...
        for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col) {
            for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++$row) {
                $this->setConditionalStyles(PHPExcel_Cell::stringFromColumnIndex($col
                                                                                 - 1)
                                            . $row, $pCellStyle);
            }
        }
    ...
    public function setBreakByColumnAndRow(
        ...
        return $this->setBreak(PHPExcel_Cell::stringFromColumnIndex($pColumn)
                               . $pRow, $pBreak);
    }
    ...
    public function unmergeCells($pRange = 'A1:A1')
    {
        ...
        if (strpos($pRange, ':') !== false) {
            if (isset($this->_mergeCells[$pRange])) {
                ...
                throw new PHPExcel_Exception('Cell range ' . $pRange
                                             . ' not known as merged.');
            }
        } else {
    ...
    public function protectCells(
        ...
        if ( ! $pAlreadyHashed) {
            $pPassword
                = PHPExcel_Shared_PasswordHasher::hashPassword($pPassword);
        }
    ...
    {
        return $this->freezePane(PHPExcel_Cell::stringFromColumnIndex($pColumn)
                                 . $pRow);
    }
    ...
    public function insertNewRowBefore($pBefore = 1, $pNumRows = 1)
    {
        if ($pBefore >= 1) {
            ...
            $objReferenceHelper->insertNewBefore('A' . $pBefore, 0, $pNumRows,
                $this);
        } else {
    ...
    public function insertNewColumnBefore($pBefore = 'A', $pNumCols = 1)
    {
        if ( ! is_numeric($pBefore)) {
            $objReferenceHelper = PHPExcel_ReferenceHelper::getInstance();
            $objReferenceHelper->insertNewBefore($pBefore . '1', $pNumCols, 0,
                $this);
        } else {
    ...
    {
        if ($pBefore >= 0) {
            return $this->insertNewColumnBefore(PHPExcel_Cell::stringFromColumnIndex($pBefore),
                $pNumCols);
        } else {
    ...
    public function removeRow($pRow = 1, $pNumRows = 1)
    {
        if ($pRow >= 1) {
            ...
            $objReferenceHelper = PHPExcel_ReferenceHelper::getInstance();
            $objReferenceHelper->insertNewBefore('A' . ($pRow + $pNumRows), 0,
                -$pNumRows, $this);
    ...
    public function removeColumn($pColumn = 'A', $pNumCols = 1)
    {
        if ( ! is_numeric($pColumn)) {
            ...
            $pColumn
                = PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::columnIndexFromString($pColumn)
                                                       - 1
                                                       + $pNumCols);
            $objReferenceHelper = PHPExcel_ReferenceHelper::getInstance();
            $objReferenceHelper->insertNewBefore($pColumn . '1', -$pNumCols, 0,
                $this);
            for ($c = 0; $c < $pNumCols; ++$c) {
                $this->getCellCacheController()->removeColumn($highestColumn);
                $highestColumn
                    = PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::columnIndexFromString($highestColumn)
                                                           - 2);
            }
        } else {
            throw new PHPExcel_Exception("Column references should not be numeric.");
    ...
    {
        if ($pColumn >= 0) {
            return $this->removeColumn(PHPExcel_Cell::stringFromColumnIndex($pColumn),
                $pNumCols);
        } else {
    ...
    {
        return $this->getComment(PHPExcel_Cell::stringFromColumnIndex($pColumn)
                                 . $pRow);
    }
    ...
    public function setSelectedCells($pCoordinate = 'A1')
    {
        ...
        $pCoordinate = preg_replace('/^([0-9]+):([0-9]+)$/', 'A${1}:XFD${2}',
            $pCoordinate);
    ...
    {
        return $this->setSelectedCells(PHPExcel_Cell::stringFromColumnIndex($pColumn)
                                       . $pRow);
    }
    ...
    public function fromArray(
        ...
        if (is_array($source)) {
            ...
            list ($startColumn, $startRow)
                = PHPExcel_Cell::coordinateFromString($startCell);
    ...
    public function namedRangeToArray(
        ...
        throw new PHPExcel_Exception('Named Range ' . $pNamedRange
                                     . ' does not exist.');
    }
...
//
        ...
        $highestColumn
            = PHPExcel_Cell::columnIndexFromString($colRow['column']);

        // Loop through column dimensions
        foreach ($this->_columnDimensions as $dimension) {
            $highestColumn = max($highestColumn,
                PHPExcel_Cell::columnIndexFromString($dimension->getColumnIndex()));
        }
        ...
        if ($highestColumn < 0) {
            ...
            $this->_cachedHighestColumn
                = PHPExcel_Cell::stringFromColumnIndex(--$highestColumn);
        }
    ...
    public function getHashCode()
    {
        if ($this->_dirty) {
            $this->_hash = md5($this->_title .
                               $this->_autoFilter .
                               ($this->_protection->isProtectionEnabled() ? 't'
                                   : 'f') .
                               __CLASS__
            );
            $this->_dirty = false;
    ...
    public function getHyperlink($pCellCoordinate = 'A1')
    {
        ...
        $this->_hyperlinkCollection[$pCellCoordinate]
    ...
    public function getDataValidation($pCellCoordinate = 'A1')
    {
        ...
        $this->_dataValidationCollection[$pCellCoordinate]
    ...
    public function shrinkRangeToFit($range)
    {
        ...
        foreach ($rangeBlocks as &$rangeSet) {
            ...
            if (PHPExcel_Cell::columnIndexFromString($rangeBoundaries[0][0])
                > $maxCol
            ) {
                $rangeBoundaries[0][0]
            ...
            if (PHPExcel_Cell::columnIndexFromString($rangeBoundaries[1][0])
                > $maxCol
            ) {
                $rangeBoundaries[1][0]
                    = PHPExcel_Cell::stringFromColumnIndex($maxCol);
    ...
    public function setCodeName($pValue = null)
    {
        ...
        if ($this->getParent()) {
            ...
            if ($this->getParent()->sheetCodeNameExists($pValue)) {
                ...
                $i = 1;
                while ($this->getParent()->sheetCodeNameExists($pValue . '_'
                                                               . $i)) {
                    ++$i;
                    if ($i == 10) {
                        if (PHPExcel_Shared_String::CountCharacters($pValue)
