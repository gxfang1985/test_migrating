 ...
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Cell_AdvancedValueBinder extends PHPExcel_Cell_DefaultValueBinder
    ...
    public function bindValue(PHPExcel_Cell $cell, $value = null)
    {
        ...
        if ($dataType === PHPExcel_Cell_DataType::TYPE_STRING
            ...
            if ($value == PHPExcel_Calculation::getTRUE()) {
                $cell->setValueExplicit(true,
                    PHPExcel_Cell_DataType::TYPE_BOOL);

                return true;
            } elseif ($value == PHPExcel_Calculation::getFALSE()) {
                $cell->setValueExplicit(false,
                    PHPExcel_Cell_DataType::TYPE_BOOL);
            ...
            if (preg_match('/^'
                           . PHPExcel_Calculation::CALCULATION_REGEXP_NUMBER
                           . '$/', $value)
            ) {
            ...
            if (preg_match('/^([+-]?)\s*([0-9]+)\s?\/\s*([0-9]+)$/', $value,
                ...
                return true;
            } elseif (preg_match('/^([+-]?)([0-9]*) +([0-9]*)\s?\/\s*([0-9]*)$/',
                $value, $matches)
            ) {
                // Convert value to number
            ...
            if (preg_match('/^' . preg_quote($currencyCode) . ' *(\d{1,3}('
                           . preg_quote($thousandsSeparator) . '\d{3})*|(\d+))('
                ...
                return true;
            } elseif (preg_match('/^\$ *(\d{1,3}(\,\d{3})*|(\d+))(\.\d{2})?$/',
                $value)
            ) {
                // Convert value to number
