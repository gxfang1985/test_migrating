...
class PHPExcel_Best_Fit
{
    ...
    protected function _calculateGoodnessOfFit(
        $sumX,
        $sumY,
        $sumX2,
        $sumY2,
        ...
        foreach ($this->_xValues as $xKey => $xValue) {
            ...
            $SSres += ($this->_yValues[$xKey] - $bestFitY)
                      * ($this->_yValues[$xKey] - $bestFitY);
            if ($const) {
                $SStot += ($this->_yValues[$xKey] - $meanY)
                          * ($this->_yValues[$xKey] - $meanY);
            } else {
                $SStot += $this->_yValues[$xKey] * $this->_yValues[$xKey];
            }
            $SScov += ($this->_xValues[$xKey] - $meanX)
                      * ($this->_yValues[$xKey] - $meanY);
            if ($const) {
                $SSsex += ($this->_xValues[$xKey] - $meanX)
                          * ($this->_xValues[$xKey] - $meanX);
            } else {
        ...
        $this->_correlation = ($this->_valueCount * $sumXY - $sumX * $sumY)
                              / sqrt(($this->_valueCount * $sumX2 - pow($sumX,
                        2)) * ($this->_valueCount * $sumY2 - pow($sumY, 2)));
        $this->_slopeSE = $this->_stdevOfResiduals / sqrt($SSsex);
        $this->_intersectSE = $this->_stdevOfResiduals * sqrt(1
                                                              / ($this->_valueCount
                                                                 - ($sumX
                                                                    * $sumX)
                                                                   / $sumX2));
        if ($this->_SSResiduals != 0.0) {
            if ($this->_DFResiduals == 0.0) {
                ...
                $this->_F = $this->_SSRegression / ($this->_SSResiduals
                                                    / $this->_DFResiduals);
            }
        } else {
            if ($this->_DFResiduals == 0.0) {
...
//		$this->_intersect = ($y_sum - ($this->_slope * $x_sum)) / $this->_valueCount;
        ...
        $this->_calculateGoodnessOfFit($x_sum, $y_sum, $xx_sum, $yy_sum,
