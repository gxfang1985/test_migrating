...
class PHPExcel_Shared_Font
{
     ...
     */
    public static function setAutoSizeMethod(
        $pValue = self::AUTOSIZE_METHOD_APPROX
    ) {
    ...
    public static function calculateColumnWidth(
        ...
        if (strpos($cellText, "\n") !== false) {
            ...
            foreach ($lineTexts as $lineText) {
                $lineWidths[] = self::calculateColumnWidth($font, $lineText,
                    $rotation = 0, $defaultFont);
            }
        ...
        if ( ! $approximate) {
            $columnWidthAdjust = ceil(self::getTextWidthPixelsExact('n', $font,
                    0) * 1.07);
            try {
                // Width of text in pixels excl. padding
                // and addition because Excel adds some padding, just use approx width of 'n' glyph
                $columnWidth = self::getTextWidthPixelsExact($cellText, $font,
        ...
        $columnWidth
            = PHPExcel_Shared_Drawing::pixelsToCellDimension($columnWidth,
            $defaultFont);
     ...
     */
    public static function getTextWidthPixelsExact(
        $text,
        PHPExcel_Style_Font $font,
        $rotation = 0
    ) {
...
//		$upperLeftCornerY  = $textBox[7];
        ...
        $textWidth = max($lowerRightCornerX - $upperLeftCornerX,
     ...
     */
    public static function getTextWidthPixelsApprox(
        $columnText,
        ...
        switch ($fontName) {
            case 'Calibri':
                ...
                $columnWidth = (int)(8.26
                                     * PHPExcel_Shared_String::CountCharacters($columnText));
                $columnWidth = $columnWidth * $fontSize
                               / 11; // extrapolate from font size
                break;
...
//				$columnWidth = (int) (7 * PHPExcel_Shared_String::CountCharacters($columnText));
    ...
    public static function getTrueTypeFontFileFromFont($font)
    {
        ...
        switch ($name) {
            ...
            case 'Liberation Sans':
                $fontFile = (
                $bold
                    ? ($italic ? self::LIBERATION_SANS_BOLD_ITALIC
            ...
            case 'Palatino Linotype':
                $fontFile = (
                $bold
                    ? ($italic ? self::PALATINO_LINOTYPE_BOLD_ITALIC
            ...
            case 'Times New Roman':
                $fontFile = (
                $bold
                    ? ($italic ? self::TIMES_NEW_ROMAN_BOLD_ITALIC
            ...
            default:
                throw new PHPExcel_Exception('Unknown font name "' . $name
     ...
     */
    public static function getDefaultColumnWidthByFont(
        PHPExcel_Style_Font $font,
        $pPixels = false
    ) {
