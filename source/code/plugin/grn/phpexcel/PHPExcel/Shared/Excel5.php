...
class PHPExcel_Shared_Excel5
{
    ...
    public static function sizeCol($sheet, $col = 'A')
    {
        ...
        if (isset($columnDimensions[$col])
            and $columnDimensions[$col]->getWidth() != -1
        ) {
            ...
            $pixelWidth
                = PHPExcel_Shared_Drawing::cellDimensionToPixels($width,
                $font);
            ...
            $pixelWidth
                = PHPExcel_Shared_Font::getDefaultColumnWidthByFont($font,
                true);
        }

        // now find the effective column width in pixels
        if (isset($columnDimensions[$col])
            and ! $columnDimensions[$col]->getVisible()
        ) {
    ...
    public static function sizeRow($sheet, $row = 1)
    {
        ...
        if (isset($rowDimensions[$row]) and $rowDimensions[$row]->getRowHeight()
                                            != -1
        ) {
            ...
            $pixelRowHeight = (int)ceil(4 * $rowHeight
                                        / 3); // here we assume Arial 10
            ...
            $pixelRowHeight
                = PHPExcel_Shared_Drawing::pointsToPixels($rowHeight);
            ...
            $pixelRowHeight
                = PHPExcel_Shared_Font::fontSizeToPixels($pointRowHeight);

        }

        // now find the effective row height in pixels
        if (isset($rowDimensions[$row])
            and ! $rowDimensions[$row]->getVisible()
        ) {
     ...
     * @return array
     */
    public static function oneAnchor2twoAnchor(
        $sheet,
        $coordinates,
        $offsetX,
        $offsetY,
        ...
        $y2 = ($height + 1) / self::sizeRow($sheet, $row_end + 1)
              * 256; // Distance to bottom of object

        $startCoordinates = PHPExcel_Cell::stringFromColumnIndex($col_start)
                            . ($row_start + 1);
        $endCoordinates = PHPExcel_Cell::stringFromColumnIndex($col_end)
                          . ($row_end + 1);
