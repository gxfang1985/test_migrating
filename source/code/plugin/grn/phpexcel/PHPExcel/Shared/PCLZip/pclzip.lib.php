...
class PclZip
{
    ...
    function create($p_filelist)
    {
        ...
        if ($v_size > 1) {
            ...
            if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
                ...
                // ----- Parse the options
                $v_result = $this->privParseOptions($v_arg_list, $v_size,
                    $v_options,
                    [
    ...
    function add($p_filelist)
    {
        ...
        // ----- Call the create fct
        $v_result = $this->privAdd($v_filedescr_list, $p_result_list,
            $v_options);
        if ($v_result != 1) {
            return 0;
        }
...
//    $v_path = "./";
    ...
    function properties()
    {
        ...
        if (@is_file($this->zipname)) {
            ...
            $v_central_dir = [];
            if (($v_result = $this->privReadEndCentralDir($v_central_dir))
                != 1
            ) {
                $this->privSwapBackMagicQuotes();
...
// --------------------------------------------------------------------------------
    ...
    function privParseOptions(
        ...
        while ($i < $p_size) {
            ...
            if ( ! isset($v_requested_options[$p_options_list[$i]])) {
                // ----- Error log
                PclZip::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER,
                    "Invalid optional parameter '" . $p_options_list[$i]
                    . "' for this method");
            ...
            switch ($p_options_list[$i]) {
                ...
                case PCLZIP_OPT_ADD_PATH :
                    ...
                    $v_result_list[$p_options_list[$i]]
                        = PclZipUtilTranslateWinPath($p_options_list[$i + 1],
                        false);
                    $i++;
                    break;
                ...
                case PCLZIP_OPT_EXTRACT_DIR_RESTRICTION :
                    ...
                    if (is_string($p_options_list[$i + 1])
                        ...
                        $v_result_list[$p_options_list[$i]]
                            = PclZipUtilTranslateWinPath($p_options_list[$i
                                                                         + 1],
                            false);
                        $i++;
                    } else {
                ...
                case PCLZIP_OPT_BY_NAME :
                    ...
                    if (is_string($p_options_list[$i + 1])) {
                        ...
                        $v_result_list[$p_options_list[$i]] = $p_options_list[$i
                                                                              + 1];
                    } else {
                        // ----- Error log
                        PclZip::privErrorLog(PCLZIP_ERR_INVALID_OPTION_VALUE,
                ...
                case PCLZIP_OPT_BY_INDEX :
                    ...
                    if (is_string($p_options_list[$i + 1])) {
                        ...
                        $v_work_list[0] = $p_options_list[$i + 1] . '-'
                                          . $p_options_list[$i + 1];
                    } elseif (is_array($p_options_list[$i + 1])) {
        ...
        foreach ($p_file_list as $v_key => $v_value) {
            ...
            switch ($v_key) {
                case PCLZIP_ATT_FILE_NAME :
                    ...
                    $p_filedescr['filename']
                        = PclZipUtilPathReduction($v_value);
    ...
    function privFileDescrExpand(&$p_filedescr_list, &$p_options)
    {
        ...
        for ($i = 0; $i < sizeof($p_filedescr_list); $i++) {
            ...
            // ----- Reduce the filename
            $v_descr['filename']
                = PclZipUtilTranslateWinPath($v_descr['filename'], false);
            $v_descr['filename']
            ...
            if ($v_descr['type'] == 'folder') {
                ...
                if ($v_dirlist_nb != 0) {
                    // ----- Expand
                    if (($v_result
                            = $this->privFileDescrExpand($v_dirlist_descr,
                            $p_options)) != 1
                    ) {
                        return $v_result;
                    }
                    ...
                    $v_result_list = array_merge($v_result_list,
                        $v_dirlist_descr);
                } else {
                }
    ...
    function privAdd($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        if (($v_zip_temp_fd = @fopen($v_zip_temp_name, 'wb')) == 0) {
            ...
            PclZip::privErrorLog(PCLZIP_ERR_READ_OPEN_FAIL,
                'Unable to open temporary file \'' . $v_zip_temp_name
        ...
        // ----- Add the files
        $v_header_list = [];
        if (($v_result = $this->privAddFileList($p_filedescr_list,
                $v_header_list, $p_options)) != 1
        ) {
        ...
        for ($i = 0, $v_count = 0; $i < sizeof($v_header_list); $i++) {
            ...
            if ($v_header_list[$i]['status'] == 'ok') {
                if (($v_result
                        = $this->privWriteCentralFileHeader($v_header_list[$i]))
                    != 1
                ) {
                    fclose($v_zip_temp_fd);
                    $this->privCloseFd();
        ...
        if (($v_result = $this->privWriteCentralHeader($v_count
    ...
    function privOpenFd($p_mode)
    {
        ...
        if (($this->zip_fd = @fopen($this->zipname, $p_mode)) == 0) {
            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_READ_OPEN_FAIL,
                'Unable to open archive \'' . $this->zipname . '\' in '
                . $p_mode . ' mode');
    ...
    function privAddFileList($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        for ($j = 0; ($j < sizeof($p_filedescr_list)) && ($v_result == 1); $j++) {
            ...
            $p_filedescr_list[$j]['filename']
                = PclZipUtilTranslateWinPath($p_filedescr_list[$j]['filename'],
                false);
            ...
            if (($p_filedescr_list[$j]['type'] != 'virtual_file')
                ...
                PclZip::privErrorLog(PCLZIP_ERR_MISSING_FILE,
                    "File '" . $p_filedescr_list[$j]['filename']
                    . "' does not exist");
        ...
        if ($p_header['status'] == 'ok') {
            ...
            if ($p_filedescr['type'] == 'file') {
                ...
                if (( ! isset($p_options[PCLZIP_OPT_TEMP_FILE_OFF]))
                    ...
                    $v_result = $this->privAddFileUsingTempFile($p_filedescr,
                        $p_header, $p_options);
                    ...
                    if (($v_result = $this->privWriteFileHeader($p_header))
                        != 1
                    ) {
                        @fclose($v_file);
            ...
            elseif ($p_filedescr['type'] == 'folder') {
                ...
                $p_header['external']
                    = 0x00000010;   // Value for a folder : to be checked
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_ADD].'(PCLZIP_CB_POST_ADD, $v_local_header);');
            $v_result = $p_options[PCLZIP_CB_POST_ADD](PCLZIP_CB_POST_ADD,
                $v_local_header);
            if ($v_result == 0) {
                // ----- Ignored
                $v_result = 1;
    ...
    function privAddFileUsingTempFile($p_filedescr, &$p_header, &$p_options)
    {
        ...
        if (($v_file_compressed = @gzopen($v_gzip_temp_name, "wb")) == 0) {
            ...
            PclZip::privErrorLog(PCLZIP_ERR_WRITE_OPEN_FAIL,
                'Unable to open temporary file \'' . $v_gzip_temp_name
                . '\' in binary write mode');
        ...
        if (filesize($v_gzip_temp_name) < 18) {
            PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT,
                'gzip temporary file \'' . $v_gzip_temp_name
        ...
        $v_data_header = unpack('a1id1/a1id2/a1cm/a1flag/Vmtime/a1xfl/a1os',
            $v_binary_data);
    ...
    function privCalculateStoredFilename(&$p_filedescr, &$p_options)
    {
        ...
        if (isset($p_filedescr['new_full_name'])) {
            ...
            $v_stored_filename
                = PclZipUtilTranslateWinPath($p_filedescr['new_full_name']);
    ...
    function privWriteCentralFileHeader(&$p_header)
    {
        ...
        if ($p_header['comment_len'] != 0) {
            fputs($this->zip_fd, $p_header['comment'],
    ...
    function privList(&$p_list)
    {
        ...
        if (@fseek($this->zip_fd, $v_central_dir['offset'])) {
            ...
            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_INVALID_ARCHIVE_ZIP,
        ...
        for ($i = 0; $i < $v_central_dir['entries']; $i++) {
            ...
            if (($v_result = $this->privReadCentralFileHeader($v_header))
                != 1
            ) {
                $this->privSwapBackMagicQuotes();
    ...
    function privConvertHeader2FileInfo($p_header, &$p_info)
    {
        ...
        $v_temp_path
            = PclZipUtilPathReduction($p_header['stored_filename']);
    ...
    function privExtractByRule(
        ...
        for ($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME]))
                ...
                for (
                    $j = 0; ($j < sizeof($p_options[PCLZIP_OPT_BY_NAME]))
                            && ( ! $v_extract); $j++
                ) {
      ...
      */
            ...
            elseif ((isset($p_options[PCLZIP_OPT_BY_INDEX]))
                ...
                for (
                    $j = $j_start;
                    ($j < sizeof($p_options[PCLZIP_OPT_BY_INDEX]))
                    && ( ! $v_extract); $j++
                ) {
            ...
            if ($v_extract) {
                ...
                if ($p_options[PCLZIP_OPT_EXTRACT_AS_STRING]) {
                    ...
                    // ----- Extracting the file
                    $v_result1 = $this->privExtractFileAsString($v_header,
                        $v_string, $p_options);
                    if ($v_result1 < 1) {
                        $this->privCloseFd();
                        $this->privSwapBackMagicQuotes();
                    ...
                    // ----- Get the only interesting attributes
                    if (($v_result
                            = $this->privConvertHeader2FileInfo($v_header,
                ...
                elseif ((isset($p_options[PCLZIP_OPT_EXTRACT_IN_OUTPUT]))
                    ...
                    $v_result1 = $this->privExtractFileInOutput($v_header,
                        $p_options);
                    if ($v_result1 < 1) {
                        $this->privCloseFd();
                        $this->privSwapBackMagicQuotes();
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_PRE_EXTRACT].'(PCLZIP_CB_PRE_EXTRACT, $v_local_header);');
            $v_result = $p_options[PCLZIP_CB_PRE_EXTRACT](PCLZIP_CB_PRE_EXTRACT,
                $v_local_header);
            if ($v_result == 0) {
                // ----- Change the file status
                ...
                elseif (filemtime($p_entry['filename']) > $p_entry['mtime']) {
                    ...
                    if ((isset($p_options[PCLZIP_OPT_REPLACE_NEWER]))
                        ...
                        if ((isset($p_options[PCLZIP_OPT_STOP_ON_ERROR]))
                            ...
                            PclZip::privErrorLog(PCLZIP_ERR_WRITE_OPEN_FAIL,
                                "Newer version of '" . $p_entry['filename']
                                . "' exists "
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if ( ! (($p_entry['external'] & 0x00000010) == 0x00000010)) {
                ...
                if ($p_entry['compression'] == 0) {
                    ...
                    while ($v_size != 0) {
                        $v_read_size = ($v_size < PCLZIP_READ_BLOCK_SIZE
            ...
            */
                    ...
                    if (( ! isset($p_options[PCLZIP_OPT_TEMP_FILE_OFF]))
                        ...
                        $v_result
                            = $this->privExtractFileUsingTempFile($p_entry,
                            $p_options);
                        if ($v_result < PCLZIP_ERR_NO_ERROR) {
                            return $v_result;
                        }
                        ...
                        @fwrite($v_dest_file, $v_file_content,
                            $p_entry['size']);
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_EXTRACT].'(PCLZIP_CB_POST_EXTRACT, $v_local_header);');
    ...
    function privExtractFileUsingTempFile(&$p_entry, &$p_options)
    {
        ...
        $v_binary_data = pack('va1a1Va1a1', 0x8b1f,
            Chr($p_entry['compression']), Chr(0x00), time(), Chr(0x00), Chr(3));
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if ( ! (($p_entry['external'] & 0x00000010) == 0x00000010)) {
                ...
                if ($p_entry['compression'] == 0) {
                    ...
                    // ----- Reading the file
                    $p_string = @fread($this->zip_fd,
                        $p_entry['compressed_size']);
                } else {

                    // ----- Reading the file
                    $v_data = @fread($this->zip_fd,
                        $p_entry['compressed_size']);
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_EXTRACT].'(PCLZIP_CB_POST_EXTRACT, $v_local_header);');
    ...
    function privReadCentralFileHeader(&$p_header)
    {
        ...
        if ($p_header['filename_len'] != 0) {
            $p_header['filename'] = fread($this->zip_fd,
        ...
        // ----- Get comment
        if ($p_header['comment_len'] != 0) {
            $p_header['comment'] = fread($this->zip_fd,
                $p_header['comment_len']);
        } else {
            $p_header['comment'] = '';
        }
    ...
    function privDeleteByRule(&$p_result_list, &$p_options)
    {
        ...
        for ($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
            ...
            if (($v_result
                    = $this->privReadCentralFileHeader($v_header_list[$v_nb_extracted]))
                != 1
            ) {
                // ----- Close the zip file
                $this->privCloseFd();
                ...
                for (
                    ...
                    if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
                        ...
                        if ((strlen($v_header_list[$v_nb_extracted]['stored_filename'])
                             > strlen($p_options[PCLZIP_OPT_BY_NAME][$j]))
                            && (substr($v_header_list[$v_nb_extracted]['stored_filename'],
                                    0,
                                    strlen($p_options[PCLZIP_OPT_BY_NAME][$j]))
                                == $p_options[PCLZIP_OPT_BY_NAME][$j])
                        ) {
      ...
      */
        ...
        if ($v_nb_extracted > 0) {
            ...
            // ----- Creates a temporay file
            $v_zip_temp_name = PCLZIP_TEMPORARY_DIR . uniqid('pclzip-')
                               . '.tmp';
            ...
            for ($i = 0; $i < sizeof($v_header_list); $i++) {
                ...
                if (($v_result
                        = $v_temp_zip->privWriteFileHeader($v_header_list[$i]))
                    != 1
                ) {
                    // ----- Close the zip file
                    $this->privCloseFd();
            ...
            for ($i = 0; $i < sizeof($v_header_list); $i++) {
                // ----- Create the file header
                if (($v_result
                        = $v_temp_zip->privWriteCentralFileHeader($v_header_list[$i]))
                    != 1
                ) {
                    $v_temp_zip->privCloseFd();
                    $this->privCloseFd();
            ...
            // ----- Create the central dir footer
            if (($v_result
                    = $v_temp_zip->privWriteCentralHeader(sizeof($v_header_list),
                    $v_size, $v_offset, $v_comment)) != 1
            ) {
                // ----- Reset the file list
                unset($v_header_list);
    ...
    function privDirCheck($p_dir, $p_is_dir = false)
    {
        ...
        if ( ! @mkdir($p_dir, 0777)) {
            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_DIR_CREATE_FAIL,
    ...
    function privMerge(&$p_archive_to_add)
    {
        ...
        if (($v_result
                = $p_archive_to_add->privReadEndCentralDir($v_central_dir_to_add))
            != 1
        ) {
            $this->privCloseFd();
        ...
        // ----- Create the central dir footer
        if (($v_result = $this->privWriteCentralHeader($v_central_dir['entries']
    ...
    function privDuplicate($p_archive_filename)
    {
        ...
        if (($v_zip_temp_fd = @fopen($p_archive_filename, 'rb')) == 0) {
            ...
            PclZip::privErrorLog(PCLZIP_ERR_READ_OPEN_FAIL,
                'Unable to open archive file \'' . $p_archive_filename
                . '\' in binary write mode');
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    if (($p_path == '.')
        ...
        $p_path = PclZipUtilTranslateWinPath(getcwd(), false) . '/'
                  . substr($p_path, 1);
    }
