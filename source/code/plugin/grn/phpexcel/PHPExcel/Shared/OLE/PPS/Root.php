...
class PHPExcel_Shared_OLE_PPS_Root extends PHPExcel_Shared_OLE_PPS
{
    ...
    public function save($filename)
    {
        ...
        $this->_BIG_BLOCK_SIZE = pow(2,
            ((isset($this->_BIG_BLOCK_SIZE))
                ? self::_adjust2($this->_BIG_BLOCK_SIZE) : 9));
        $this->_SMALL_BLOCK_SIZE = pow(2,
            ((isset($this->_SMALL_BLOCK_SIZE))
                ? self::_adjust2($this->_SMALL_BLOCK_SIZE) : 6));
    ...
    public function _calcSize(&$raList)
    {
        ...
        for ($i = 0; $i < $iCount; ++$i) {
            if ($raList[$i]->Type == PHPExcel_Shared_OLE::OLE_PPS_TYPE_FILE) {
                ...
                if ($raList[$i]->Size
                    < PHPExcel_Shared_OLE::OLE_DATA_SIZE_SMALL
                ) {
        ...
        $iSlCnt = floor($this->_BIG_BLOCK_SIZE
                        / PHPExcel_Shared_OLE::OLE_LONG_INT_SIZE);
    ...
    public function _saveHeader($iSBDcnt, $iBBcnt, $iPPScnt)
    {
        ...
        // Calculate Basic Setting
        $iBlCnt = $this->_BIG_BLOCK_SIZE
                  / PHPExcel_Shared_OLE::OLE_LONG_INT_SIZE;
        $i1stBdL = ($this->_BIG_BLOCK_SIZE - 0x4C)
                   / PHPExcel_Shared_OLE::OLE_LONG_INT_SIZE;
        ...
        $iBdCnt = floor(($iAll + $iBdCntW) / $iBlCnt) + ((($iAllW + $iBdCntW)
                                                          % $iBlCnt) ? 1 : 0);
        ...
        if ($iBdCnt > $i1stBdL) {
            while (1) {
                ++$iBdExL;
                ++$iAllW;
                $iBdCntW = floor($iAllW / $iBlCnt) + (($iAllW % $iBlCnt) ? 1
                        : 0);
                ...
                if (($raList[$i]->Size
                    ...
                    if ($raList[$i]->Size % $this->_BIG_BLOCK_SIZE) {
                        fwrite($FILE, str_repeat("\x00",
                            $this->_BIG_BLOCK_SIZE - ($raList[$i]->Size
                                                      % $this->_BIG_BLOCK_SIZE)));
                    }
                    // Set For PPS
                    $raList[$i]->_StartBlock = $iStBlk;
                    $iStBlk += (floor($raList[$i]->Size
                                      / $this->_BIG_BLOCK_SIZE)
                                +
                                (($raList[$i]->Size
                                  % $this->_BIG_BLOCK_SIZE) ? 1
                                    : 0));
                }
    ...
    public function _makeSmallData(&$raList)
    {
            ...
            if ($raList[$i]->Type == PHPExcel_Shared_OLE::OLE_PPS_TYPE_FILE) {
                ...
                if ($raList[$i]->Size
                    ...
                    if ($raList[$i]->Size % $this->_SMALL_BLOCK_SIZE) {
                        $sRes .= str_repeat("\x00",
                            $this->_SMALL_BLOCK_SIZE - ($raList[$i]->Size
                                                        % $this->_SMALL_BLOCK_SIZE));
                    }
                    // Set for PPS
        ...
        $iSbCnt = floor($this->_BIG_BLOCK_SIZE
                        / PHPExcel_Shared_OLE::OLE_LONG_INT_SIZE);
        if ($iSmBlk % $iSbCnt) {
    ...
    public function _savePps(&$raList)
    {
        ...
        if ($iCnt % $iBCnt) {
            fwrite($this->_FILEH_, str_repeat("\x00",
                ($iBCnt - ($iCnt % $iBCnt))
                * PHPExcel_Shared_OLE::OLE_PPS_SIZE));
        }
    }
    ...
    public function _saveBbd($iSbdSize, $iBsize, $iPpsCnt)
    {
        ...
        $iBdCnt = floor(($iAll + $iBdCntW) / $iBbCnt) + ((($iAllW + $iBdCntW)
                                                          % $iBbCnt) ? 1 : 0);
        // Calculate BD count
        if ($iBdCnt > $i1stBdL) {
            while (1) {
                ++$iBdExL;
                ++$iAllW;
                $iBdCntW = floor($iAllW / $iBbCnt) + (($iAllW % $iBbCnt) ? 1
                        : 0);
