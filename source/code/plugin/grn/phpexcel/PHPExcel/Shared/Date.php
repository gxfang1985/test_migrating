...
class PHPExcel_Shared_Date
{
     ...
     */
    public static function ExcelToPHP(
        $dateValue = 0,
        ...
        if ($dateValue >= 1) {
            ...
            $secs = round($dateValue * 86400) - round($hours * 3600)
                    - round($mins * 60);
        ...
        $timezoneAdjustment = ($adjustToTimezone)
            ?
            PHPExcel_Shared_TimeZone::getTimezoneAdjustment($timezone,
                $returnValue)
            :
            0;
    ...
    public static function PHPToExcel(
        ...
        if ((is_object($dateValue)) && ($dateValue instanceof DateTime)) {
            $retValue = self::FormattedPHPToExcel($dateValue->format('Y'),
                $dateValue->format('m'), $dateValue->format('d'),
                $dateValue->format('H'), $dateValue->format('i'),
                $dateValue->format('s')
            );
        } elseif (is_numeric($dateValue)) {
            $retValue = self::FormattedPHPToExcel(date('Y', $dateValue),
                date('m', $dateValue), date('d', $dateValue),
                date('H', $dateValue), date('i', $dateValue),
                date('s', $dateValue)
            );
        }
     ...
     */
    public static function FormattedPHPToExcel(
        $year,
        $month,
        $day,
        $hours = 0,
        ...
        $excelDate = floor((146097 * $century) / 4) + floor((1461 * $decade)
                                                            / 4) + floor((153
                                                                          * $month
                                                                          + 2)
                                                                         / 5)
                     + $day + 1721119 - $my_excelBaseDate
                     + $excel1900isLeapYear;
    ...
    public static function isDateTimeFormatCode($pFormatCode = '')
    {
        ...
        if (preg_match('/(^|\])[^\[]*[' . self::$possibleDateFormatCharacters
                       . ']/i', $pFormatCode)
        ) {
    ...
    public static function stringToExcel($dateValue = '')
    {
        ...
        if ($dateValueNew === PHPExcel_Calculation_Functions::VALUE()) {
            ...
            if (strpos($dateValue, ':') !== false) {
                $timeValue
                    = PHPExcel_Calculation_DateTime::TIMEVALUE($dateValue);
