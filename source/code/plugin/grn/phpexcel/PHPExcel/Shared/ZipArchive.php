...
if ( ! defined('PCLZIP_TEMPORARY_DIR')) {
    define('PCLZIP_TEMPORARY_DIR',
        PHPExcel_Shared_File::sys_get_temp_dir() . DIRECTORY_SEPARATOR);
}
...
class PHPExcel_Shared_ZipArchive
{
    ...
    public function addFromString($localname, $contents)
    {
        ...
        $handle = fopen($this->_tempDir . '/' . $filenameParts["basename"],
            "wb");
        ...
        if ($res == 0) {
            throw new PHPExcel_Writer_Exception("Error zipping files : "
                                                . $this->_zip->errorInfo(true));
        }
    ...
    public function locateName($fileName)
    {
        ...
        for ($i = 0; $i < $listCount; ++$i) {
            if (strtolower($list[$i]["filename"]) == strtolower($fileName)
                || strtolower($list[$i]["stored_filename"])
                   == strtolower($fileName)
            ) {
                $list_index = $i;
                break;
    ...
    public function getFromName($fileName)
    {
        ...
        $extracted = "";
        if ($list_index != -1) {
            $extracted = $this->_zip->extractByIndex($list_index,
