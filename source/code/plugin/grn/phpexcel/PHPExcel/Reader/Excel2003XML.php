 ...
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_Excel2003XML extends PHPExcel_Reader_Abstract
    implements PHPExcel_Reader_IReader
{
    ...
    public function canRead($pFilename)
    {
        ...
        if (preg_match('/<?xml.*encoding=[\'"](.*?)[\'"].*?>/um', $data,
            $matches)
        ) {
...
//		echo 'Character Set is ',$this->_charSet,'<br />';
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        if ( ! $this->canRead($pFilename)) {
            throw new PHPExcel_Reader_Exception($pFilename
        ...
        $xml
            = simplexml_load_string($this->securityScan(file_get_contents($pFilename)),
        ...
        foreach ($xml_ss->Worksheet as $worksheet) {
            ...
            $worksheetNames[]
                = self::_convertStringEncoding((string)$worksheet_ss['Name'],
                $this->_charSet);
        }
    ...
    protected static function identifyFixedStyleValue(
        $styleList,
        &$styleAttributeValue
    ) {
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
    {
        ...
        if (isset($xml->DocumentProperties[0])) {
            foreach (
                ...
                switch ($propertyName) {
                    case 'Title' :
                        $docProps->setTitle(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                    case 'Subject' :
                        $docProps->setSubject(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                    case 'Author' :
                        $docProps->setCreator(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                    case 'Created' :
                        ...
                        break;
                    case 'LastAuthor' :
                        $docProps->setLastModifiedBy(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                    case 'LastSaved' :
                        ...
                        break;
                    case 'Company' :
                        $docProps->setCompany(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                    case 'Category' :
                        $docProps->setCategory(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                    case 'Manager' :
                        $docProps->setManager(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                    case 'Keywords' :
                        $docProps->setKeywords(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                    case 'Description' :
                        $docProps->setDescription(self::_convertStringEncoding($propertyValue,
                            $this->_charSet));
                        break;
                }
            }
        ...
        if (isset($xml->CustomDocumentProperties)) {
            foreach (
                $xml->CustomDocumentProperties[0] as $propertyName =>
                $propertyValue
            ) {
                $propertyAttributes
                ...
                switch ((string)$propertyAttributes) {
                    case 'string' :
                        $propertyType
                            = PHPExcel_DocumentProperties::PROPERTY_TYPE_STRING;
                        $propertyValue = trim($propertyValue);
                        break;
                    case 'boolean' :
                        $propertyType
                            = PHPExcel_DocumentProperties::PROPERTY_TYPE_BOOLEAN;
                        $propertyValue = (bool)$propertyValue;
                        break;
                    case 'integer' :
                        $propertyType
                            = PHPExcel_DocumentProperties::PROPERTY_TYPE_INTEGER;
                        $propertyValue = intval($propertyValue);
                        break;
                    case 'float' :
                        $propertyType
                            = PHPExcel_DocumentProperties::PROPERTY_TYPE_FLOAT;
                        $propertyValue = floatval($propertyValue);
                        break;
                    case 'dateTime.tz' :
                        $propertyType
                            = PHPExcel_DocumentProperties::PROPERTY_TYPE_DATE;
                        $propertyValue = strtotime(trim($propertyValue));
                        break;
                }
                $docProps->setCustomProperty($propertyName, $propertyValue,
                    $propertyType);
            }
        }
...
//				echo $styleType.'<br />';
                switch ($styleType) {
                    case 'Alignment' :
                        foreach (
                            $styleAttributes as $styleAttributeKey =>
                            $styleAttributeValue
                        ) {
//								echo $styleAttributeKey.' = '.$styleAttributeValue.'<br />';
                            ...
                            switch ($styleAttributeKey) {
                                case 'Vertical' :
                                    if (self::identifyFixedStyleValue($verticalAlignmentStyles,
                                        ...
                                        $this->_styles[$styleID]['alignment']['vertical']
                                            = $styleAttributeValue;
                                    }
                                    break;
                                case 'Horizontal' :
                    ...
                    case 'Borders' :
                        foreach ($styleData->Border as $borderStyle) {
                            ...
                            $thisBorder = [];
                            foreach (
                                $borderAttributes as $borderStyleKey =>
                                $borderStyleValue
                            ) {
...
//												$thisBorder['style'] = $borderStyleValue;
                                    ...
                                    case 'Position' :
                                        $borderPosition
                                            = strtolower($borderStyleValue);
                                        break;
                                    case 'Color' :
                                        $borderColour
                                            = substr($borderStyleValue, 1);
                            ...
                            if ( ! empty($thisBorder)) {
                                if (($borderPosition == 'left')
                                    ...
                                    $this->_styles[$styleID]['borders'][$borderPosition]
                                        = $thisBorder;
                                }
                            }
                        }
...
//								echo $styleAttributeKey.' = '.$styleAttributeValue.'<br />';
                            ...
                            switch ($styleAttributeKey) {
                                case 'FontName' :
                                    $this->_styles[$styleID]['font']['name']
                                        = $styleAttributeValue;
                                    break;
                                case 'Size' :
                                    $this->_styles[$styleID]['font']['size']
                                        = $styleAttributeValue;
                                    break;
                                case 'Color' :
                                    ...
                                    break;
                                case 'Bold' :
                                    $this->_styles[$styleID]['font']['bold']
                                        = true;
                                    break;
                                case 'Italic' :
                                    $this->_styles[$styleID]['font']['italic']
                                        = true;
                                    break;
                                case 'Underline' :
                                    if (self::identifyFixedStyleValue($underlineStyles,
                                        ...
                                        $this->_styles[$styleID]['font']['underline']
                                            = $styleAttributeValue;
                                    }
                                    break;
                            }
...
//								echo $styleAttributeKey.' = '.$styleAttributeValue.'<br />';
                            $styleAttributeValue = str_replace($fromFormats,
                                $toFormats, $styleAttributeValue);
                            ...
                            if ($styleAttributeValue > '') {
                                $this->_styles[$styleID]['numberformat']['code']
                                    = $styleAttributeValue;
                            }
                        }
                        break;
            ...
            if (isset($worksheet->Table->Column)) {
                foreach ($worksheet->Table->Column as $columnData) {
                    ...
                    if (isset($columnData_ss['Index'])) {
                        $columnID
                            = PHPExcel_Cell::stringFromColumnIndex($columnData_ss['Index']
                                                                   - 1);
                    }
...
//					echo '<b>Row '.$rowID.'</b><br />';
                    ...
                    foreach ($rowData->Cell as $cell) {
                        ...
                        if (isset($cell_ss['Index'])) {
                            $columnID
                                = PHPExcel_Cell::stringFromColumnIndex($cell_ss['Index']
                                                                       - 1);
                        }
                            ...
                            if (isset($cellData_ss['Type'])) {
                                ...
                                switch ($cellDataType) {
                                    ...
                                    */
                                    case 'String' :
                                        $cellValue
                                            = self::_convertStringEncoding($cellValue,
                                            $this->_charSet);
                                        $type
                                    ...
                                    case 'DateTime' :
                                        $type
                                            = PHPExcel_Cell_DataType::TYPE_NUMERIC;
                                        $cellValue
                                            = PHPExcel_Shared_Date::PHPToExcel(strtotime($cellValue));
                                        break;
                                    case 'Error' :
                                        $type
...
//								echo 'FORMULA<br />';
                                ...
                                if (substr($cellDataFormula, 0, 3) == 'of:') {
                                    $cellDataFormula = substr($cellDataFormula,
                                        3);
                                    ...
                                    foreach ($temp as &$value) {
                                        ...
                                        if ($key = ! $key) {
                                            ...
                                            foreach (
                                                ...
                                                $A1CellReference
                                                    = PHPExcel_Cell::stringFromColumnIndex($columnReference
                                                                                           - 1)
                                                      . $rowReference;
...
//
                            $objPHPExcel->getActiveSheet()->getCell($columnID
                                                                    . $rowID)
                                        ->setValueExplicit((($hasCalculatedValue)
                                            ? $cellDataFormula : $cellValue),
                                            $type);
                            if ($hasCalculatedValue) {
...
//							echo 'Annotation: ',$annotation,'<br />';
                            $objPHPExcel->getActiveSheet()->getComment($columnID
                                                                       . $rowID)
                                        ->setAuthor(self::_convertStringEncoding($author,
                                            $this->_charSet))
