 ...
 * @copyright    Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract
...
//		}
        ...
        if ($zip->open($pFilename) === true) {
            ...
            $rels
                = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip,
            ...
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    $xmlWorkbook = simplexml_load_string(
                        $this->securityScan($this->_getFromZipArchive($zip,
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        foreach ($rels->Relationship as $rel) {
            if ($rel["Type"]
                ...
                $relsWorkbook
                    = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip,
                    "$dir/_rels/" . basename($rel["Target"]) . ".rels")),
                    'SimpleXMLElement',
                    PHPExcel_Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/package/2006/relationships");
                ...
                if ($xmlWorkbook->sheets) {
                    ...
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        ...
                        $xml = new XMLReader();
                        $res = $xml->xml($this->securityScanFile('zip://'
                        ...
                        $tmpInfo['totalColumns'] = max($tmpInfo['totalColumns'],
                            $currCells);
                        $xml->close();

                        $tmpInfo['lastColumnIndex'] = $tmpInfo['totalColumns']
                                                      - 1;
...
//				echo 'Formula is ', $sharedFormulas[$instance]['formula'], PHP_EOL;
                ...
                $difference = [0, 0];
                $difference[0]
                    = PHPExcel_Cell::columnIndexFromString($current[0])
                      - PHPExcel_Cell::columnIndexFromString($master[0]);
                $difference[1] = $current[1] - $master[1];

                $value
                    = $this->_referenceHelper->updateFormulaReferences($sharedFormulas[$instance]['formula'],
                    'A1',
                    $difference[0],
                    $difference[1]
                );
//				echo 'Adjusted Formula is ', $value, PHP_EOL;
    ...
    public function load($pFilename)
    {
        ...
        foreach ($wbRels->Relationship as $rel) {
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
                    ...
                    $xmlTheme
                        = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip,
                        "xl/{$rel['Target']}")), 'SimpleXMLElement',
                        PHPExcel_Settings::getLibXmlLoaderOptions());
                    if (is_object($xmlTheme)) {
                        ...
                        self::$_theme
                            = new PHPExcel_Reader_Excel2007_Theme($themeName,
                    ...
                    if (is_object($xmlCore)) {
                        ...
                        foreach ($xmlCore as $xmlProperty) {
                            ...
                            if (isset($cellDataOfficeAttributes['name'])) {
                                $propertyName
                                    = (string)$cellDataOfficeAttributes['name'];
                                $cellDataOfficeChildren
                                    = $xmlProperty->children('http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes');
                                $attributeType
                                    = $cellDataOfficeChildren->getName();
                                $attributeValue
                                    = (string)$cellDataOfficeChildren->{$attributeType};
                                $attributeValue
                                    = PHPExcel_DocumentProperties::convertProperty($attributeValue,
                                    $attributeType);
                                $attributeType
                                    = PHPExcel_DocumentProperties::convertPropertyType($attributeType);
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    if ( ! is_null($macros)) {
                        ...
                        if ($macrosCode !== false) {
                            ...
                            $Certificate = $this->_getFromZipArchive($zip,
                            ...
                            foreach (
                                ...
                                if (intval($cellStyle['builtinId']) == 0) {
                                    if (isset($cellStyles[intval($cellStyle['xfId'])])) {
                                        ...
                                        self::_readStyle($style,
                                            $cellStyles[intval($cellStyle['xfId'])]);
                    ...
                    $sheetId
                        = 0; // keep track of new sheet id in final workbook
                    $oldSheetId
                        = -1; // keep track of old sheet id in final workbook
                    $countSkippedSheets
                        = 0; // keep track of number of skipped sheets
                    $mapSheetId
                    ...
                    if ($xmlWorkbook->sheets) {
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            if (isset($xmlSheet->sheetViews)
                                && isset($xmlSheet->sheetViews->sheetView)
                            ) {
                                ...
                                if (isset($xmlSheet->sheetViews->sheetView->pane)) {
                                    if (isset($xmlSheet->sheetViews->sheetView->pane['topLeftCell'])) {
                                        ...
                                        if (isset($xmlSheet->sheetViews->sheetView->pane['xSplit'])) {
                                            $xSplit = 1
                                                      + intval($xmlSheet->sheetViews->sheetView->pane['xSplit']);
                                        }

                                        if (isset($xmlSheet->sheetViews->sheetView->pane['ySplit'])) {
                                            $ySplit = 1
                                                      + intval($xmlSheet->sheetViews->sheetView->pane['ySplit']);
                                        }

                                        $docSheet->freezePaneByColumnAndRow($xSplit,
                                            $ySplit);
                                    }
                                }
                                ...
                                if (isset($xmlSheet->sheetViews->sheetView->selection)) {
                                    if (isset($xmlSheet->sheetViews->sheetView->selection['sqref'])) {
                                        $sqref
                                            = (string)$xmlSheet->sheetViews->sheetView->selection['sqref'];
                                        $sqref = explode(' ', $sqref);
                                        $sqref = $sqref[0];
                            ...
                            if (isset($xmlSheet->sheetPr)
                                && isset($xmlSheet->sheetPr->tabColor)
                            ) {
                            ...
                            if (isset($xmlSheet->sheetPr)
                                && isset($xmlSheet->sheetPr->outlinePr)
                            ) {
                            ...
                            if (isset($xmlSheet->sheetPr)
                                && isset($xmlSheet->sheetPr->pageSetUpPr)
                            ) {
                            ...
                            if (isset($xmlSheet->sheetFormatPr)) {
                                ...
                                if (isset($xmlSheet->sheetFormatPr['zeroHeight'])
                                    && ((string)$xmlSheet->sheetFormatPr['zeroHeight']
                                        == '1')
                                ) {
                                    $docSheet->getDefaultRowDimension()
                                             ->setZeroHeight(true);
                            ...
                            if (isset($xmlSheet->printOptions)
                                && ! $this->_readDataOnly
                            ) {
                            ...
                            $conditionals = [];
                            if ( ! $this->_readDataOnly && $xmlSheet
                                 && $xmlSheet->conditionalFormatting
                            ) {
                                foreach (
                                    $xmlSheet->conditionalFormatting as
                                    $conditional
                                ) {
                                ...
                                foreach ($conditionals as $ref => $cfRules) {
                                    ...
                                    $aReferences
                                        = PHPExcel_Cell::extractAllCellReferencesInRange($ref);
                            ...
                            if ( ! $this->_readDataOnly && $xmlSheet
                                ...
                                $docSheet->getProtection()
                                         ->setPassword((string)$xmlSheet->sheetProtection["password"],
                                             true);
                            ...
                            if ($xmlSheet && $xmlSheet->autoFilter
                                ...
                                $autoFilterRange
                                    = (string)$xmlSheet->autoFilter["ref"];
                                if (strpos($autoFilterRange, ':') !== false) {
                                    $autoFilter = $docSheet->getAutoFilter();
                                    $autoFilter->setRange($autoFilterRange);

                                    foreach (
                                        $xmlSheet->autoFilter->filterColumn as
                                        $filterColumn
                                    ) {
                                        $column
                                        ...
                                        if ($filterColumn->dynamicFilter) {
                                            ...
                                            foreach (
                                                ...
                                                if (isset($filterRule["val"])) {
                                                    $column->setAttribute('val',
                                                        (string)$filterRule["val"]);
                                                }
                                                if (isset($filterRule["maxVal"])) {
                                                    $column->setAttribute('maxVal',
                                                        (string)$filterRule["maxVal"]);
                                                }
                                            }
                                        }
                                        //	Check for dynamic filters
                            ...
                            if ($xmlSheet && $xmlSheet->mergeCells
                                && $xmlSheet->mergeCells->mergeCell
                                && ! $this->_readDataOnly
                            ) {
                                foreach (
                                    $xmlSheet->mergeCells->mergeCell as
                                    $mergeCell
                                ) {
                            ...
                            if ($xmlSheet && $xmlSheet->pageMargins
                                && ! $this->_readDataOnly
                            ) {
                            ...
                            if ($xmlSheet && $xmlSheet->pageSetup
                                && ! $this->_readDataOnly
                            ) {
                                ...
                                if (isset($xmlSheet->pageSetup["fitToHeight"])
                                    && intval($xmlSheet->pageSetup["fitToHeight"])
                                       >= 0
                                ) {
                                ...
                                if (isset($xmlSheet->pageSetup["fitToWidth"])
                                    && intval($xmlSheet->pageSetup["fitToWidth"])
                                       >= 0
                                ) {
                            ...
                            if ($xmlSheet && $xmlSheet->headerFooter
                                && ! $this->_readDataOnly
                            ) {
                            ...
                            if ($xmlSheet && $xmlSheet->rowBreaks
                                && $xmlSheet->rowBreaks->brk
                                && ! $this->_readDataOnly
                            ) {
                                foreach ($xmlSheet->rowBreaks->brk as $brk) {
                                    if ($brk["man"]) {
                                        $docSheet->setBreak("A$brk[id]",
                                            PHPExcel_Worksheet::BREAK_ROW);
                                    }
                                }
                            }
                            if ($xmlSheet && $xmlSheet->colBreaks
                            ...
                            if ($xmlSheet && $xmlSheet->dataValidations
                                && ! $this->_readDataOnly
                            ) {
                                foreach (
                                    $xmlSheet->dataValidations->dataValidation
                                    as $dataValidation
                                ) {
                                    // Uppercase coordinate
                                    $range
                                        = strtoupper($dataValidation["sqref"]);
                                    $rangeSet = explode(' ', $range);
                                    foreach ($rangeSet as $range) {
                                        $stRange
                                            = $docSheet->shrinkRangeToFit($range);

                                        // Extract all cell references in $range
                                        $aReferences
                                            = PHPExcel_Cell::extractAllCellReferencesInRange($stRange);
                                        foreach ($aReferences as $reference) {
                                            // Create validation
                                            $docValidation
                                                = $docSheet->getCell($reference)
                                                           ->getDataValidation();
                            ...
                            if ( ! $this->_readDataOnly) {
                                ...
                                if ($xmlSheet && $xmlSheet->hyperlinks) {
                                    foreach (
                                        $xmlSheet->hyperlinks->hyperlink as
                                        $hyperlink
                                    ) {
                                        // Link url
                                        $linkRel
                                        ...
                                        foreach (
                                            PHPExcel_Cell::extractAllCellReferencesInRange($hyperlink['ref'])
                                            as $cellReference
                                        ) {
                                            $cell
                                                = $docSheet->getCell($cellReference);
                                            if (isset($linkRel['id'])) {
                                                $hyperlinkUrl
                                                    = $hyperlinks[(string)$linkRel['id']];
                            ...
                            if ( ! $this->_readDataOnly) {
                                ...
                                if ($zip->locateName(dirname("$dir/$fileWorksheet")
                                    ...
                                    foreach (
                                        ...
                                        if ($ele["Type"]
                                            ...
                                            $comments[(string)$ele["Id"]]
                                                = (string)$ele["Target"];
                                        }
                                        if ($ele["Type"]
                                            == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing"
                                        ) {
                                            $vmlComments[(string)$ele["Id"]]
                                                = (string)$ele["Target"];
                                        }
                                    }
                                }
                                ...
                                foreach ($comments as $relName => $relPath) {
                                    // Load comments file
                                    $relPath
                                        = PHPExcel_Shared_File::realpath(dirname("$dir/$fileWorksheet")
                                                                         . "/"
                                                                         . $relPath);
                                    $commentsFile
                                        = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip,
                                        $relPath)), 'SimpleXMLElement',
                                        PHPExcel_Settings::getLibXmlLoaderOptions());
                                    ...
                                    foreach (
                                        $commentsFile->authors->author as
                                        $author
                                    ) {
                                    ...
                                    foreach (
                                        $commentsFile->commentList->comment as
                                        $comment
                                    ) {
                                ...
                                foreach ($vmlComments as $relName => $relPath) {
                                    ...
                                    foreach ($shapes as $shape) {
                                        ...
                                        if (isset($shape['style'])) {
                                            ...
                                            $fillColor
                                                = strtoupper(substr((string)$shape['fillcolor'],
                                                1));
                                            $column = null;
                                            $row = null;
                                            ...
                                            if (($column !== null)
                                                ...
                                                $comment
                                                    = $docSheet->getCommentByColumnAndRow((string)$column,
                                                    $row + 1);
                                    ...
                                    if ($zip->locateName(dirname("$dir/$fileWorksheet")
                                        ...
                                        foreach (
                                            ...
                                            if ($ele["Type"]
                                                ...
                                                $vmlRelationship
                                                    = self::dir_add("$dir/$fileWorksheet",
                                                    $ele["Target"]);
                                            }
                                        }
                                            ...
                                            foreach (
                                                ...
                                                if ($ele["Type"]
                                                    ...
                                                    $images[(string)$ele["Id"]]
                                                        = self::dir_add($fileDrawing,
                                                        $ele["Target"]);
                                                } elseif ($ele["Type"]
                                                          == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart"
                                                ) {
                                                    if ($this->_includeCharts) {
                                                        $charts[self::dir_add($fileDrawing,
                                                            $ele["Target"])]
                                                            = [
                                                            'id'    => (string)$ele["Id"],
                                        ...
                                        if ($xmlDrawing->twoCellAnchor) {
                                            foreach (
                                                ...
                                                if ($twoCellAnchor->pic->blipFill) {
                                                    ...
                                                    $chartDetails[$docSheet->getTitle()
                                                                  . '!'
                                                                  . $thisChart]
                                                        = [
                            ...
                            if ($xmlWorkbook->definedNames) {
                                foreach (
                                    $xmlWorkbook->definedNames->definedName as
                                    $definedName
                                ) {
                                    // Extract range
                                    ...
                                    // Valid range?
                                    if (stripos((string)$definedName, '#REF!')
                                        !== false
                                        || $extractedRange == ''
                                    ) {
                                        continue;
                        ...
                        if ($xmlWorkbook->definedNames) {
                            foreach (
                                ...
                                // Extract range
                                $extractedRange = (string)$definedName;
                                $extractedRange = preg_replace('/\'(\w+)\'\!/',
                                    '', $extractedRange);
                                if (($spos = strpos($extractedRange, '!'))
                                    !== false
                                ) {
                                    $extractedRange = substr($extractedRange, 0,
                                            $spos) . str_replace('$', '',
                                            substr($extractedRange, $spos));
                                } else {
                                    $extractedRange = str_replace('$', '',
                                ...
                                if ((string)$definedName['localSheetId']
                                    ...
                                    if (strpos((string)$definedName, '!')
                                        ...
                                        // Extract sheet name
                                        $extractedSheetName
                                            = PHPExcel_Worksheet::extractSheetTitle((string)$definedName,
                                            true);
                                        $extractedSheetName
        ...
        if ( ! $this->_readDataOnly) {
            $contentTypes
                = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip,
                "[Content_Types].xml")), 'SimpleXMLElement',
                PHPExcel_Settings::getLibXmlLoaderOptions());
                ...
                switch ($contentType["ContentType"]) {
                    case "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
                        if ($this->_includeCharts) {
                            ...
                            $chartElements
                                = simplexml_load_string($this->securityScan($this->_getFromZipArchive($zip,
                                $chartEntryRef)), 'SimpleXMLElement',
                                PHPExcel_Settings::getLibXmlLoaderOptions());
                            $objChart
                                = PHPExcel_Reader_Excel2007_Chart::readChart($chartElements,
...
//									var_dump($chartDetails[$chartPositionRef]);
    ...
    private static function _readColor($color, $background = false)
    {
        if (isset($color["rgb"])) {
            ...
            return PHPExcel_Style_Color::indexedColor($color["indexed"] - 7,
                $background)->getARGB();
        } elseif (isset($color["theme"])) {
            if (self::$_theme !== null) {
                $returnColour
                    = self::$_theme->getColourByIndex((int)$color["theme"]);
                if (isset($color["tint"])) {
                    $tintAdjust = (float)$color["tint"];
                    $returnColour
                        = PHPExcel_Style_Color::changeBrightness($returnColour,
                        $tintAdjust);
                }
...
//		}
        ...
        if (isset($style->border)) {
            ...
            $diagonalDown
                = self::boolean((string)$style->border["diagonalDown"]);
            ...
            self::_readBorder($docStyle->getBorders()->getLeft(),
                $style->border->left);
            self::_readBorder($docStyle->getBorders()->getRight(),
                $style->border->right);
            self::_readBorder($docStyle->getBorders()->getTop(),
                $style->border->top);
            self::_readBorder($docStyle->getBorders()->getBottom(),
                $style->border->bottom);
            self::_readBorder($docStyle->getBorders()->getDiagonal(),
        ...
        if (isset($style->alignment)) {
            ...
            $docStyle->getAlignment()
                     ->setReadorder(intval((string)$style->alignment["readingOrder"])
                                    > 0
                         ? intval((string)$style->alignment["readingOrder"])
                         : 0);
        }
                ...
                foreach ($is->r as $run) {
                    if ( ! isset($run->rPr)) {
                        ...
                        if (isset($run->rPr->vertAlign)
                            ...
                            $vertAlign
                                = strtolower((string)$run->rPr->vertAlign["val"]);
                            if ($vertAlign == 'superscript') {
    ...
    private function _readRibbon($excel, $customUITarget, $zip)
    {
        ...
        $localRibbon = $this->_getFromZipArchive($zip,
            $customUITarget);
        $customUIImagesNames = [];
        ...
        if ($dataRels) {
            ...
            $UIRels = simplexml_load_string($this->securityScan($dataRels),
                'SimpleXMLElement',
                PHPExcel_Settings::getLibXmlLoaderOptions());
            if ($UIRels) {
    ...
    private static function toCSSArray($style)
    {
        ...
        foreach ($temp as $item) {
            ...
            if (strpos($item[1], 'cm') !== false) {
                ...
                $item[1]
                    = PHPExcel_Shared_Font::centimeterSizeToPixels($item[1]);
            }
