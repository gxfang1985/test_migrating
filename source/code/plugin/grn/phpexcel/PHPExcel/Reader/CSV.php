 ...
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_CSV extends PHPExcel_Reader_Abstract
    implements PHPExcel_Reader_IReader
{
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        while (($rowData = fgetcsv($fileHandle, 0, $this->_delimiter,
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
    {
        ...
        $currentRow = 1;
        if ($this->_contiguous) {
            $currentRow = ($this->_contiguousRow == -1)
                ? $sheet->getHighestRow() : $this->_contiguousRow;
        }
        ...
        while (($rowData = fgetcsv($fileHandle, 0, $this->_delimiter,
            ...
            foreach ($rowData as $rowDatum) {
                if ($rowDatum != ''
                    ...
                    // Unescape enclosures
                    $rowDatum = str_replace($escapeEnclosures,
                        $this->_enclosure, $rowDatum);

                    // Convert encoding if necessary
                    if ($this->_inputEncoding !== 'UTF-8') {
                        $rowDatum
                            = PHPExcel_Shared_String::ConvertEncoding($rowDatum,
                            'UTF-8', $this->_inputEncoding);
                    }
