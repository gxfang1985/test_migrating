 ...
 * @copyright      Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract
    implements PHPExcel_Reader_IReader
{
            ...
            while ($this->_pos <= $this->_dataSize - 4) {
                ...
                switch ($code) {
                    ...
                    case self::XLS_Type_LABEL:
                        $length = self::_GetInt2d($this->_data,
                            $this->_pos + 2);
                        $recordData = $this->_readRecordData($this->_data,
                            $this->_pos + 4, $length);
                        ...
                        $tmpInfo['totalRows'] = max($tmpInfo['totalRows'],
                            $rowIndex);
    ...
    public function load($pFilename)
    {
        ...
        if ( ! $this->_readDataOnly) {
            foreach ($this->_objFonts as $objFont) {
                if (isset($objFont->colorIndex)) {
                    $color = self::_readColor($objFont->colorIndex,
                        $this->_palette, $this->_version);
            ...
            foreach ($this->_phpExcel->getCellXfCollection() as $objStyle) {
                ...
                if (isset($fill->startcolorIndex)) {
                    $startColor = self::_readColor($fill->startcolorIndex,
                        $this->_palette, $this->_version);
                    $fill->getStartColor()->setRGB($startColor['rgb']);
                }

                if (isset($fill->endcolorIndex)) {
                    $endColor = self::_readColor($fill->endcolorIndex,
                        $this->_palette, $this->_version);
                ...
                if (isset($top->colorIndex)) {
                    $borderTopColor = self::_readColor($top->colorIndex,
                        $this->_palette, $this->_version);
                ...
                if (isset($bottom->colorIndex)) {
                    $borderBottomColor = self::_readColor($bottom->colorIndex,
                        $this->_palette, $this->_version);
                    $bottom->getColor()->setRGB($borderBottomColor['rgb']);
                }

                if (isset($left->colorIndex)) {
                    $borderLeftColor = self::_readColor($left->colorIndex,
                        $this->_palette, $this->_version);
                ...
                if (isset($diagonal->colorIndex)) {
                    $borderDiagonalColor
                        = self::_readColor($diagonal->colorIndex,
                        $this->_palette, $this->_version);
        ...
        if ( ! $this->_readDataOnly && $this->_drawingGroupData) {
            ...
            $reader
                = new PHPExcel_Reader_Excel5_Escher($escherWorkbook);
        ...
        foreach ($this->_sheets as $sheet) {
            ...
            if (isset($this->_loadSheetsOnly)
                && ! in_array($sheet['name'], $this->_loadSheetsOnly)
            ) {
                continue;
            }
            ...
            if ( ! $this->_readDataOnly && $this->_drawingData) {
                ...
                $reader
                    = new PHPExcel_Reader_Excel5_Escher($escherWorksheet);
...
//				echo '<br />';
                ...
                if (isset($allSpContainers[$n + 1])
                    ...
                    $width
                        = PHPExcel_Shared_Excel5::getDistanceX($this->_phpSheet,
                        $startColumn, $startOffsetX, $endColumn, $endOffsetX);
                    $height
                        = PHPExcel_Shared_Excel5::getDistanceY($this->_phpSheet,
                        $startRow, $startOffsetY, $endRow, $endOffsetY);

                    // calculate offsetX and offsetY of the shape
                    $offsetX = $startOffsetX
                               * PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet,
                            $startColumn) / 1024;
                    $offsetY = $startOffsetY
                               * PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet,
                            $startRow) / 256;
...
//
                            if (isset($this->_cellNotes[$obj['idObjID']])) {
                                ...
                                if (isset($this->_textObjects[$obj['idObjID']])) {
                                    $textObject
                                        = $this->_textObjects[$obj['idObjID']];
                                    $this->_cellNotes[$obj['idObjID']]['objTextData']
                                        = $textObject;
                                }
                            }
                            break;
...
//							echo 'Picture Object<br />';
            ...
            if ($this->_version == self::XLS_BIFF8) {
                foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
                    list($column, $row)
                        = PHPExcel_Cell::coordinateFromString($cell);
                    if (($this->getReadFilter() !== null)
                        && $this->getReadFilter()->readCell($column, $row,
                            $this->_phpSheet->getTitle())
                    ) {
                        $formula
                            = $this->_getFormulaFromStructure($this->_sharedFormulas[$baseCell],
                            $cell);
...
//					echo '<br />';
            ...
            if ($definedName['isBuiltInName']) {
                switch ($definedName['name']) {
                    ...
                    case pack('C', 0x06):
                        ...
                        $ranges = explode(',',
                            $definedName['formula']); // FIXME: what if sheetname contains comma?
                        ...
                        foreach ($ranges as $range) {
                            ...
                            $explodes = explode('!',
                                $range);    // FIXME: what if sheetname contains exclamation mark?
                            ...
                            if (count($explodes) == 2) {
                                ...
                                $extractedRanges[] = str_replace('$', '',
                                    $explodes[1]); // C7:J66
                            }
                        }
                        if ($docSheet
                            = $this->_phpExcel->getSheetByName($sheetName)
                        ) {
                    ...
                    case pack('C', 0x07):
                        ...
                        foreach ($ranges as $range) {
                            ...
                            if (count($explodes) == 2) {
                                if ($docSheet
                                    = $this->_phpExcel->getSheetByName($explodes[0])
                                ) {
                                    ...
                                    if (count($coordinateStrings) == 2) {
                                        ...
                                        if ($firstColumn == 'A' and $lastColumn
                                                                    == 'IV'
                                        ) {
                ...
                if (count($explodes) == 2) {
                    if (($docSheet
                            = $this->_phpExcel->getSheetByName($explodes[0]))
                        || ($docSheet
                            = $this->_phpExcel->getSheetByName(trim($explodes[0],
                            "'")))
                    ) {
                        $extractedRange = $explodes[1];
                        $extractedRange = str_replace('$', '', $extractedRange);

                        $localOnly = ($definedName['scope'] == 0) ? false
                            : true;

                        $scope = ($definedName['scope'] == 0)
                            ?
                            null
                            : $this->_phpExcel->getSheetByName($this->_sheets[$definedName['scope']
                                                                              - 1]['name']);

                        $this->_phpExcel->addNamedRange(new PHPExcel_NamedRange((string)$definedName['name'],
                            $docSheet, $extractedRange, $localOnly, $scope));
                    }
                } else {
                    //	Named Value
    ...
    private function _readRecordData($data, $pos, $len)
    {
        ...
        if ($this->_encryption == self::MS_BIFF_CRYPTO_NONE
            || $pos < $this->_encryptionStartPos
        ) {
            return $data;
        }
        ...
        if ($this->_encryption == self::MS_BIFF_CRYPTO_RC4) {
            ...
            if ($block != $oldBlock || $pos < $this->_rc4Pos
                || ! $this->_rc4Key
            ) {
    ...
    private function _loadOLE($pFilename)
    {
        ...
        $this->_documentSummaryInformation
...
//		$this->_userDefinedProperties = $ole->getUserDefinedProperties();
    ...
    private function _readSummaryInformation()
    {
        ...
        $countProperties = self::_GetInt4d($this->_summaryInformation,
            $secOffset + 4);
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            $id = self::_GetInt4d($this->_summaryInformation,
                ($secOffset + 8) + (8 * $i));

            // Use value of property id as appropriate
            // offset: ($secOffset+12) + (8 * $i); size: 4; offset from beginning of section (48)
            $offset = self::_GetInt4d($this->_summaryInformation,
                ($secOffset + 12) + (8 * $i));

            $type = self::_GetInt4d($this->_summaryInformation,
                $secOffset + $offset);
            ...
            switch ($type) {
                case 0x02: // 2 byte signed integer
                    $value = self::_GetInt2d($this->_summaryInformation,
                        $secOffset + 4 + $offset);
                    break;

                case 0x03: // 4 byte signed integer
                    $value = self::_GetInt4d($this->_summaryInformation,
                        $secOffset + 4 + $offset);
                    break;
                ...
                case 0x1E: // null-terminated string prepended by dword string length
                    $byteLength = self::_GetInt4d($this->_summaryInformation,
                        $secOffset + 4 + $offset);
                    $value = substr($this->_summaryInformation,
                        $secOffset + 8 + $offset, $byteLength);
                    $value
                        = PHPExcel_Shared_String::ConvertEncoding($value,
                        'UTF-8', $codePage);
                    $value = rtrim($value);
                    break;
                ...
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-time
                    $value
                        = PHPExcel_Shared_OLE::OLE2LocalDate(substr($this->_summaryInformation,
...
//		echo '$secOffset = ',$secOffset,'<br />';
        ...
        //	section header
        //	offset: $secOffset;	size: 4;	section length
        $secLength = self::_GetInt4d($this->_documentSummaryInformation,
            $secOffset);
//		echo '$secLength = ',$secLength,'<br />';

        //	offset: $secOffset+4;	size: 4;	property count
        $countProperties = self::_GetInt4d($this->_documentSummaryInformation,
            $secOffset + 4);
...
//			echo 'Property ',$i,'<br />';
            //	offset: ($secOffset+8) + (8 * $i);	size: 4;	property ID
            $id = self::_GetInt4d($this->_documentSummaryInformation,
                ($secOffset + 8) + (8 * $i));
//			echo 'ID is ',$id,'<br />';

            // Use value of property id as appropriate
            // offset: 60 + 8 * $i;	size: 4;	offset from beginning of section (48)
            $offset = self::_GetInt4d($this->_documentSummaryInformation,
                ($secOffset + 12) + (8 * $i));

            $type = self::_GetInt4d($this->_documentSummaryInformation,
                $secOffset + $offset);
//			echo 'Type is ',$type,', ';
            ...
            switch ($type) {
                case 0x02:    //	2 byte signed integer
                    $value = self::_GetInt2d($this->_documentSummaryInformation,
                        $secOffset + 4 + $offset);
                    break;

                case 0x03:    //	4 byte signed integer
                    $value = self::_GetInt4d($this->_documentSummaryInformation,
                        $secOffset + 4 + $offset);
                    break;
                ...
                case 0x1E:    //	null-terminated string prepended by dword string length
                    $byteLength
                        = self::_GetInt4d($this->_documentSummaryInformation,
                        $secOffset + 4 + $offset);
                    $value = substr($this->_documentSummaryInformation,
                        $secOffset + 8 + $offset, $byteLength);
                    $value = PHPExcel_Shared_String::ConvertEncoding($value,
                        'UTF-8', $codePage);
                    $value = rtrim($value);
                    break;
                ...
                case 0x40:    //	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-Time
                    $value
                        = PHPExcel_Shared_OLE::OLE2LocalDate(substr($this->_documentSummaryInformation,
                        $secOffset + 4 + $offset, 8));
                    break;
...
//
            ...
            if ($cellAddress == '$B$65536') {
                ...
                $extension = true;
                $cellAddress
                    = array_pop(array_keys($this->_phpSheet->getComments()));
            }
//			echo 'Note Address=',$cellAddress,'<br />';
...
//			echo 'Note Text=',$noteText,'<br />';
            ...
            if ($extension) {
                ...
                $comment->setText($this->_parseRichText($commentText
                                                        . $noteText));
            } else {
...
//													->setAuthor( $author )
    ...
    private function _readTextObject()
    {
        ...
        $this->_textObjects[$this->textObjRef] = [
            'text'      => substr($text["recordData"],
                $text["spliceOffsets"][0] + 1, $cchText),
...
//		echo '<br />';
    ...
    private function _readFilepass()
    {
        ...
        $this->_encryptionStartPos = $this->_pos + self::_GetInt2d($this->_data,
                $this->_pos + 2);
    }
    ...
    private function _readFont()
    {
        ...
        if ( ! $this->_readDataOnly) {
            ...
            if ($this->_version == self::XLS_BIFF8) {
                $string = self::_readUnicodeStringShort(substr($recordData,
                    14));
            } else {
        ...
        if ( ! $this->_readDataOnly) {
            ...
            if (isset($this->_formats[$numberFormatIndex])) {
                ...
                $numberformat = ['code' => $this->_formats[$numberFormatIndex]];
                    ...
                    = PHPExcel_Style_NumberFormat::builtInFormatCode($numberFormatIndex))
                      !== ''
            ) {
            ...
            if ($this->_version == self::XLS_BIFF8) {
                ...
                if ($bordersLeftStyle = self::_mapBorderStyle((0x0000000F
                                                               & self::_GetInt4d($recordData,
                            10)) >> 0)
                ) {
                    $objStyle->getBorders()->getLeft()
                             ->setBorderStyle($bordersLeftStyle);
                }
                // bit: 7-4; mask: 0x000000F0; right style
                if ($bordersRightStyle = self::_mapBorderStyle((0x000000F0
                                                                & self::_GetInt4d($recordData,
                            10)) >> 4)
                ) {
                    $objStyle->getBorders()->getRight()
                             ->setBorderStyle($bordersRightStyle);
                }
                // bit: 11-8; mask: 0x00000F00; top style
                if ($bordersTopStyle = self::_mapBorderStyle((0x00000F00
                                                              & self::_GetInt4d($recordData,
                            10)) >> 8)
                ) {
                    $objStyle->getBorders()->getTop()
                             ->setBorderStyle($bordersTopStyle);
                }
                // bit: 15-12; mask: 0x0000F000; bottom style
                if ($bordersBottomStyle = self::_mapBorderStyle((0x0000F000
                                                                 & self::_GetInt4d($recordData,
                            10)) >> 12)
                ) {
                    $objStyle->getBorders()->getBottom()
                             ->setBorderStyle($bordersBottomStyle);
                }
                // bit: 22-16; mask: 0x007F0000; left color
                $objStyle->getBorders()->getLeft()->colorIndex = (0x007F0000
                                                                  & self::_GetInt4d($recordData,
                            10)) >> 16;

                // bit: 29-23; mask: 0x3F800000; right color
                $objStyle->getBorders()->getRight()->colorIndex = (0x3F800000
                                                                   & self::_GetInt4d($recordData,
                            10)) >> 23;

                // bit: 30; mask: 0x40000000; 1 = diagonal line from top left to right bottom
                $diagonalDown = (0x40000000 & self::_GetInt4d($recordData, 10))
                                >> 30 ?
                    true : false;

                // bit: 31; mask: 0x80000000; 1 = diagonal line from bottom left to top right
                $diagonalUp = (0x80000000 & self::_GetInt4d($recordData, 10))
                              >> 31 ?
                    true : false;
                ...
                $objStyle->getBorders()->getTop()->colorIndex = (0x0000007F
                                                                 & self::_GetInt4d($recordData,
                            14)) >> 0;

                // bit: 13-7; mask: 0x00003F80; bottom color
                $objStyle->getBorders()->getBottom()->colorIndex = (0x00003F80
                                                                    & self::_GetInt4d($recordData,
                            14)) >> 7;

                // bit: 20-14; mask: 0x001FC000; diagonal color
                $objStyle->getBorders()->getDiagonal()->colorIndex = (0x001FC000
                                                                      & self::_GetInt4d($recordData,
                            14)) >> 14;

                // bit: 24-21; mask: 0x01E00000; diagonal style
                if ($bordersDiagonalStyle = self::_mapBorderStyle((0x01E00000
                                                                   & self::_GetInt4d($recordData,
                            14)) >> 21)
                ) {
                    $objStyle->getBorders()->getDiagonal()
                             ->setBorderStyle($bordersDiagonalStyle);
                }

                // bit: 31-26; mask: 0xFC000000 fill pattern
                if ($fillType = self::_mapFillPattern((0xFC000000
                                                       & self::_GetInt4d($recordData,
                            14)) >> 26)
                ) {
                ...
                $objStyle->getFill()
                         ->setFillType(self::_mapFillPattern((0x003F0000
                                                              & $borderAndBackground)
                                                             >> 16));

                // bit: 24-22; mask: 0x01C00000; bottom line style
                $objStyle->getBorders()->getBottom()
                         ->setBorderStyle(self::_mapBorderStyle((0x01C00000
                                                                 & $borderAndBackground)
                                                                >> 22));

                // bit: 31-25; mask: 0xFE000000; bottom line color
                $objStyle->getBorders()->getBottom()->colorIndex = (0xFE000000
                                                                    & $borderAndBackground)
                                                                   >> 25;
                ...
                $objStyle->getBorders()->getTop()
                         ->setBorderStyle(self::_mapBorderStyle((0x00000007
                                                                 & $borderLines)
                                                                >> 0));

                // bit: 5-3; mask: 0x00000038; left line style
                $objStyle->getBorders()->getLeft()
                         ->setBorderStyle(self::_mapBorderStyle((0x00000038
                                                                 & $borderLines)
                                                                >> 3));

                // bit: 8-6; mask: 0x000001C0; right line style
                $objStyle->getBorders()->getRight()
                         ->setBorderStyle(self::_mapBorderStyle((0x000001C0
                                                                 & $borderLines)
                                                                >> 6));

                // bit: 15-9; mask: 0x0000FE00; top line color index
                $objStyle->getBorders()->getTop()->colorIndex = (0x0000FE00
                                                                 & $borderLines)
                                                                >> 9;
            ...
            if ($isCellStyleXf) {
                ...
                $this->_mapCellXfIndex[$this->_xfIndex]
                    = count($this->_phpExcel->getCellXfCollection()) - 1;
            }
            ...
            while ($offset < $length) {
                ...
                switch ($extType) {
                    case 4:        // fill start color
                        ...
                        $xclrValue = substr($extData, 4,
                            4); // color value (value based on color type)

                        if ($xclfType == 2) {
                            $rgb = sprintf('%02X%02X%02X', ord($xclrValue{0}),
                                ord($xclrValue{1}), ord($xclrValue{2}));

                            // modify the relevant style property
                            if (isset($this->_mapCellXfIndex[$ixfe])) {
                                $fill
                                    = $this->_phpExcel->getCellXfByIndex($this->_mapCellXfIndex[$ixfe])
                                                      ->getFill();
    ...
    private function _readExternalBook()
    {
        ...
        if (strlen($recordData) > 4) {
            ...
            $encodedUrlString = self::_readUnicodeStringLong(substr($recordData,
                2));
            ...
            for ($i = 0; $i < $nm; ++$i) {
                $externalSheetNameString
                    = self::_readUnicodeStringLong(substr($recordData,
                    $offset));
        ...
        } elseif (substr($recordData, 0, 4) == pack('vCC', 0x0001, 0x01,
                0x3A)
        ) {
            // add-in function
    ...
    private function _readExternName()
    {
        ...
        if ($this->_version == self::XLS_BIFF8) {
            ...
            $formula = $this->_getFormulaFromStructure(substr($recordData,
                $offset));
        ...
        if ($this->_version == self::XLS_BIFF8) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                $this->_ref[] = [
                    ...
                    'externalBookIndex' => self::_GetInt2d($recordData,
                        2 + 6 * $i),
                    // offset: 4 + 6 * $i; index to first sheet in EXTERNALBOOK record
                    'firstSheetIndex'   => self::_GetInt2d($recordData,
                        4 + 6 * $i),
                    // offset: 6 + 6 * $i; index to last sheet in EXTERNALBOOK record
                    'lastSheetIndex'    => self::_GetInt2d($recordData,
                        6 + 6 * $i),
                ];
            }
        }
        ...
        for ($i = 0; $i < $nm; ++$i) {
            ...
            if ($hasRichText) {
                ...
                for ($j = 0; $j < $formattingRuns; ++$j) {
                    ...
                    $fontIndex = self::_GetInt2d($recordData,
                        $pos + 2 + $j * 4);
    ...
    private function _readSheetPr()
    {
        ...
        $this->_isFitToPages = (bool)((0x0100 & self::_GetInt2d($recordData, 0))
                                      >> 8);
    }
    ...
    private function _readHorizontalPageBreaks()
    {
        ...
        if ($this->_version == self::XLS_BIFF8 && ! $this->_readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->_phpSheet->setBreakByColumnAndRow($cf, $r,
    ...
    private function _readVerticalPageBreaks()
    {
        ...
        if ($this->_version == self::XLS_BIFF8 && ! $this->_readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->_phpSheet->setBreakByColumnAndRow($c, $rf,
    ...
    private function _readPassword()
    {
        ...
        if ( ! $this->_readDataOnly) {
            ...
            $password = strtoupper(dechex(self::_GetInt2d($recordData,
    ...
    private function _readRow()
    {
        ...
        if ( ! $this->_readDataOnly) {
            ...
            $useDefaultHeight = (0x8000 & self::_GetInt2d($recordData, 6))
                                >> 15;

            if ( ! $useDefaultHeight) {
                $this->_phpSheet->getRowDimension($r + 1)->setRowHeight($height
                                                                        / 20);
            }
            ...
            $hasExplicitFormat = (0x00000080 & self::_GetInt4d($recordData, 12))
                                 >> 7;
        ...
        if (($this->getReadFilter() !== null)
            ...
            if (($fmtRuns = $this->_sst[$index]['fmtRuns'])
                ...
                for ($i = 0; $i <= $sstCount; ++$i) {
                    if (isset($fmtRuns[$i])) {
                        $text
                            = PHPExcel_Shared_String::Substring($this->_sst[$index]['value'],
                            $charPos, $fmtRuns[$i]['charPos'] - $charPos);
                ...
                $cell = $this->_phpSheet->getCell($columnString . ($row + 1));
                $cell->setValueExplicit($this->_sst[$index]['value'],
                    PHPExcel_Cell_DataType::TYPE_STRING);
            }
    ...
    private function _readMulRk()
    {
        ...
        for ($i = 0; $i < $columns; ++$i) {
            $columnString = PHPExcel_Cell::stringFromColumnIndex($colFirst
                                                                 + $i);
            ...
            if (($this->getReadFilter() !== null)
                ...
                $numValue = self::_GetIEEE754(self::_GetInt4d($recordData,
                    $offset + 2));
                $cell = $this->_phpSheet->getCell($columnString . ($row
                                                                   + 1));
                if ( ! $this->_readDataOnly) {
                    // add style
                    $cell->setXfIndex($this->_mapCellXfIndex[$xfIndex]);
    ...
    private function _readFormula()
    {
        ...
        if ($isPartOfSharedFormula) {
            ...
            $this->_baseCell = PHPExcel_Cell::stringFromColumnIndex($baseCol)
                               . ($baseRow + 1);
        }
        ...
        if (($this->getReadFilter() !== null)
            ...
            if ($isPartOfSharedFormula) {
                ...
                $this->_sharedFormulaParts[$columnString . ($row + 1)]
                    = $this->_baseCell;
            }
            ...
            if ( ! $isPartOfSharedFormula) {
                ...
                try {
                    ...
                    $formula
                        = $this->_getFormulaFromStructure($formulaStructure); // get formula in human language
    ...
    private function _readSharedFmla()
    {
        ...
        $cellRange
            = $this->_readBIFF5CellRangeAddressFixed($cellRange); // note: even BIFF8 uses BIFF5 syntax
...
//		echo '<br />';
    ...
    private function _readWindow2()
    {
        ...
        if ($this->_phpSheet->getSheetView()->getView()
            ...
            if ($this->_version === self::XLS_BIFF8) {
                $zoomScale = $isPageBreakPreview ? $zoomscaleInPageBreakPreview
                    : $zoomscaleInNormalView;
    ...
    private function _readScl()
    {
        ...
        $this->_phpSheet->getSheetView()->setZoomScale($numerator * 100
                                                       / $denumerator);
    }
    ...
    private function _readPane()
    {
        ...
        if ( ! $this->_readDataOnly) {
            ...
            if ($this->_frozen) {
                // frozen panes
                $this->_phpSheet->freezePane(PHPExcel_Cell::stringFromColumnIndex($px)
                                             . ($py + 1));
            } else {
    ...
    private function _readSelection()
    {
        ...
        if ( ! $this->_readDataOnly) {
            ...
            if (preg_match('/^([A-Z]+1\:[A-Z]+)16384$/', $selectedCells)) {
                $selectedCells = preg_replace('/^([A-Z]+1\:[A-Z]+)16384$/',
                    '${1}1048576', $selectedCells);
            }

            // first row '1' + last row '65536' indicates that full column is selected
            if (preg_match('/^([A-Z]+1\:[A-Z]+)65536$/', $selectedCells)) {
                $selectedCells = preg_replace('/^([A-Z]+1\:[A-Z]+)65536$/',
                    '${1}1048576', $selectedCells);
            }

            // first column 'A' + last column 'IV' indicates that full row is selected
            if (preg_match('/^(A[0-9]+\:)IV([0-9]+)$/', $selectedCells)) {
                $selectedCells = preg_replace('/^(A[0-9]+\:)IV([0-9]+)$/',
    ...
    private function _includeCellRangeFiltered($cellRangeAddress)
    {
        ...
        if ($this->getReadFilter() !== null) {
            ...
            $rangeBoundaries
                = PHPExcel_Cell::getRangeBoundaries($cellRangeAddress);
            $rangeBoundaries[1][0]++;
            for (
                $row = $rangeBoundaries[0][1]; $row <= $rangeBoundaries[1][1];
                $row++
            ) {
                for (
                    $column = $rangeBoundaries[0][0];
                    $column != $rangeBoundaries[1][0]; $column++
                ) {
    ...
    private function _readMergedCells()
    {
        ...
        if ($this->_version == self::XLS_BIFF8 && ! $this->_readDataOnly) {
            $cellRangeAddressList
                = $this->_readBIFF8CellRangeAddressList($recordData);
            foreach (
                $cellRangeAddressList['cellRangeAddresses'] as $cellRangeAddress
            ) {
    ...
    private function _readHyperLink()
    {
        ...
        if ( ! $this->_readDataOnly) {
            ...
            $isFileLinkOrUrl = (0x00000001 & self::_GetInt2d($recordData, 28))
                               >> 0;

            // bit: 1; mask: 0x00000002; 0 = relative path, 1 = absolute path or URL
            $isAbsPathOrUrl = (0x00000001 & self::_GetInt2d($recordData, 28))
                              >> 1;
            ...
            if ($hasDesc) {
                ...
                $desc = self::_encodeUTF16(substr($recordData, 36,
                    2 * ($dl - 1)), false);
                $offset += 4 + 2 * $dl;
            }
            if ($hasFrame) {
            ...
            switch ($hyperlinkType) {
                case 'URL':
                    ...
                    $url = self::_encodeUTF16(substr($recordData,
                        $offset, $us - 2), false);
                ...
                case 'local':
                    ...
                    $shortenedFilePath = self::_encodeUTF16($shortenedFilePath,
                        true);
                    $shortenedFilePath = substr($shortenedFilePath, 0,
                    ...
                    if ($sz > 0) {
                        ...
                        $extendedFilePath
                            = self::_encodeUTF16($extendedFilePath,
                            false);
                        $offset += $xl;
                    }
            ...
            if ($hasText) {
                ...
                $text = self::_encodeUTF16(substr($recordData, $offset,
                    2 * ($tl - 1)), false);
                $url .= $text;
            }
            ...
            foreach (
                PHPExcel_Cell::extractAllCellReferencesInRange($cellRange) as
                $coordinate
            ) {
                $this->_phpSheet->getCell($coordinate)->getHyperLink()
                                ->setUrl($url);
            }
        }
    }
    ...
    private function _readDataValidation()
    {
        ...
        switch ($operator) {
            ...
            case 0x06:
                $operator
                    = PHPExcel_Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL;
                break;
            case 0x07:
                $operator
                    = PHPExcel_Cell_DataValidation::OPERATOR_LESSTHANOREQUAL;
                break;
        }

        // offset: 4; size: var; title of the prompt box
        $offset = 4;
        $string = self::_readUnicodeStringLong(substr($recordData,
            $offset));
        ...
        $cellRangeAddressList
            = $this->_readBIFF8CellRangeAddressList(substr($recordData,
            $offset));
    ...
    private function _readRangeProtection()
    {
        ...
        if ( ! $this->_readDataOnly) {
            ...
            for ($i = 0; $i < $cref; ++$i) {
                try {
                    $cellRange
                        = $this->_readBIFF8CellRangeAddressFixed(substr($recordData,
                        27 + 8 * $i, 8));
                } catch (PHPExcel_Exception $e) {
                    return;
                }
            ...
            if ($cellRanges) {
                $this->_phpSheet->protectCells(implode(' ', $cellRanges),
                    strtoupper(dechex($wPassword)), true);
            }
        }
    }
...
//			var_dump($bcBitCount);
                ...
                foreach ($rgbTriples as $i => $rgbTriple) {
                    $color = imagecolorallocate($ih, $rgbTriple['r'],
                        $rgbTriple['g'], $rgbTriple['b']);
    ...
    private function _getSplicedRecordData()
    {
        ...
        do {
            ...
            $length = self::_GetInt2d($this->_data, $this->_pos + 2);
            $data .= $this->_readRecordData($this->_data, $this->_pos + 4,
                $length);
     ...
     */
    private function _getFormulaFromStructure(
        $formulaStructure,
        $baseCell = 'A1'
    ) {
     ...
     * @return string Human readable formula
     */
    private function _getFormulaFromData(
        $formulaData,
        $additionalData = '',
        $baseCell = 'A1'
    ) {
        ...
        while (strlen($formulaData) > 0 and
               $token = $this->_getNextToken($formulaData, $baseCell)) {
            $tokens[] = $token;
        ...
        $formulaString = $this->_createFormulaFromTokens($tokens,
            $additionalData);
            ...
            switch ($token['name']) {
                ...
                case 'tFuncV': // function with variable number of arguments
                    if ($token['data']['function'] != '') {
                        ...
                        $formulaStrings[] = "$space1$space0$function("
                                            . implode(',', $ops) . ")";
                        unset($space0, $space1);
                    }
                    break;
                ...
                case 'tArray': // array constant
                    $constantArray
                        = self::_readBIFF8ConstantArray($additionalData);
                ...
                case 'tMemArea':
                    ...
                    $cellRangeAddressList
                        = $this->_readBIFF8CellRangeAddressList($additionalData);
                    $additionalData = substr($additionalData,
    ...
    private function _getNextToken($formulaData, $baseCell = 'A1')
    {
        ...
        switch ($id) {
            ...
            case 0x17:    //	string
                ...
                $string = self::_readUnicodeStringShort(substr($formulaData,
                    1));
                $size = 1 + $string['size'];
            ...
            case 0x1F:    //	number
                ...
                $data = str_replace(',', '.',
                    (string)$data); // in case non-English locale
                break;
            ...
            case 0x64:
                $name = 'tRef';
                $size = 5;
                $data = $this->_readBIFF8CellAddress(substr($formulaData, 1,
                    4));
                break;
            ...
            case 0x65:
                $name = 'tArea';
                $size = 9;
                $data = $this->_readBIFF8CellRangeAddress(substr($formulaData,
                    1, 8));
                break;
            ...
            case 0x66:
                ...
                $size = 7 + $subSize;
                $data = $this->_getFormulaFromData(substr($formulaData, 7,
                    $subSize));
                break;
            ...
            case 0x69:
                ...
                $size = 3 + $subSize;
                $data = $this->_getFormulaFromData(substr($formulaData, 3,
                    $subSize));
                break;
            ...
            case 0x6C:
                $name = 'tRefN';
                $size = 5;
                $data = $this->_readBIFF8CellAddressB(substr($formulaData, 1,
                    4), $baseCell);
                break;
            ...
            case 0x6D:
                $name = 'tAreaN';
                $size = 9;
                $data = $this->_readBIFF8CellRangeAddressB(substr($formulaData,
                    1, 8), $baseCell);
                break;
            ...
            case 0x7A:
                ...
                try {
                    ...
                    $sheetRange
                        = $this->_readSheetRangeByRefIndex(self::_GetInt2d($formulaData,
                        1));
                    // offset: 3; size: 4; cell address
                    $cellAddress
                        = $this->_readBIFF8CellAddress(substr($formulaData, 3,
                        4));
            ...
            case 0x7B:
                ...
                try {
                    ...
                    $cellRangeAddress
                        = $this->_readBIFF8CellRangeAddress(substr($formulaData,
                        3, 8));
            ...
            default:
                throw new PHPExcel_Reader_Exception('Unrecognized token '
                                                    . sprintf('%02X', $id)
                                                    . ' in formula');
                break;
        }
    ...
    private function _readBIFF8CellAddress($cellAddressStructure)
    {
        ...
        $column = PHPExcel_Cell::stringFromColumnIndex(0x00FF
                                                       & self::_GetInt2d($cellAddressStructure,
                2));
     ...
     * @return string
     */
    private function _readBIFF8CellAddressB(
        $cellAddressStructure,
        $baseCell = 'A1'
    ) {
        list($baseCol, $baseRow)
            = PHPExcel_Cell::coordinateFromString($baseCell);
        ...
        if ( ! (0x4000 & self::_GetInt2d($cellAddressStructure, 2))) {
            ...
            $column = PHPExcel_Cell::stringFromColumnIndex($baseCol
                                                           + $colIndex);
        }
    ...
    private function _readBIFF8CellRangeAddress($subData)
    {
        ...
        // bit: 7-0; mask 0x00FF; column index
        $fc = PHPExcel_Cell::stringFromColumnIndex(0x00FF
                                                   & self::_GetInt2d($subData,
                4));
        ...
        // bit: 7-0; mask 0x00FF; column index
        $lc = PHPExcel_Cell::stringFromColumnIndex(0x00FF
                                                   & self::_GetInt2d($subData,
                6));
    ...
    private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
    {
        ...
        if ( ! (0x4000 & self::_GetInt2d($subData, 4))) {
            ...
            $fc = PHPExcel_Cell::stringFromColumnIndex($baseCol
                                                       + $fcIndex);
        }
        ...
        if ( ! (0x4000 & self::_GetInt2d($subData, 6))) {
            ...
            // column offset
            $lcIndex = ($lcIndex <= 127) ? $lcIndex : $lcIndex - 256;
            $lc = PHPExcel_Cell::stringFromColumnIndex($baseCol
                                                       + $lcIndex);
        }
    ...
    private function _readBIFF8CellRangeAddressList($subData)
    {
        ...
        for ($i = 0; $i < $nm; ++$i) {
            $cellRangeAddresses[]
                = $this->_readBIFF8CellRangeAddressFixed(substr($subData,
                $offset, 8));
            $offset += 8;
        }
    ...
    private function _readBIFF5CellRangeAddressList($subData)
    {
        ...
        for ($i = 0; $i < $nm; ++$i) {
            $cellRangeAddresses[]
                = $this->_readBIFF5CellRangeAddressFixed(substr($subData,
                $offset, 6));
            $offset += 6;
        }
    ...
    private function _readSheetRangeByRefIndex($index)
    {
        if (isset($this->_ref[$index])) {
            ...
            $type
                = $this->_externalBooks[$this->_ref[$index]['externalBookIndex']]['type'];

            switch ($type) {
                case 'internal':
                    // check if we have a deleted 3d reference
                    if ($this->_ref[$index]['firstSheetIndex'] == 0xFFFF
                        or $this->_ref[$index]['lastSheetIndex'] == 0xFFFF
                    ) {
                    ...
                    $firstSheetName
                        = $this->_sheets[$this->_ref[$index]['firstSheetIndex']]['name'];
                    $lastSheetName
                        = $this->_sheets[$this->_ref[$index]['lastSheetIndex']]['name'];
    ...
    private static function _readBIFF8ConstantArray($arrayData)
    {
        ...
        for ($r = 1; $r <= $nr + 1; ++$r) {
            ...
            $matrixChunks[] = implode(',',
                $items); // looks like e.g. '1,"hello"'
        }
    ...
    private static function _readUnicodeStringShort($subData)
    {
        ...
        $string = self::_readUnicodeString(substr($subData, 1),
            $characterCount);
    ...
    private static function _readUnicodeStringLong($subData)
    {
        ...
        $string = self::_readUnicodeString(substr($subData, 2),
            $characterCount);
    ...
    private static function _readUnicodeString($subData, $characterCount)
    {
        ...
        // needs to be fixed
        $value = self::_encodeUTF16(substr($subData, 1,
            $isCompressed ? $characterCount : 2 * $characterCount),
            $isCompressed);

        return [
            'value' => $value,
            'size'  => $isCompressed ? 1 + $characterCount
                : 1 + 2 * $characterCount,
    ...
    private static function _encodeUTF16($string, $compressed = '')
    {
        ...
        return PHPExcel_Shared_String::ConvertEncoding($string, 'UTF-8',
            'UTF-16LE');
    }
    ...
    public static function _GetInt4d($data, $pos)
    {
        ...
        return ord($data[$pos]) | (ord($data[$pos + 1]) << 8) | (ord($data[$pos
                                                                           + 2])
                                                                 << 16)
               | $_ord_24;
    }
