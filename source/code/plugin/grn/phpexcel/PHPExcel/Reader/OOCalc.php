 ...
 * @copyright      Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_OOCalc extends PHPExcel_Reader_Abstract
...
//		}
        ...
        if ($zip->open($pFilename) === true) {
            ...
            if ($stat && ($stat['size'] <= 255)) {
                ...
                $xml
                    = simplexml_load_string($this->securityScan($zip->getFromName('META-INF/manifest.xml')),
                    'SimpleXMLElement',
                ...
                if (isset($namespacesContent['manifest'])) {
                    ...
                    foreach ($manifest as $manifestDataSet) {
                        ...
                        if ($manifestAttributes->{'full-path'} == '/') {
                            $mimeType
                                = (string)$manifestAttributes->{'media-type'};
                            break;
                        }
                    }
                }
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        $xml = new XMLReader();
        $res = $xml->xml($this->securityScanFile('zip://' . realpath($pFilename)
                                                 . '#content.xml'), null,
            PHPExcel_Settings::getLibXmlLoaderOptions());
...
//			}
    ...
    private static function identifyFixedStyleValue(
        $styleList,
        &$styleAttributeValue
    ) {
...
//		echo '<h1>Meta Information</h1>';
        $xml
            = simplexml_load_string($this->securityScan($zip->getFromName("meta.xml")),
            ...
            foreach ($officePropertyMeta as $propertyName => $propertyValue) {
                ...
                switch ($propertyName) {
                    ...
                    case 'user-defined' :
                        ...
                        foreach ($propertyValueAttributes as $key => $value) {
                            if ($key == 'name') {
                                ...
                                switch ($value) {
                                    case 'date'    :
                                        $propertyValue
                                            = PHPExcel_DocumentProperties::convertProperty($propertyValue,
                                            'date');
                                        $propertyValueType
                        ...
                        $docProps->setCustomProperty($propertyValueName,
...
//		echo '<h1>Workbook Content</h1>';
        $xml
            = simplexml_load_string($this->securityScan($zip->getFromName("content.xml")),
            'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());
        $namespacesContent = $xml->getNamespaces(true);
//		echo '<pre>';
...
//		echo '</pre><hr />';
        ...
        foreach ($workbook->body->spreadsheet as $workbookData) {
            $workbookData
                = $workbookData->children($namespacesContent['table']);
            $worksheetID = 0;
            foreach ($workbookData->table as $worksheetDataSet) {
                $worksheetData
                    = $worksheetDataSet->children($namespacesContent['table']);
//				print_r($worksheetData);
//				echo '<br />';
                $worksheetDataAttributes
                    = $worksheetDataSet->attributes($namespacesContent['table']);
//				print_r($worksheetDataAttributes);
//				echo '<br />';
                if ((isset($this->_loadSheetsOnly))
                    && (isset($worksheetDataAttributes['name']))
                    && ( ! in_array($worksheetDataAttributes['name'],
                    ...
                    switch ($key) {
                        ...
                        case 'table-row' :
                            $rowDataTableAttributes
                                = $rowData->attributes($namespacesContent['table']);
                            $rowRepeats
                                = (isset($rowDataTableAttributes['number-rows-repeated']))
                                ?
                                $rowDataTableAttributes['number-rows-repeated']
                                : 1;
                            $columnID = 'A';
                            foreach ($rowData as $key => $cellData) {
...
//								echo '<b>'.$columnID.$rowID.'</b><br />';
                                $cellDataText
                                    ...
                                    '';
                                $cellDataOffice
                                    = $cellData->children($namespacesContent['office']);
                                $cellDataOfficeAttributes
                                    = $cellData->attributes($namespacesContent['office']);
                                $cellDataTableAttributes
                                    = $cellData->attributes($namespacesContent['table']);
...
//
                                ...
                                if (isset($cellDataTableAttributes['formula'])) {
                                    $cellDataFormula
                                        = $cellDataTableAttributes['formula'];
...
//									echo 'Value Type is '.$cellDataOfficeAttributes['value-type'].'<br />';
                                    switch ($cellDataOfficeAttributes['value-type']) {
                                        ...
                                        case 'percentage' :
                                            ...
                                            $dataValue
                                                = (float)$cellDataOfficeAttributes['value'];
                                            if (floor($dataValue)
                                                == $dataValue
                                            ) {
                                                $dataValue
                                        ...
                                        case 'date' :
                                            $type
                                                = PHPExcel_Cell_DataType::TYPE_NUMERIC;
                                            $dateObj
                                                = new DateTime($cellDataOfficeAttributes['date-value'],
                                                $GMT);
                                            ...
                                            $dataValue
                                                = PHPExcel_Shared_Date::FormattedPHPToExcel($year,
                                        ...
                                        case 'time' :
                                            $type
                                                = PHPExcel_Cell_DataType::TYPE_NUMERIC;
                                            $dataValue
                                                = PHPExcel_Shared_Date::PHPToExcel(strtotime('01-01-1970 '
                                                                                             . implode(':',
...
//									echo 'Adjusted Formula: ', $cellDataFormula, PHP_EOL;
                                ...
                                $colRepeats
                                    ...
                                    ?
                                    $cellDataTableAttributes['number-columns-repeated']
                                    : 1;
                                if ($type !== null) {
                                ...
                                if ((isset($cellDataTableAttributes['number-columns-spanned']))
                                    ...
                                    if (($type
                                        ...
                                        if (isset($cellDataTableAttributes['number-rows-spanned'])) {
                                            $rowTo = $rowTo
                                                     + $cellDataTableAttributes['number-rows-spanned']
                                                     - 1;
                                        }
