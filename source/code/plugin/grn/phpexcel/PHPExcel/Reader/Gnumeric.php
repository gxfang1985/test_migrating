 ...
 * @copyright      Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_Gnumeric extends PHPExcel_Reader_Abstract
...
//		echo '</pre><hr />';
//
        $xml = simplexml_load_string($this->securityScan($gFileData),
...
//			echo '<b>Worksheet: ',$worksheetName,'</b><br />';
            if ((isset($this->_loadSheetsOnly))
                && ( ! in_array($worksheetName, $this->_loadSheetsOnly))
            ) {
                continue;
            }
            ...
            if (( ! $this->_readDataOnly)
                ...
                if (isset($sheet->PrintInformation->Margins)) {
                    foreach (
                        $sheet->PrintInformation->Margins->children('gnm', true)
                        as $key => $margin
                    ) {
                        ...
                        switch ($marginAttributes['PrefUnit']) {
                            case 'mm' :
                                $marginSize
                                    = intval($marginAttributes['Points']) / 100;
                                break;
                        }
                        switch ($key) {
                            case 'top' :
...
//						echo 'NEW EXPRESSION ',$ExprID,'<br />';
                        ...
                        $cell
                            = $this->_referenceHelper->updateFormulaReferences($expression['formula'],
                            'A1',
...
//						echo 'New Value is ',$cell,'<br />';
                ...
                $objPHPExcel->getActiveSheet()->getCell($column . $row)
                            ->setValueExplicit($cell, $type);
            }

            if (( ! $this->_readDataOnly) && (isset($sheet->Objects))) {
                foreach (
                    $sheet->Objects->children('gnm', true) as $key => $comment
                ) {
...
//
            foreach ($sheet->Styles->StyleRegion as $styleRegion) {
                ...
                if (($styleAttributes['startRow'] <= $maxRow)
                    ...
                    $startColumn
                        = PHPExcel_Cell::stringFromColumnIndex((int)$styleAttributes['startCol']);
                    $startRow = $styleAttributes['startRow'] + 1;

                    $endColumn = ($styleAttributes['endCol'] > $maxCol)
                        ? $maxCol : (int)$styleAttributes['endCol'];
                    $endColumn
                        = PHPExcel_Cell::stringFromColumnIndex($endColumn);
                    $endRow = ($styleAttributes['endRow'] > $maxRow)
                        ? $maxRow : $styleAttributes['endRow'];
                    $endRow += 1;
                    $cellRange = $startColumn . $startRow . ':' . $endColumn
                                 . $endRow;
//					echo $cellRange,'<br />';
                    ...
                    if (( ! $this->_readDataOnly)
                        ...
                        if ( ! $this->_readDataOnly) {
                            ...
                            $styleArray['alignment']['wrap']
                                = ($styleAttributes['WrapText'] == '1') ? true
                                : false;
                            $styleArray['alignment']['shrinkToFit']
                                = ($styleAttributes['ShrinkToFit'] == '1')
                                ? true : false;
                            $styleArray['alignment']['indent']
                                = (intval($styleAttributes["Indent"]) > 0)
                                ? $styleAttributes["indent"] : 0;

                            $RGB
                                = self::_parseGnumericColour($styleAttributes["Fore"]);
                            $styleArray['font']['color']['rgb'] = $RGB;
                            $RGB
                                = self::_parseGnumericColour($styleAttributes["Back"]);
                            $shade
                                = $styleAttributes["Shade"];
                            if (($RGB != '000000') || ($shade != '0')) {
                                $styleArray['fill']['color']['rgb']
                                    = $styleArray['fill']['startcolor']['rgb'] = $RGB;
                                $RGB2
                                    = self::_parseGnumericColour($styleAttributes["PatternColor"]);
...
//							echo '<br />';
                            ...
                            $styleArray['font']['size']
                                = intval($fontAttributes['Unit']);
                            $styleArray['font']['bold']
                                = ($fontAttributes['Bold'] == '1') ? true
                                : false;
                            $styleArray['font']['italic']
                                = ($fontAttributes['Italic'] == '1') ? true
                                : false;
...
//						var_dump($styleArray);
//						echo '<br />';
                        $objPHPExcel->getActiveSheet()->getStyle($cellRange)
            ...
            if (( ! $this->_readDataOnly) && (isset($sheet->Cols))) {
                ...
                foreach ($sheet->Cols->ColInfo as $columnOverride) {
                    ...
                    $hidden = ((isset($columnAttributes['Hidden']))
                               && ($columnAttributes['Hidden']
                                   == '1')) ? true : false;
                    $columnCount = (isset($columnAttributes['Count']))
            ...
            if (( ! $this->_readDataOnly) && (isset($sheet->Rows))) {
                ...
                foreach ($sheet->Rows->RowInfo as $rowOverride) {
                    ...
                    while ($r < $row) {
                        ++$r;
                        $objPHPExcel->getActiveSheet()->getRowDimension($r)
        ...
        if (isset($gnmXML->Names)) {
            foreach ($gnmXML->Names->Name as $namedRange) {
                ...
                if ($worksheet = $objPHPExcel->getSheetByName($range[0])) {
                    ...
                    $objPHPExcel->addNamedRange(new PHPExcel_NamedRange($name,
