 ...
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Reader_HTML extends PHPExcel_Reader_Abstract
...
//				echo 'FLUSH CELL: ' , $column , $row , ' => ' , $cellContent , '<br />';
    ...
    protected function _processDomElement(
        DOMNode $element,
        $sheet,
        &$row,
        &$column,
...
//					echo '<b>ATTRIBUTE: </b>' , $attribute->name , ' => ' , $attribute->value , '<br />';
                ...
                switch ($child->nodeName) {
                    case 'meta' :
                        foreach (
                            $attributeArray as $attributeName => $attributeValue
                        ) {
                        ...
                        $this->_processDomElement($child, $sheet, $row, $column,
                            $cellContent);
                        break;
                    case 'title' :
                        $this->_processDomElement($child, $sheet, $row, $column,
...
//						echo 'END OF STYLING, SPAN OR DIV<br />';
                    ...
                    case 'hr' :
                        ...
                        if (isset($this->_formats[$child->nodeName])) {
                            ...
                            $cellContent = '----------';
                            $this->_flushCell($sheet, $column, $row,
                                $cellContent);
                        }
                        ++$row;
                    case 'br' :
...
//						echo 'END OF HYPERLINK:' , '<br />';
                    ...
                    case 'p' :
                        if ($this->_tableLevel > 0) {
                            ...
                            $cellContent .= "\n";
//							echo 'LIST ENTRY: ' , '<br />';
                            $this->_processDomElement($child, $sheet, $row,
                                $column, $cellContent);
...
//                        }
                        ...
                        if (isset($attributeArray['rowspan'])
                            ...
                            $columnTo = $column;
                            for (
                                $i = 0; $i < $attributeArray['colspan'] - 1;
                                $i++
                            ) {
                                ++$columnTo;
                            }
                            $range = $column . $row . ':' . $columnTo . ($row
                                                                         + $attributeArray['rowspan']
                                                                         - 1);
                            foreach (
                                \PHPExcel_Cell::extractAllCellReferencesInRange($range)
                                as $value
                            ) {
                            ...
                            $sheet->mergeCells($column . $row . ':' . $columnTo
                                               . $row);
                            $column = $columnTo;
                        }
                        ++$column;
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
    {
        ...
        if ($loaded === false) {
            throw new PHPExcel_Reader_Exception('Failed to load ', $pFilename,
                ' as a DOM Document');
        }
        ...
        $column = 'A';
        $content = '';
        $this->_processDomElement($dom, $objPHPExcel->getActiveSheet(), $row,
            $column, $content);
