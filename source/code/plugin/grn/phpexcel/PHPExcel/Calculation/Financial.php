...
class PHPExcel_Calculation_Financial
{
    ...
    private static function _coupFirstPeriodDate(
        $settlement,
        $maturity,
        $frequency,
        $next
    ...
    private static function _daysPerYear($year, $basis = 0)
    {
        switch ($basis) {
            ...
            case 1 :
                $daysPerYear
                    = (PHPExcel_Calculation_DateTime::_isLeapYear($year)) ? 366
                    : 365;
                break;
            default    :
    ...
    private static function _interestAndPrincipal(
        $rate = 0,
        $per = 0,
        $nper = 0,
        $pv = 0,
    ...
    public static function ACCRINT(
        ...
        $settlement
            = PHPExcel_Calculation_Functions::flattenSingleValue($settlement);
        $rate
            = PHPExcel_Calculation_Functions::flattenSingleValue($rate);
        $par = (is_null($par)) ? 1000
            : PHPExcel_Calculation_Functions::flattenSingleValue($par);
        $frequency = (is_null($frequency)) ? 1
            : PHPExcel_Calculation_Functions::flattenSingleValue($frequency);
        $basis = (is_null($basis)) ? 0
            : PHPExcel_Calculation_Functions::flattenSingleValue($basis);
        ...
        if ((is_numeric($rate)) && (is_numeric($par))) {
            ...
            $daysBetweenIssueAndSettlement
                = PHPExcel_Calculation_DateTime::YEARFRAC($issue, $settlement,
                $basis);
     ...
     * @return    float
     */
    public static function ACCRINTM(
        $issue,
        $settlement,
        $rate,
        $par = 1000,
    ...
    public static function AMORDEGRC(
        ...
        $purchased
            = PHPExcel_Calculation_Functions::flattenSingleValue($purchased);
        $firstPeriod
            = PHPExcel_Calculation_Functions::flattenSingleValue($firstPeriod);
        $salvage
            = PHPExcel_Calculation_Functions::flattenSingleValue($salvage);
        $period
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($period));
        $rate
            = PHPExcel_Calculation_Functions::flattenSingleValue($rate);
        $basis = (is_null($basis)) ? 0
            : (int)PHPExcel_Calculation_Functions::flattenSingleValue($basis);
        ...
        $rate *= $amortiseCoeff;
        $fNRate = round(PHPExcel_Calculation_DateTime::YEARFRAC($purchased,
                $firstPeriod, $basis) * $rate * $cost, 0);
        $cost -= $fNRate;
        $fRest = $cost - $salvage;
    ...
    public static function AMORLINC(
        ...
        $salvage
            = PHPExcel_Calculation_Functions::flattenSingleValue($salvage);
        $period
            = PHPExcel_Calculation_Functions::flattenSingleValue($period);
        $rate
            = PHPExcel_Calculation_Functions::flattenSingleValue($rate);
        $basis = (is_null($basis)) ? 0
            : (int)PHPExcel_Calculation_Functions::flattenSingleValue($basis);
        ...
        $yearFrac = PHPExcel_Calculation_DateTime::YEARFRAC($purchased,
            $firstPeriod, $basis);
    ...
    public static function COUPDAYBS(
        ...
        $settlement
            = PHPExcel_Calculation_Functions::flattenSingleValue($settlement);
        $maturity
            = PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
        $frequency
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($frequency);
        $basis = (is_null($basis)) ? 0
            : (int)PHPExcel_Calculation_Functions::flattenSingleValue($basis);
        ...
        if (is_string(
            $maturity = PHPExcel_Calculation_DateTime::_getDateValue($maturity))
        ) {
            return PHPExcel_Calculation_Functions::VALUE();
        }
        ...
        $daysPerYear
            = self::_daysPerYear(PHPExcel_Calculation_DateTime::YEAR($settlement),
            $basis);
        $prev = self::_coupFirstPeriodDate($settlement, $maturity, $frequency,
            false);

        return PHPExcel_Calculation_DateTime::YEARFRAC($prev, $settlement,
                $basis) * $daysPerYear;
    }    //	function COUPDAYBS()
    ...
    public static function COUPDAYS(
        ...
        switch ($basis) {
            ...
            case 1: // Actual/actual
                if ($frequency == 1) {
                    $daysPerYear
                        = self::_daysPerYear(PHPExcel_Calculation_DateTime::YEAR($maturity),
                        $basis);

                    return ($daysPerYear / $frequency);
                } else {
                    $prev = self::_coupFirstPeriodDate($settlement, $maturity,
                        $frequency, false);
                    $next = self::_coupFirstPeriodDate($settlement, $maturity,
                        $frequency, true);
    ...
    public static function COUPDAYSNC(
        ...
        $daysPerYear
            = self::_daysPerYear(PHPExcel_Calculation_DateTime::YEAR($settlement),
            $basis);
        $next = self::_coupFirstPeriodDate($settlement, $maturity, $frequency,
            true);

        return PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $next,
                $basis) * $daysPerYear;
    }    //	function COUPDAYSNC()
    ...
    public static function COUPNCD(
        ...
        return self::_coupFirstPeriodDate($settlement, $maturity, $frequency,
            true);
    }    //	function COUPNCD()
    ...
    public static function COUPNUM(
        ...
        $settlement = self::_coupFirstPeriodDate($settlement, $maturity,
            $frequency, true);
        $daysBetweenSettlementAndMaturity
            = PHPExcel_Calculation_DateTime::YEARFRAC($settlement,
                $maturity, $basis) * 365;
    ...
    public static function CUMIPMT($rate, $nper, $pv, $start, $end, $type = 0)
    {
        ...
        $start
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($start);
    ...
    public static function DB($cost, $salvage, $life, $period, $month = 12)
    {
        ...
        if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life))
            ...
            if ($cost == 0) {
                return 0.0;
            } elseif (($cost < 0) || (($salvage / $cost) < 0) || ($life <= 0)
                      || ($period < 1)
                      || ($month < 1)
            ) {
                return PHPExcel_Calculation_Functions::NaN();
            ...
            for ($per = 1; $per <= $period; ++$per) {
                if ($per == 1) {
                    ...
                    $depreciation = ($cost - $previousDepreciation)
                                    * $fixedDepreciationRate * (12 - $month)
                                    / 12;
                } else {
                    $depreciation = ($cost - $previousDepreciation)
    ...
    public static function DDB($cost, $salvage, $life, $period, $factor = 2.0)
    {
        ...
        if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life))
            ...
            $period = (int)$period;
            $factor = (float)$factor;
            if (($cost <= 0) || (($salvage / $cost) < 0) || ($life <= 0)
                || ($period < 1)
                || ($factor <= 0.0)
                || ($period > $life)
            ) {
    ...
    public static function DISC(
        ...
        $maturity
            = PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
        $price
            = PHPExcel_Calculation_Functions::flattenSingleValue($price);
        $redemption
            = PHPExcel_Calculation_Functions::flattenSingleValue($redemption);
        $basis
            = PHPExcel_Calculation_Functions::flattenSingleValue($basis);
    ...
    {
        $fractional_dollar
            = PHPExcel_Calculation_Functions::flattenSingleValue($fractional_dollar);
        $fraction
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($fraction);
    ...
    {
        $decimal_dollar
            = PHPExcel_Calculation_Functions::flattenSingleValue($decimal_dollar);
        $fraction
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($fraction);
    ...
    {
        $nominal_rate
            = PHPExcel_Calculation_Functions::flattenSingleValue($nominal_rate);
        $npery
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($npery);
    ...
    public static function FV(
        ...
        if ( ! is_null($rate) && $rate != 0) {
            return -$pv * pow(1 + $rate, $nper) - $pmt * (1 + $rate * $type)
                                                  * (pow(1 + $rate, $nper) - 1)
                                                  / $rate;
        } else {
            return -$pv - $pmt * $nper;
     ...
     * @return    float
     */
    public static function INTRATE(
        $settlement,
        $maturity,
        $investment,
        $redemption,
        ...
        $maturity
            = PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
        $investment
            = PHPExcel_Calculation_Functions::flattenSingleValue($investment);
        $redemption
            = PHPExcel_Calculation_Functions::flattenSingleValue($redemption);
        $basis
            = PHPExcel_Calculation_Functions::flattenSingleValue($basis);
    ...
    public static function IPMT($rate, $per, $nper, $pv, $fv = 0, $type = 0)
    {
        ...
        // Calculate
        $interestAndPrincipal = self::_interestAndPrincipal($rate, $per, $nper,
            $pv, $fv, $type);
    ...
    public static function IRR($values, $guess = 0.1)
    {
        ...
        for ($i = 0; $i < FINANCIAL_MAX_ITERATIONS; ++$i) {
            ...
            if ((abs($f_mid) < FINANCIAL_PRECISION)
                || (abs($dx) < FINANCIAL_PRECISION)
            ) {
                return $x_mid;
            }
        }
    ...
    public static function MIRR($values, $finance_rate, $reinvestment_rate)
    {
        ...
        $values = PHPExcel_Calculation_Functions::flattenArray($values);
        $finance_rate
            = PHPExcel_Calculation_Functions::flattenSingleValue($finance_rate);
        $reinvestment_rate
            = PHPExcel_Calculation_Functions::flattenSingleValue($reinvestment_rate);
        $n = count($values);
    ...
    {
        $effect_rate
            = PHPExcel_Calculation_Functions::flattenSingleValue($effect_rate);
        $npery
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($npery);
    ...
    public static function NPER(
        ...
        if ( ! is_null($rate) && $rate != 0) {
            ...
            return log(($pmt * (1 + $rate * $type) / $rate - $fv) / ($pv + $pmt
                                                                           * (1
                                                                              + $rate
                                                                                * $type)
                                                                           / $rate))
    ...
    public static function PRICE(
        ...
        $maturity
            = PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
        $rate
            = (float)PHPExcel_Calculation_Functions::flattenSingleValue($rate);
        $yield
            = (float)PHPExcel_Calculation_Functions::flattenSingleValue($yield);
        $redemption
            = (float)PHPExcel_Calculation_Functions::flattenSingleValue($redemption);
        $frequency
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($frequency);
        $basis = (is_null($basis)) ? 0
            : (int)PHPExcel_Calculation_Functions::flattenSingleValue($basis);
     ...
     * @return    float
     */
    public static function PRICEDISC(
        $settlement,
        $maturity,
        $discount,
        $redemption,
        ...
        $maturity
            = PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
        $discount
            = (float)PHPExcel_Calculation_Functions::flattenSingleValue($discount);
        $redemption
            = (float)PHPExcel_Calculation_Functions::flattenSingleValue($redemption);
        $basis
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($basis);
     ...
     * @return    float
     */
    public static function PRICEMAT(
        $settlement,
        $maturity,
        $issue,
        $rate,
        ...
        if (is_numeric($rate) && is_numeric($yield)) {
            ...
            $daysBetweenIssueAndMaturity
                = PHPExcel_Calculation_DateTime::YEARFRAC($issue,
                $maturity, $basis);
    ...
    public static function PV(
        ...
        // Calculate
        if ( ! is_null($rate) && $rate != 0) {
            return (-$pmt * (1 + $rate * $type) * ((pow(1 + $rate, $nper) - 1)
                                                   / $rate) - $fv) / pow(1
                                                                         + $rate,
                    $nper);
        } else {
    ...
    public static function RATE(
        ...
        $pv = PHPExcel_Calculation_Functions::flattenSingleValue($pv);
        $fv = (is_null($fv)) ? 0.0
            : PHPExcel_Calculation_Functions::flattenSingleValue($fv);
        $type = (is_null($type)) ? 0
            : (int)PHPExcel_Calculation_Functions::flattenSingleValue($type);
        $guess = (is_null($guess)) ? 0.1
            : PHPExcel_Calculation_Functions::flattenSingleValue($guess);
        ...
        $i = $x0 = 0.0;
        $x1 = $rate;
        while ((abs($y0 - $y1) > FINANCIAL_PRECISION)
    ...
    public static function RECEIVED(
        ...
        $maturity
            = PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
        $investment
            = (float)PHPExcel_Calculation_Functions::flattenSingleValue($investment);
        $discount
            = (float)PHPExcel_Calculation_Functions::flattenSingleValue($discount);
        $basis
            = (int)PHPExcel_Calculation_Functions::flattenSingleValue($basis);
    ...
    public static function SYD($cost, $salvage, $life, $period)
    {
        ...
        if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life))
            ...
            return (($cost - $salvage) * ($life - $period + 1) * 2) / ($life
                                                                       * ($life
                                                                          + 1));
        }
    ...
    public static function TBILLEQ($settlement, $maturity, $discount)
    {
        ...
        $maturity
            = PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
        $discount
            = PHPExcel_Calculation_Functions::flattenSingleValue($discount);
        ...
        if (PHPExcel_Calculation_Functions::getCompatibilityMode()
            ...
            $daysBetweenSettlementAndMaturity
                = (PHPExcel_Calculation_DateTime::_getDateValue($maturity)
                   - PHPExcel_Calculation_DateTime::_getDateValue($settlement));
        }
    ...
    public static function TBILLPRICE($settlement, $maturity, $discount)
    {
        ...
        if (is_numeric($discount)) {
            ...
            $price = 100 * (1 - (($discount * $daysBetweenSettlementAndMaturity)
                                 / 360));
            if ($price <= 0) {
                return PHPExcel_Calculation_Functions::NaN();
            }
    ...
    public static function XNPV($rate, $values, $dates)
    {
        ...
        for ($i = 0; $i < $valCount; ++$i) {
            ...
            $xnpv += $values[$i] / pow(1 + $rate,
                    PHPExcel_Calculation_DateTime::DATEDIF($dates[0],
                        $dates[$i], 'd') / 365);
        }

        return (is_finite($xnpv)) ? $xnpv
            : PHPExcel_Calculation_Functions::VALUE();
    }    //	function XNPV()
     ...
     * @return    float
     */
    public static function YIELDMAT(
        $settlement,
        $maturity,
        $issue,
        $rate,
