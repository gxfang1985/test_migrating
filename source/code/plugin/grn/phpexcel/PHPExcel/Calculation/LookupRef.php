...
class PHPExcel_Calculation_LookupRef
{
     ...
     * @return    string
     */
    public static function CELL_ADDRESS(
        $row,
        $column,
        $relativity = 1,
        $referenceStyle = true,
        ...
        $column
            = PHPExcel_Calculation_Functions::flattenSingleValue($column);
        $relativity
            = PHPExcel_Calculation_Functions::flattenSingleValue($relativity);
        $sheetText
            = PHPExcel_Calculation_Functions::flattenSingleValue($sheetText);
    ...
    public static function COLUMNS($cellAddress = null)
    {
        ...
        list($columns, $rows)
            = PHPExcel_Calculation::_getMatrixDimensions($cellAddress);
    ...
    public static function HYPERLINK(
        ...
        $linkURL = (is_null($linkURL)) ? ''
            : PHPExcel_Calculation_Functions::flattenSingleValue($linkURL);
        $displayName = (is_null($displayName)) ? ''
            : PHPExcel_Calculation_Functions::flattenSingleValue($displayName);
    ...
    public static function INDIRECT(
        ...
        if (( ! preg_match('/^'
            ...
            if (strpos($cellAddress, '!') !== false) {
                ...
                $pSheet = $pCell->getWorksheet()->getParent()
        ...
        return PHPExcel_Calculation::getInstance()
                                   ->extractCellRange($cellAddress, $pSheet,
                                       false);
    }    //	function INDIRECT()
    ...
    public static function OFFSET(
        ...
        list($startCellColumn, $startCellRow)
            = PHPExcel_Cell::coordinateFromString($startCell);
        list($endCellColumn, $endCellRow)
            = PHPExcel_Cell::coordinateFromString($endCell);

        $startCellRow += $rows;
        $startCellColumn
            = PHPExcel_Cell::columnIndexFromString($startCellColumn)
              - 1;
        ...
        $endCellColumn = PHPExcel_Cell::columnIndexFromString($endCellColumn)
                         - 1;
        ...
        $startCellColumn
            = PHPExcel_Cell::stringFromColumnIndex($startCellColumn);
    ...
    public static function CHOOSE()
    {
        ...
        $chosenEntry
            = PHPExcel_Calculation_Functions::flattenArray(array_shift($chooseArgs));
    ...
    {
        $lookup_array
            = PHPExcel_Calculation_Functions::flattenArray($lookup_array);
        $lookup_value
            = PHPExcel_Calculation_Functions::flattenSingleValue($lookup_value);
        $match_type = (is_null($match_type)) ? 1
            : (int)PHPExcel_Calculation_Functions::flattenSingleValue($match_type);
        //	MATCH is not case sensitive
        $lookup_value = strtolower($lookup_value);

        //	lookup_value type has to be number, text, or logical values
        if (( ! is_numeric($lookup_value)) && ( ! is_string($lookup_value))
            && ( ! is_bool($lookup_value))
        ) {
...
//				echo '$i='.$i.'<br />';
    ...
    private static function _vlookupSort($a, $b)
    {
        ...
        return (strtolower($a[$firstColumn]) < strtolower($b[$firstColumn]))
            ? -1 : 1;
     ...
     */
    public static function VLOOKUP(
        $lookup_value,
        $lookup_array,
        $index_number,
     ...
     * @return   mixed           The value of the found cell
     */
    public static function HLOOKUP(
        $lookup_value,
        $lookup_array,
        $index_number,
        $not_exact_match = true
