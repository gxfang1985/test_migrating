...
class PHPExcel_Calculation_Statistical
{
    ...
    private static function _incompleteBeta($x, $p, $q)
    {
        if ($x <= 0.0) {
            ...
            return 1.0;
                  ...
                  || (($p + $q) > LOG_GAMMA_X_MAX_VALUE)
        ) {
            return 0.0;
        }
        $beta_gam = exp((0 - self::_logBeta($p, $q)) + $p * log($x) + $q
                                                                      * log(1.0
                                                                            - $x));
        if ($x < ($p + 1.0) / ($p + $q + 2.0)) {
    ...
    private static function _logBeta($p, $q)
    {
        if ($p != self::$_logBetaCache_p || $q != self::$_logBetaCache_q) {
            ...
            if (($p <= 0.0) || ($q <= 0.0)
                ...
                self::$_logBetaCache_result = self::_logGamma($p)
                                              + self::_logGamma($q)
                                              - self::_logGamma($p + $q);
            }
        }
    ...
    {
        $aArgs
            = PHPExcel_Calculation_Functions::flattenArrayIndexed(func_get_args());
    ...
    public static function AVERAGEIF($aArgs, $condition, $averageArgs = [])
    {
        ...
        $averageArgs
            = PHPExcel_Calculation_Functions::flattenArray($averageArgs);
    ...
    public static function BETADIST($value, $alpha, $beta, $rMin = 0, $rMax = 1)
    {
        ...
        if ((is_numeric($value)) && (is_numeric($alpha)) && (is_numeric($beta))
            ...
            if (($value < $rMin) || ($value > $rMax) || ($alpha <= 0)
                || ($beta <= 0)
                || ($rMin == $rMax)
            ) {
                return PHPExcel_Calculation_Functions::NaN();
     ...
     *
     */
    public static function BETAINV(
        $probability,
        $alpha,
        $beta,
        $rMin = 0,
        ...
        $probability
            = PHPExcel_Calculation_Functions::flattenSingleValue($probability);
        $alpha = PHPExcel_Calculation_Functions::flattenSingleValue($alpha);
        $beta = PHPExcel_Calculation_Functions::flattenSingleValue($beta);
        $rMin = PHPExcel_Calculation_Functions::flattenSingleValue($rMin);
        $rMax = PHPExcel_Calculation_Functions::flattenSingleValue($rMax);
    ...
    {
        $value
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($value));
        $trials
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($trials));
        $probability
            = PHPExcel_Calculation_Functions::flattenSingleValue($probability);
        ...
        if ((is_numeric($value)) && (is_numeric($trials))
            ...
            if ((is_numeric($cumulative)) || (is_bool($cumulative))) {
                if ($cumulative) {
                    $summer = 0;
                    for ($i = 0; $i <= $value; ++$i) {
                        $summer += PHPExcel_Calculation_MathTrig::COMBIN($trials,
                                $i) * pow($probability, $i) * pow(1
                                                                  - $probability,
                                $trials - $i);
                    }
                    ...
                    return PHPExcel_Calculation_MathTrig::COMBIN($trials,
                            $value) * pow($probability, $value) * pow(1
                                                                      - $probability,
                            $trials - $value);
                }
            }
    ...
    public static function CHIDIST($value, $degrees)
    {
        $value = PHPExcel_Calculation_Functions::flattenSingleValue($value);
        $degrees
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($degrees));
        ...
        if ((is_numeric($value)) && (is_numeric($degrees))) {
            ...
            return 1 - (self::_incompleteGamma($degrees / 2, $value / 2)
                        / self::_gamma($degrees / 2));
        }
    ...
    public static function CONFIDENCE($alpha, $stdDev, $size)
    {
        ...
        $size
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($size));
    ...
    {
        if ((is_null($xValues)) || ( ! is_array($yValues))
            || ( ! is_array($xValues))
        ) {
            return PHPExcel_Calculation_Functions::VALUE();
        }
    ...
    public static function CRITBINOM($trials, $probability, $alpha)
    {
        ...
        $probability
            = PHPExcel_Calculation_Functions::flattenSingleValue($probability);
        $alpha
            = PHPExcel_Calculation_Functions::flattenSingleValue($alpha);
        ...
        if ((is_numeric($trials)) && (is_numeric($probability))
            ...
            if ($alpha <= 0.5) {
                ...
                $trialsApprox = 0 - ($t + (2.515517 + 0.802853 * $t + 0.010328
                                                                      * $t * $t)
                                          / (1 + 1.432788 * $t + 0.189269 * $t
                                                                 * $t + 0.001308
                                                                        * $t
                                                                        * $t
                                                                        * $t));
                ...
                $trialsApprox = $t - (2.515517 + 0.802853 * $t + 0.010328 * $t
                                                                 * $t) / (1
                                                                          + 1.432788
                                                                            * $t
                                                                          + 0.189269
            ...
            $Guess = floor($trials * $probability + $trialsApprox * sqrt($trials
                                                                         * $probability
                                                                         * (1
                                                                            - $probability)));
            if ($Guess < 0) {
            ...
            $TotalUnscaledProbability
                = $UnscaledPGuess = $UnscaledCumPGuess = 0.0;
...
//			$CumPGuessMinus1 = $CumPGuess - $PGuess;
    ...
    public static function EXPONDIST($value, $lambda, $cumulative)
    {
        $value = PHPExcel_Calculation_Functions::flattenSingleValue($value);
        $lambda
            = PHPExcel_Calculation_Functions::flattenSingleValue($lambda);
        $cumulative
            = PHPExcel_Calculation_Functions::flattenSingleValue($cumulative);
    ...
    public static function GAMMADIST($value, $a, $b, $cumulative)
    {
        ...
        if ((is_numeric($value)) && (is_numeric($a)) && (is_numeric($b))) {
            ...
            if ((is_numeric($cumulative)) || (is_bool($cumulative))) {
                if ($cumulative) {
                    return self::_incompleteGamma($a, $value / $b)
                           / self::_gamma($a);
                } else {
    ...
    public static function GAMMAINV($probability, $alpha, $beta)
    {
        ...
        if ((is_numeric($probability)) && (is_numeric($alpha))
            ...
            if (($alpha <= 0) || ($beta <= 0) || ($probability < 0)
                || ($probability > 1)
            ) {
                return PHPExcel_Calculation_Functions::NaN();
            }
    ...
    public static function GROWTH(
        ...
        $bestFitExponential
            = trendClass::calculate(trendClass::TREND_EXPONENTIAL, $yValues,
            $xValues, $const);
        if (empty($newValues)) {
     ...
     *
     */
    public static function HYPGEOMDIST(
        $sampleSuccesses,
        $sampleNumber,
        $populationSuccesses,
        $populationNumber
        ...
        $sampleSuccesses
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($sampleSuccesses));
        $sampleNumber
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($sampleNumber));
        $populationSuccesses
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($populationSuccesses));
        $populationNumber
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($populationNumber));
        ...
        if ((is_numeric($sampleSuccesses)) && (is_numeric($sampleNumber))
            ...
            return PHPExcel_Calculation_MathTrig::COMBIN($populationSuccesses,
                    $sampleSuccesses) *
                   PHPExcel_Calculation_MathTrig::COMBIN($populationNumber
                                                         - $populationSuccesses,
    ...
    public static function LINEST(
        ...
        $const = (is_null($const)) ? true
            : (boolean)PHPExcel_Calculation_Functions::flattenSingleValue($const);
        $stats = (is_null($stats)) ? false
            : (boolean)PHPExcel_Calculation_Functions::flattenSingleValue($stats);
        if (is_null($xValues)) {
            $xValues = range(1,
                count(PHPExcel_Calculation_Functions::flattenArray($yValues)));
        }
    ...
    public static function MEDIAN()
    {
        ...
        if ($mValueCount > 0) {
            ...
            if ($mValueCount == floor($mValueCount)) {
                $returnValue = ($mArgs[$mValueCount--] + $mArgs[$mValueCount])
                               / 2;
            } else {
    ...
    {
        $failures
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($failures));
        $successes
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($successes));
        $probability
            = PHPExcel_Calculation_Functions::flattenSingleValue($probability);
        ...
        if ((is_numeric($failures)) && (is_numeric($successes))
            ...
            return (PHPExcel_Calculation_MathTrig::COMBIN($failures + $successes
                                                          - 1, $successes - 1))
                   * (pow($probability, $successes)) * (pow(1 - $probability,
                    $failures));
        }
    ...
    public static function NORMDIST($value, $mean, $stdDev, $cumulative)
    {
        ...
        if ((is_numeric($value)) && (is_numeric($mean))
            ...
            if ((is_numeric($cumulative)) || (is_bool($cumulative))) {
                if ($cumulative) {
                    return 0.5 * (1
                                  + PHPExcel_Calculation_Engineering::_erfVal(($value
                                                                               - $mean)
                                                                              / ($stdDev
                                                                                 * sqrt(2))));
                } else {
                    return (1 / (SQRT2PI * $stdDev)) * exp(0 - (pow($value
                                                                    - $mean, 2)
                                                                / (2 * ($stdDev
                                                                        * $stdDev))));
                }
    ...
    public static function PERCENTRANK($valueSet, $value, $significance = 3)
    {
        ...
        $value
            = PHPExcel_Calculation_Functions::flattenSingleValue($value);
    ...
    public static function PERMUT($numObjs, $numInSet)
    {
        ...
        $numInSet
            = PHPExcel_Calculation_Functions::flattenSingleValue($numInSet);
        ...
        if ((is_numeric($numObjs)) && (is_numeric($numInSet))) {
            ...
            return round(PHPExcel_Calculation_MathTrig::FACT($numObjs)
                         / PHPExcel_Calculation_MathTrig::FACT($numObjs
                                                               - $numInSet));
        }
            ...
            if ((is_numeric($cumulative)) || (is_bool($cumulative))) {
                if ($cumulative) {
                    ...
                    for ($i = 0; $i <= floor($value); ++$i) {
                        $summer += pow($mean, $i)
                                   / PHPExcel_Calculation_MathTrig::FACT($i);
                    }

                    return exp(0 - $mean) * $summer;
                } else {
                    return (exp(0 - $mean) * pow($mean, $value))
                           / PHPExcel_Calculation_MathTrig::FACT($value);
                }
            }
        }
    ...
    public static function TDIST($value, $degrees, $tails)
    {
        ...
        $degrees
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($degrees));
        $tails
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($tails));
        ...
        if ((is_numeric($value)) && (is_numeric($alpha))
            ...
            if ((is_numeric($cumulative)) || (is_bool($cumulative))) {
                if ($cumulative) {
                    ...
                    return ($alpha / pow($beta, $alpha)) * pow($value,
                            $alpha - 1) * exp(0 - pow($value / $beta, $alpha));
                }
            }
        }
    ...
    public static function ZTEST($dataSet, $m0, $sigma = null)
    {
        ...
        return 1 - self::NORMSDIST((self::AVERAGE($dataSet) - $m0) / ($sigma
                                                                      / SQRT($n)));
    }    //	function ZTEST()
