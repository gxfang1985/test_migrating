...
class PHPExcel_Calculation_DateTime
{
     ...
     */
    private static function _dateDiff360(
        $startDay,
        $startMonth,
        $startYear,
        $endDay,
    ...
    private static function _adjustDateByMonths(
        $dateValue = 0,
        $adjustmentMonths = 0
    ) {
        // Execute function
    ...
    public static function DATE($year = 0, $month = 1, $day = 1)
    {
        ...
        // Execute function
        $excelDateValue = PHPExcel_Shared_Date::FormattedPHPToExcel($year,
            $month, $day);
        switch (PHPExcel_Calculation_Functions::getReturnDateType()) {
            case PHPExcel_Calculation_Functions::RETURNDATE_EXCEL :
    ...
    public static function TIME($hour = 0, $minute = 0, $second = 0)
    {
        ...
        switch (PHPExcel_Calculation_Functions::getReturnDateType()) {
            case PHPExcel_Calculation_Functions::RETURNDATE_EXCEL :
                ...
                return (float)PHPExcel_Shared_Date::FormattedPHPToExcel($calendar,
                    1, $date, $hour, $minute, $second);
            case PHPExcel_Calculation_Functions::RETURNDATE_PHP_NUMERIC :
                return (integer)PHPExcel_Shared_Date::ExcelToPHP(PHPExcel_Shared_Date::FormattedPHPToExcel(1970,
                    1, 1, $hour, $minute,
            ...
            case PHPExcel_Calculation_Functions::RETURNDATE_PHP_OBJECT :
                ...
                $phpDateObject = new DateTime('1900-01-01 ' . $hour . ':'
                                              . $minute . ':' . $second);
                if ($dayAdjust != 0) {
    ...
    public static function DATEVALUE($dateValue = 1)
    {
        ...
        $dateValue = preg_replace('/(\d)(st|nd|rd|th)([ -\/])/Ui', '$1$3',
            $dateValue);
        ...
        if (($PHPDateArray === false) || ($PHPDateArray['error_count'] > 0)) {
            ...
            $PHPDateArray = date_parse($testVal1 . '-' . $testVal2 . '-'
                                       . $testVal3);
            if (($PHPDateArray === false)
                || ($PHPDateArray['error_count'] > 0)
            ) {
                $PHPDateArray = date_parse($testVal2 . '-' . $testVal1 . '-'
                                           . $testVal3);
                if (($PHPDateArray === false)
                    || ($PHPDateArray['error_count'] > 0)
                ) {
        ...
        if (($PHPDateArray !== false) && ($PHPDateArray['error_count'] == 0)) {
            ...
            $excelDateValue
                = floor(PHPExcel_Shared_Date::FormattedPHPToExcel($PHPDateArray['year'],
                $PHPDateArray['month'], $PHPDateArray['day'],
    ...
    public static function TIMEVALUE($timeValue)
    {
        ...
        if (($PHPDateArray !== false) && ($PHPDateArray['error_count'] == 0)) {
            if (PHPExcel_Calculation_Functions::getCompatibilityMode()
                ...
                $excelDateValue
                    = PHPExcel_Shared_Date::FormattedPHPToExcel($PHPDateArray['year'],
                    $PHPDateArray['month'], $PHPDateArray['day'],
                    $PHPDateArray['hour'], $PHPDateArray['minute'],
                    $PHPDateArray['second']);
            } else {
                $excelDateValue
                    = PHPExcel_Shared_Date::FormattedPHPToExcel(1900, 1, 1,
                        $PHPDateArray['hour'], $PHPDateArray['minute'],
                ...
                case PHPExcel_Calculation_Functions::RETURNDATE_PHP_NUMERIC :
                    return (integer)$phpDateValue
                        = PHPExcel_Shared_Date::ExcelToPHP($excelDateValue
                                                           + 25569) - 3600;;
                case PHPExcel_Calculation_Functions::RETURNDATE_PHP_OBJECT :
                    return new DateTime('1900-01-01 ' . $PHPDateArray['hour']
                                        . ':' . $PHPDateArray['minute'] . ':'
                                        . $PHPDateArray['second']);
            }
        }
    ...
    public static function DATEDIF($startDate = 0, $endDate = 0, $unit = 'D')
    {
        ...
        switch ($unit) {
            case 'D':
                ...
                break;
            case 'M':
                $retVal = intval($endMonths - $startMonths) + (intval($endYears
                                                                      - $startYears)
                                                               * 12);
            ...
            case 'YD':
                ...
                if ($endYears > $startYears) {
                    ...
                    $retVal = $PHPEndDateObject->format('z')
                              - $PHPStartDateObject->format('z');
                    if ($retVal < 0) {
                        $retVal += 365;
                    }
                }
     ...
     */
    public static function DAYS360(
        $startDate = 0,
        $endDate = 0,
        $method = false
    ) {
        ...
        return self::_dateDiff360($startDay, $startMonth, $startYear, $endDay,
            $endMonth, $endYear, ! $method);
    }    //	function DAYS360()
                    ...
                    if ($years == 1) {
                        ...
                        for ($year = $startYear; $year <= $endYear; ++$year) {
                            if ($year == $startYear) {
                                ...
                                if ($startMonth < 3) {
                                    $leapDays += (self::_isLeapYear($year)) ? 1
                                        : 0;
                                }
                            } elseif ($year == $endYear) {
                                $endMonth = self::MONTHOFYEAR($endDate);
                                $endDay = self::DAYOFMONTH($endDate);
                                if (($endMonth * 100 + $endDay) >= (2 * 100
                                                                    + 29)
                                ) {
                                    $leapDays += (self::_isLeapYear($year)) ? 1
                                        : 0;
    ...
    public static function WORKDAY($startDate, $endDays)
    {
        ...
        //	Add endDays
        $endDate = (float)$startDate + (intval($endDays / 5) * 7) + ($endDays
                                                                     % 5);
        ...
        if ( ! empty($dateArgs)) {
            ...
            foreach ($dateArgs as $holidayDate) {
                if (($holidayDate !== null) && (trim($holidayDate) > '')) {
                    if (is_string(
                        $holidayDate = self::_getDateValue($holidayDate))
                    ) {
                        return PHPExcel_Calculation_Functions::VALUE();
                    }
    ...
    {
        $dateValue
            = PHPExcel_Calculation_Functions::flattenSingleValue($dateValue);
    ...
    {
        $timeValue
            = PHPExcel_Calculation_Functions::flattenSingleValue($timeValue);
    ...
    public static function EDATE($dateValue = 1, $adjustmentMonths = 0)
    {
        $dateValue
            = PHPExcel_Calculation_Functions::flattenSingleValue($dateValue);
        $adjustmentMonths
            = PHPExcel_Calculation_Functions::flattenSingleValue($adjustmentMonths);
        ...
        // Execute function
        $PHPDateObject = self::_adjustDateByMonths($dateValue,
            $adjustmentMonths);
