...
class PHPExcel_Calculation_Engineering
{
    ...
    public static function _parseComplex($complexNumber)
    {
        ...
        if (strlen($workString) > 0) {
            $leadingSign = (($workString{0} == '+') || ($workString{0} == '-'))
                ? 1 : 0;
        }
        $power = '';
        ...
        $realNumber = substr($workString, 0,
            strlen($realNumber) + strlen($power) + $leadingSign);
    ...
    {
        $x = (is_null($x)) ? 0.0
            : PHPExcel_Calculation_Functions::flattenSingleValue($x);
        $ord = (is_null($ord)) ? 0.0
            : PHPExcel_Calculation_Functions::flattenSingleValue($ord);
        ...
        if ((is_numeric($x)) && (is_numeric($ord))) {
            ...
            if (abs($x) <= 30) {
                $fResult
                    = $fTerm = pow($x / 2, $ord)
                               / PHPExcel_Calculation_MathTrig::FACT($ord);
                $ordK = 1;
                $fSqrX = ($x * $x) / 4;
                do {
                    $fTerm *= $fSqrX;
            ...
            return (is_nan($fResult)) ? PHPExcel_Calculation_Functions::NaN()
                : $fResult;
        }
    ...
    public static function BESSELJ($x, $ord)
    {
        ...
        if ((is_numeric($x)) && (is_numeric($ord))) {
            ...
            if (abs($x) <= 30) {
                ...
                $fXAbs = abs($x);
                $fResult = sqrt(M_2DIVPI / $fXAbs) * cos($fXAbs - $ord
                                                                  * $f_PI_DIV_2
                                                         - $f_PI_DIV_4);
                if (($ord & 1) && ($x < 0)) {
                    $fResult = -$fResult;
    ...
    public static function BESSELK($x, $ord)
    {
        ...
        if ((is_numeric($x)) && (is_numeric($ord))) {
            ...
            return (is_nan($fBk)) ? PHPExcel_Calculation_Functions::NaN()
                : $fBk;
        }
    ...
    private static function _Bessely0($fNum)
    {
        if ($fNum < 8.0) {
            $y = ($fNum * $fNum);
            $f1 = -2957821389.0 + $y * (7062834065.0 + $y * (-512359803.6 + $y
                                                                            * (10879881.29
                                                                               + $y
                                                                                 * (-86327.92757
                                                                                    + $y
                                                             ...
                                                             * (-0.6911147651e-5
                                                                + $y
                                                                  * (0.7621095161e-6
                                                                     + $y
                                                                       * (-0.934945152e-7))));
            $fRet = sqrt(0.636619772 / $fNum) * (sin($xx) * $f1 + $z * cos($xx)
                                                                  * $f2);
        }
    ...
    public static function BESSELY($x, $ord)
    {
        ...
        if ((is_numeric($x)) && (is_numeric($ord))) {
            ...
            return (is_nan($fBy)) ? PHPExcel_Calculation_Functions::NaN()
                : $fBy;
        }
    ...
    public static function COMPLEX(
        ...
        $realNumber = (is_null($realNumber)) ? 0.0
            : PHPExcel_Calculation_Functions::flattenSingleValue($realNumber);
        $imaginary = (is_null($imaginary)) ? 0.0
            : PHPExcel_Calculation_Functions::flattenSingleValue($imaginary);
        $suffix = (is_null($suffix)) ? 'i'
            : PHPExcel_Calculation_Functions::flattenSingleValue($suffix);
    ...
    {
        $complexNumber
            = PHPExcel_Calculation_Functions::flattenSingleValue($complexNumber);
    ...
    public static function IMARGUMENT($complexNumber)
    {
        ...
        if ($parsedComplex['real'] == 0.0) {
            ...
            return 0 - (M_PI - atan(abs($parsedComplex['imaginary'])
                                    / abs($parsedComplex['real'])));
        } else {
            return M_PI - atan($parsedComplex['imaginary']
                               / abs($parsedComplex['real']));
        }
    }    //	function IMARGUMENT()
    ...
    public static function IMSIN($complexNumber)
    {
        ...
        if ($parsedComplex['imaginary'] == 0.0) {
            ...
            return self::COMPLEX(sin($parsedComplex['real'])
                                 * cosh($parsedComplex['imaginary']),
    ...
    public static function IMPOWER($complexNumber, $realNumber)
    {
        ...
        if ($theta == 0) {
            ...
            return self::COMPLEX($rPower * cos($theta), $rPower * sin($theta),
                $parsedComplex['suffix']);
        } else {
            return self::COMPLEX($rPower * cos($theta), $rPower * sin($theta),
                $parsedComplex['suffix']);
    ...
    public static function IMDIV($complexDividend, $complexDivisor)
    {
        ...
        $complexDivisor
            = PHPExcel_Calculation_Functions::flattenSingleValue($complexDivisor);

        $parsedComplexDividend = self::_parseComplex($complexDividend);
        $parsedComplexDivisor = self::_parseComplex($complexDivisor);

        if (($parsedComplexDividend['suffix'] != '')
            && ($parsedComplexDivisor['suffix'] != '')
            && ($parsedComplexDividend['suffix']
                != $parsedComplexDivisor['suffix'])
        ) {
            return PHPExcel_Calculation_Functions::NaN();
        }
        ...
        $d1 = ($parsedComplexDividend['real'] * $parsedComplexDivisor['real'])
              + ($parsedComplexDividend['imaginary']
                 * $parsedComplexDivisor['imaginary']);
        $d2 = ($parsedComplexDividend['imaginary']
               * $parsedComplexDivisor['real'])
              - ($parsedComplexDividend['real']
                 * $parsedComplexDivisor['imaginary']);
    ...
    {
        $complexNumber1
            = PHPExcel_Calculation_Functions::flattenSingleValue($complexNumber1);
        $complexNumber2
            = PHPExcel_Calculation_Functions::flattenSingleValue($complexNumber2);

        $parsedComplex1 = self::_parseComplex($complexNumber1);
        $parsedComplex2 = self::_parseComplex($complexNumber2);

        if ((($parsedComplex1['suffix'] != '')
             && ($parsedComplex2['suffix'] != ''))
            && ($parsedComplex1['suffix'] != $parsedComplex2['suffix'])
        ) {
            return PHPExcel_Calculation_Functions::NaN();
        } elseif (($parsedComplex1['suffix'] == '')
                  && ($parsedComplex2['suffix'] != '')
        ) {
            $parsedComplex1['suffix'] = $parsedComplex2['suffix'];
        }
    ...
    public static function IMSUM()
    {
        ...
        return self::COMPLEX($returnValue['real'], $returnValue['imaginary'],
            $activeSuffix);
    }    //	function IMSUM()
    ...
    public static function IMPRODUCT()
    {
        ...
        foreach ($aArgs as $arg) {
            ...
            $returnValue['real'] = ($workValue['real']
                                    * $parsedComplex['real'])
                                   - ($workValue['imaginary']
                                      * $parsedComplex['imaginary']);
            $returnValue['imaginary'] = ($workValue['real']
                                         * $parsedComplex['imaginary'])
                                        + ($workValue['imaginary']
                                           * $parsedComplex['real']);
        }
    ...
    public static function CONVERTUOM($value, $fromUOM, $toUOM)
    {
        ...
        if (isset(self::$_conversionUnits[$fromUOM])) {
            ...
            if (isset(self::$_conversionMultipliers[$fromMultiplier])) {
                $fromMultiplier
                    = self::$_conversionMultipliers[$fromMultiplier]['multiplier'];
            } else {
        ...
        if (isset(self::$_conversionUnits[$toUOM])) {
            ...
            if (isset(self::$_conversionMultipliers[$toMultiplier])) {
                $toMultiplier
                    = self::$_conversionMultipliers[$toMultiplier]['multiplier'];
            } else {
                return PHPExcel_Calculation_Functions::NA();
            }
            if ((isset(self::$_conversionUnits[$toUOM]))
        ...
        return ($value * self::$_unitConversions[$unitGroup1][$fromUOM][$toUOM])
               / $toMultiplier;
    }    //	function CONVERTUOM()
