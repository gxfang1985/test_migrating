...
class PHPExcel_Calculation_MathTrig
{
    ...
    {
        $xCoordinate
            = PHPExcel_Calculation_Functions::flattenSingleValue($xCoordinate);
        $yCoordinate
            = PHPExcel_Calculation_Functions::flattenSingleValue($yCoordinate);

        $xCoordinate = ($xCoordinate !== null) ? $xCoordinate : 0.0;
        $yCoordinate = ($yCoordinate !== null) ? $yCoordinate : 0.0;

        if (((is_numeric($xCoordinate)) || (is_bool($xCoordinate)))
            && ((is_numeric($yCoordinate)))
            || (is_bool($yCoordinate))
        ) {
    ...
    public static function CEILING($number, $significance = null)
    {
        ...
        $significance
            = PHPExcel_Calculation_Functions::flattenSingleValue($significance);
    ...
    {
        $factLoop
            = PHPExcel_Calculation_Functions::flattenSingleValue($factVal);
    ...
    public static function GCD()
    {
        ...
        foreach (
            PHPExcel_Calculation_Functions::flattenArray(func_get_args()) as
            $value
        ) {
            if ( ! is_numeric($value)) {
                return PHPExcel_Calculation_Functions::VALUE();
        ...
        for ($i = 1; $i < $allValuesCount; ++$i) {
            $mergedArray = array_intersect_key($mergedArray,
                $allValuesFactors[$i]);
        }
            ...
            foreach ($mergedArray as $mergedKey => $mergedValue) {
                foreach ($allValuesFactors as $highestPowerTest) {
                    foreach ($highestPowerTest as $testKey => $testValue) {
                        if (($testKey == $mergedKey)
                            && ($testValue < $mergedValue)
                        ) {
    ...
    {
        $number = PHPExcel_Calculation_Functions::flattenSingleValue($number);
        $base = (is_null($base)) ? 10
            : (float)PHPExcel_Calculation_Functions::flattenSingleValue($base);
    ...
    public static function MROUND($number, $multiple)
    {
        $number = PHPExcel_Calculation_Functions::flattenSingleValue($number);
        $multiple
            = PHPExcel_Calculation_Functions::flattenSingleValue($multiple);
    ...
    public static function POWER($x = 0, $y = 2)
    {
        ...
        return ( ! is_nan($result) && ! is_infinite($result)) ? $result
    ...
    public static function SUMPRODUCT()
    {
        ...
        $wrkArray
            = PHPExcel_Calculation_Functions::flattenArray(array_shift($arrayList));
    ...
    public static function TRUNC($value = 0, $digits = 0)
    {
        ...
        if (($digits > 0)
            && (rtrim(intval((abs($value) - abs(intval($value))) * $adjust),
                    '0') < $adjust / 10)
        ) {
            return $value;
        }
