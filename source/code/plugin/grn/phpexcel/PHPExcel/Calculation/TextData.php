...
class PHPExcel_Calculation_TextData
{
    ...
    private static function _uniord($c)
    {
        ...
        if (ord($c{0}) >= 224 && ord($c{0}) <= 239) {
            return (ord($c{0}) - 224) * 4096 + (ord($c{1}) - 128) * 64
                   + (ord($c{2}) - 128);
        }
        if (ord($c{0}) >= 240 && ord($c{0}) <= 247) {
            return (ord($c{0}) - 240) * 262144 + (ord($c{1}) - 128) * 4096
                   + (ord($c{2}) - 128) * 64 + (ord($c{3}) - 128);
        }
        if (ord($c{0}) >= 248 && ord($c{0}) <= 251) {
            return (ord($c{0}) - 248) * 16777216 + (ord($c{1}) - 128) * 262144
                   + (ord($c{2}) - 128) * 4096 + (ord($c{3}) - 128) * 64
                   + (ord($c{4}) - 128);
        }
        if (ord($c{0}) >= 252 && ord($c{0}) <= 253) {
            return (ord($c{0}) - 252) * 1073741824 + (ord($c{1}) - 128)
                                                     * 16777216 + (ord($c{2})
                                                                   - 128)
                                                                  * 262144
                   + (ord($c{3}) - 128) * 4096 + (ord($c{4}) - 128) * 64
                   + (ord($c{5}) - 128);
        }
    ...
    {
        $character
            = PHPExcel_Calculation_Functions::flattenSingleValue($character);
        ...
        if (function_exists('mb_convert_encoding')) {
            return mb_convert_encoding('&#' . intval($character) . ';', 'UTF-8',
                'HTML-ENTITIES');
        } else {
    ...
    {
        $stringValue
            = PHPExcel_Calculation_Functions::flattenSingleValue($stringValue);

        if (is_bool($stringValue)) {
            return ($stringValue) ? PHPExcel_Calculation::getTRUE()
        ...
        if (is_string($stringValue) || is_numeric($stringValue)) {
            return str_replace(self::$_invalidChars, '',
    ...
    public static function ASCIICODE($characters)
    {
        ...
        $characters
            = PHPExcel_Calculation_Functions::flattenSingleValue($characters);
        if (is_bool($characters)) {
            if (PHPExcel_Calculation_Functions::getCompatibilityMode()
                ...
                $characters = ($characters) ? PHPExcel_Calculation::getTRUE()
                    : PHPExcel_Calculation::getFALSE();
            }
        }
        ...
        foreach ($aArgs as $arg) {
            if (is_bool($arg)) {
                if (PHPExcel_Calculation_Functions::getCompatibilityMode()
                    ...
                    $arg = ($arg) ? PHPExcel_Calculation::getTRUE()
                        : PHPExcel_Calculation::getFALSE();
                }
            }
            $returnValue .= $arg;
    ...
    public static function DOLLAR($value = 0, $decimals = 2)
    {
        ...
        $decimals = is_null($decimals) ? 0
            : PHPExcel_Calculation_Functions::flattenSingleValue($decimals);
    ...
    public static function SEARCHSENSITIVE($needle, $haystack, $offset = 1)
    {
        ...
        $haystack
            = PHPExcel_Calculation_Functions::flattenSingleValue($haystack);
        $offset = PHPExcel_Calculation_Functions::flattenSingleValue($offset);

        if ( ! is_bool($needle)) {
            if (is_bool($haystack)) {
                $haystack = ($haystack) ? PHPExcel_Calculation::getTRUE()
                    : PHPExcel_Calculation::getFALSE();
            }

            if (($offset > 0)
                && (PHPExcel_Shared_String::CountCharacters($haystack)
                    > $offset)
            ) {
     ...
     * @return    boolean
     */
    public static function FIXEDFORMAT(
        $value,
        $decimals = 2,
        $no_commas = false
    ) {
        ...
        $decimals
            = PHPExcel_Calculation_Functions::flattenSingleValue($decimals);
        $no_commas
            = PHPExcel_Calculation_Functions::flattenSingleValue($no_commas);
    ...
    public static function LEFT($value = '', $chars = 1)
    {
        ...
        if (is_bool($value)) {
            $value = ($value) ? PHPExcel_Calculation::getTRUE()
                : PHPExcel_Calculation::getFALSE();
        }
    ...
    public static function RIGHT($value = '', $chars = 1)
    {
        ...
        if ((function_exists('mb_substr')) && (function_exists('mb_strlen'))) {
            return mb_substr($value, mb_strlen($value, 'UTF-8') - $chars,
                $chars, 'UTF-8');
        } else {
    ...
    {
        $mixedCaseString
            = PHPExcel_Calculation_Functions::flattenSingleValue($mixedCaseString);
    ...
    public static function REPLACE(
        ...
        $right = self::RIGHT($oldText,
            self::STRINGLENGTH($oldText) - ($start + $chars) + 1);
    ...
    public static function SUBSTITUTE(
        ...
        $fromText
            = PHPExcel_Calculation_Functions::flattenSingleValue($fromText);
        $toText = PHPExcel_Calculation_Functions::flattenSingleValue($toText);
        $instance
            = floor(PHPExcel_Calculation_Functions::flattenSingleValue($instance));
        ...
        if ($instance == 0) {
            ...
            if ($pos !== false) {
                if (function_exists('mb_strlen')) {
                    return self::REPLACE($text, ++$pos,
                        mb_strlen($fromText, 'UTF-8'), $toText);
                } else {
                    return self::REPLACE($text, ++$pos, strlen($fromText),
                        $toText);
                }
            }
        }
    ...
    {
        $testValue
            = PHPExcel_Calculation_Functions::flattenSingleValue($testValue);
    ...
    public static function TEXTFORMAT($value, $format)
    {
        ...
        return (string)PHPExcel_Style_NumberFormat::toFormattedString($value,
            $format);
    }    //	function TEXTFORMAT()
