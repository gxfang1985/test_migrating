 ...
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Worksheet_RowCellIterator extends PHPExcel_Worksheet_CellIterator
    implements Iterator
{
    /**
     * Row index
    ...
    public function resetEnd($endColumn = null)
    {
        $endColumn = ($endColumn) ? $endColumn
            : $this->_subject->getHighestColumn();
        $this->_endColumn = PHPExcel_Cell::columnIndexFromString($endColumn)
                            - 1;
        $this->adjustForExistingOnlyRange();
    ...
    public function seek($column = 'A')
    {
        ...
        } elseif ($this->_onlyExistingCells
                  && ! ($this->_subject->cellExistsByColumnAndRow($column,
                $this->_rowIndex))
        ) {
    ...
    {
        return $this->_subject->getCellByColumnAndRow($this->_position,
            $this->_rowIndex);
    }
    ...
    public function next()
    {
        do {
            ++$this->_position;
                 ...
                 && ( ! $this->_subject->cellExistsByColumnAndRow($this->_position,
            $this->_rowIndex))
    ...
    protected function adjustForExistingOnlyRange()
    {
        if ($this->_onlyExistingCells) {
            while (( ! $this->_subject->cellExistsByColumnAndRow($this->_startColumn,
                    $this->_rowIndex))
            ...
            while (( ! $this->_subject->cellExistsByColumnAndRow($this->_endColumn,
                    $this->_rowIndex))
