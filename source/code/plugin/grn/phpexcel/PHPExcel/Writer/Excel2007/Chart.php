    ...
    PHPExcel_Writer_Excel2007_WriterPart
{
    ...
    private function _writePlotArea(
        ...
        foreach ($chartTypes as $chartType) {
            ...
            for ($i = 0; $i < $groupCount; ++$i) {
                ...
                if ($groupType == $chartType) {
                    ...
                    $this->_writePlotGroup($plotGroup, $chartType, $objWriter,
                        $catIsMultiLevelSeries, $valIsMultiLevelSeries,
                        $plotGroupingType, $pSheet);
                }
            }
            ...
            if ($chartType === PHPExcel_Chart_DataSeries::TYPE_LINECHART) {
                ...
                $objWriter->endElement();
                      ...
                      || ($chartType
                          === PHPExcel_Chart_DataSeries::TYPE_BARCHART_3D)
            ) {
                ...
                if ($plotGroupingType == 'percentStacked'
                    ...
                    $objWriter->endElement();
                      ...
                      === PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART
            ) {
        ...
        if (($chartType !== PHPExcel_Chart_DataSeries::TYPE_PIECHART)
            ...
            if ($chartType === PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
                $this->_writeValAx($objWriter, $plotArea, $xAxisLabel,
                    $chartType, $id1, $id2, $catIsMultiLevelSeries, $xAxis,
                    $yAxis, $majorGridlines, $minorGridlines);
            } else {
                $this->_writeCatAx($objWriter, $plotArea, $xAxisLabel,
                    $chartType, $id1, $id2, $catIsMultiLevelSeries, $xAxis,
                    $yAxis);
            }

            $this->_writeValAx($objWriter, $plotArea, $yAxisLabel, $chartType,
                $id1, $id2, $valIsMultiLevelSeries, $xAxis, $yAxis,
    ...
    private function _writeValAx(
        ...
        if ( ! is_null($majorGridlines->getLineColorProperty('value'))) {
            ...
            if ( ! is_null($majorGridlines->getLineStyleProperty([
                ...
                $objWriter->writeAttribute('type',
                    $majorGridlines->getLineStyleProperty([
                        'arrow',
                        'head',
                        'type'
                    ]));
                $objWriter->writeAttribute('w',
                    $majorGridlines->getLineStyleArrowParameters('head', 'w'));
            ...
            if ( ! is_null($majorGridlines->getLineStyleProperty([
                ...
                $objWriter->writeAttribute('w',
                    $majorGridlines->getLineStyleArrowParameters('end', 'w'));
                $objWriter->writeAttribute('len',
                    $majorGridlines->getLineStyleArrowParameters('end', 'len'));
                $objWriter->endElement();
            }
        ...
        if ( ! is_null($majorGridlines->getShadowProperty('presets'))) {
            ...
            $objWriter->writeAttribute('val',
                $majorGridlines->getShadowProperty(['color', 'value']));

            $objWriter->startElement('a:alpha');
            $objWriter->writeAttribute('val',
                $majorGridlines->getShadowProperty(['color', 'alpha']));
            $objWriter->endElement(); //end alpha
        ...
        if ($minorGridlines->getObjectState()) {
            ...
            if ( ! is_null($minorGridlines->getLineColorProperty('value'))) {
                ...
                if ( ! is_null($minorGridlines->getLineStyleProperty([
                    ...
                    $objWriter->startElement('a:headEnd');
                    $objWriter->writeAttribute('type',
                        $minorGridlines->getLineStyleProperty([
                            'arrow',
                            'head',
                            'type'
                        ]));
            ...
            if ( ! is_null($minorGridlines->getShadowProperty('presets'))) {
                ...
                $objWriter->writeAttribute('val',
                    $minorGridlines->getShadowProperty(['color', 'value']));
                $objWriter->startElement('a:alpha');
                $objWriter->writeAttribute('val',
                    $minorGridlines->getShadowProperty(['color', 'alpha']));
                $objWriter->endElement(); //end alpha
                $objWriter->endElement(); //end color:type
                $objWriter->endElement(); //end shadow
            }
        ...
        if ( ! is_null($xAxis->getLineStyleProperty([
            ...
            $objWriter->startElement('a:headEnd');
            $objWriter->writeAttribute('type',
                $xAxis->getLineStyleProperty(['arrow', 'head', 'type']));
            $objWriter->writeAttribute('w',
        ...
        if ( ! is_null($xAxis->getLineStyleProperty([
            ...
            $objWriter->startElement('a:tailEnd');
            $objWriter->writeAttribute('type',
                $xAxis->getLineStyleProperty(['arrow', 'end', 'type']));
            $objWriter->writeAttribute('w',
    ...
    private function _writePlotGroup(
        ...
        if (($groupType !== PHPExcel_Chart_DataSeries::TYPE_RADARCHART)
            ...
            if ($groupType !== PHPExcel_Chart_DataSeries::TYPE_LINECHART) {
                if (($groupType == PHPExcel_Chart_DataSeries::TYPE_PIECHART)
                    ...
                    || ($groupType
                        == PHPExcel_Chart_DataSeries::TYPE_DONUTCHART)
                    || ($plotSeriesCount > 1)
                ) {
        ...
        foreach ($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
            ...
            $objWriter->writeAttribute('val',
                $this->_seriesIndex + $plotSeriesIdx);
            $objWriter->endElement();

            $objWriter->startElement('c:order');
            $objWriter->writeAttribute('val',
                $this->_seriesIndex + $plotSeriesRef);
            $objWriter->endElement();
            ...
            if ($plotSeriesCategory
                ...
                $this->_writePlotSeriesValues($plotSeriesCategory, $objWriter,
                    $groupType, 'str', $pSheet);
                $objWriter->endElement();
            }
            ...
            if ($plotSeriesValues) {
                ...
                $this->_writePlotSeriesValues($plotSeriesValues, $objWriter,
                    $groupType, 'num', $pSheet);
                $objWriter->endElement();
            }
    ...
    private function _writePlotSeriesLabel($plotSeriesLabel, $objWriter)
    {
        ...
        foreach (
            $plotSeriesLabel->getDataValues() as $plotLabelKey =>
            $plotLabelValue
        ) {
    ...
    private function _writePlotSeriesValues(
        ...
        if ($plotSeriesValues->isMultiLevelSeries()) {
            ...
            for ($level = 0; $level < $levelCount; ++$level) {
                ...
                foreach (
                    $plotSeriesValues->getDataValues() as $plotSeriesKey =>
                    $plotSeriesValue
                ) {
     ...
     * @throws  PHPExcel_Writer_Exception
     */
    private function _writeBubbles(
        $plotSeriesValues,
        $objWriter,
        PHPExcel_Worksheet $pSheet
    ) {
