    ...
    extends PHPExcel_Writer_Excel2007_WriterPart
{
     ...
     * @throws    PHPExcel_Writer_Exception
     */
    private function _writeFileVersion(
     ...
     * @throws    PHPExcel_Writer_Exception
     */
    private function _writeWorkbookPr(
        PHPExcel_Shared_XMLWriter $objWriter = null
    ) {
        $objWriter->startElement('workbookPr');

        if (PHPExcel_Shared_Date::getExcelCalendar()
     ...
     * @throws    PHPExcel_Writer_Exception
     */
    private function _writeBookViews(
        PHPExcel_Shared_XMLWriter $objWriter = null,
        PHPExcel $pPHPExcel = null
    ) {
        // bookViews
     ...
     * @throws    PHPExcel_Writer_Exception
     */
    private function _writeWorkbookProtection(
        PHPExcel_Shared_XMLWriter $objWriter = null,
        PHPExcel $pPHPExcel = null
    ) {
     ...
     * @throws    PHPExcel_Writer_Exception
     */
    private function _writeDefinedNames(
        PHPExcel_Shared_XMLWriter $objWriter = null,
        PHPExcel $pPHPExcel = null
    ) {
        // Write defined names
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            ...
            $this->_writeDefinedNameForAutofilter($objWriter,
                $pPHPExcel->getSheet($i), $i);

            // definedName for Print_Titles
            $this->_writeDefinedNameForPrintTitles($objWriter,
                $pPHPExcel->getSheet($i), $i);

            // definedName for Print_Area
            $this->_writeDefinedNameForPrintArea($objWriter,
                $pPHPExcel->getSheet($i), $i);
        }
     ...
     * @throws    PHPExcel_Writer_Exception
     */
    private function _writeNamedRanges(
        PHPExcel_Shared_XMLWriter $objWriter = null,
        PHPExcel $pPHPExcel
    ) {
        // Loop named ranges
    ...
    private function _writeDefinedNameForNamedRange(
        ...
        if ($pNamedRange->getLocalOnly()) {
            $objWriter->writeAttribute('localSheetId',
                $pNamedRange->getScope()->getParent()
    ...
    private function _writeDefinedNameForAutofilter(
        ...
        if ( ! empty($autoFilterRange)) {
            ...
            $objWriter->writeRawData('\'' . str_replace("'", "''",
    ...
    private function _writeDefinedNameForPrintArea(
        ...
        if ($pSheet->getPageSetup()->isPrintAreaSet()) {
            ...
            foreach ($printArea as $printAreaRect) {
                $printAreaRect[0]
                    = PHPExcel_Cell::absoluteReference($printAreaRect[0]);
                $printAreaRect[1]
                    = PHPExcel_Cell::absoluteReference($printAreaRect[1]);
                $chunks[] = '\'' . str_replace("'", "''", $pSheet->getTitle())
                            . '\'!' . implode(':', $printAreaRect);
            }
