    ...
    implements PHPExcel_Writer_IWriter
{
    ...
    public function save($pFilename = null)
    {
        ...
        $this->_writerWorkbook
            = new PHPExcel_Writer_Excel5_Workbook($this->_phpExcel,
        ...
        for ($i = 0; $i < $countSheets; ++$i) {
            $this->_writerWorksheets[$i]
                = new PHPExcel_Writer_Excel5_Worksheet($this->_str_total,
                $this->_str_unique,
        ...
        for ($i = 0; $i < $countSheets; ++$i) {
            foreach (
                $this->_writerWorksheets[$i]->_phpSheet->getCellCollection() as
                $cellID
            ) {
                $cell
                    = $this->_writerWorksheets[$i]->_phpSheet->getCell($cellID);
                $cVal = $cell->getValue();
        ...
        $OLE
            = new PHPExcel_Shared_OLE_PPS_File(PHPExcel_Shared_OLE::Asc2Ucs($workbookStreamName));
        ...
        $this->_documentSummaryInformation
            = $this->_writeDocumentSummaryInformation();
        // initialize OLE Document Summary Information
        if (isset($this->_documentSummaryInformation)
            && ! empty($this->_documentSummaryInformation)
        ) {
            $OLE_DocumentSummaryInformation
                = new PHPExcel_Shared_OLE_PPS_File(PHPExcel_Shared_OLE::Asc2Ucs(chr(5)
        ...
        if (isset($this->_summaryInformation)
            && ! empty($this->_summaryInformation)
        ) {
    ...
    private function _buildWorksheetEschers()
    {
        ...
        foreach ($this->_phpExcel->getAllsheets() as $sheet) {
            ...
            $countShapes[$sheetIndex]
                = 0; // count number of shapes (minus group shape), in sheet
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                $spId = $reducedSpId
                        | ($sheet->getParent()->getIndex($sheet) + 1)
                          << 10;
                ...
                $twoAnchor = PHPExcel_Shared_Excel5::oneAnchor2twoAnchor($sheet,
            ...
            if ( ! empty($filterRange)) {
                ...
                while ($iInc <= $iNumColEnd) {
                    ...
                    $cDrawing = PHPExcel_Cell::stringFromColumnIndex($iInc - 1)
                                . $rangeBounds[0][1];
                    ...
                    $spContainer->setOPT(0x007F,
                        0x01040104); // Protection -> fLockAgainstGrouping
                    $spContainer->setOPT(0x00BF,
                        0x00080008); // Text -> fFitTextToShape
                    $spContainer->setOPT(0x01BF,
                        0x00010000); // Fill Style -> fNoFillHitTest
                    $spContainer->setOPT(0x01FF,
                        0x00080000); // Line Style -> fNoLineDrawDash
                    $spContainer->setOPT(0x03BF,
                        0x000A0000); // Group Shape -> fPrint

                    // set coordinates and offsets, client anchor
                    $endCoordinates
                        = PHPExcel_Cell::stringFromColumnIndex(PHPExcel_Cell::stringFromColumnIndex($iInc
                                                                                                    - 1));
    ...
    private function _buildWorkbookEscher()
    {
        ...
        // bstoreContainer
        $bstoreContainer
            = new PHPExcel_Shared_Escher_DggContainer_BstoreContainer();
        ...
        foreach ($this->_phpExcel->getAllsheets() as $sheet) {
            foreach ($sheet->getDrawingCollection() as $drawing) {
                if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
                    ...
                    list($imagesx, $imagesy, $imageFormat)
                    ...
                    switch ($imageFormat) {
                        ...
                        case 1: // GIF, not supported by BIFF8, we convert to PNG
                            $blipType
                                = PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
                            ob_start();
                        ...
                        case 2: // JPEG
                            $blipType
                                = PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
                    ...
                    $blip
                        = new PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip();
                    $blip->setData($blipData);

                    $BSE
                        = new PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE();
                    $BSE->setBlipType($blipType);
                    $BSE->setBlip($blip);

                    $bstoreContainer->addBSE($BSE);

                } elseif ($drawing instanceof
    ...
    private function _writeDocumentSummaryInformation()
    {
        ...
        $data .= pack('vvvvvvvv', 0xD502, 0xD5CD, 0x2E9C, 0x101B, 0x9793,
            0x0008, 0x2C2B, 0xAEF9);
        ...
        foreach ($dataSection as $dataProp) {
            // Summary
            $dataSection_Summary .= pack($dataProp['summary']['pack'],
                $dataProp['summary']['data']);
            // Offset
            $dataSection_Summary .= pack($dataProp['offset']['pack'],
                $dataSection_Content_Offset);
            // DataType
            $dataSection_Content .= pack($dataProp['type']['pack'],
                $dataProp['type']['data']);
            // Data
            if ($dataProp['type']['data'] == 0x02) { // 2 byte signed integer
                ...
                $dataProp['data']['length'] = $dataProp['data']['length'] + ((4
                                                                              - $dataProp['data']['length']
                                                                                % 4)
                                                                             == 4
                        ? 0 : (4 - $dataProp['data']['length'] % 4));
                $dataProp['data']['data'] = str_pad($dataProp['data']['data'],
    ...
    private function _writeSummaryInformation()
    {
        ...
        $data .= pack('vvvvvvvv', 0x85E0, 0xF29F, 0x4FF9, 0x1068, 0x91AB,
            0x0008, 0x272B, 0xD9B3);
        // offset: 44; size: 4; offset of the start
        $data .= pack('V', 0x30);
