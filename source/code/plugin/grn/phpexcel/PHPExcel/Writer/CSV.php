    ...
    implements PHPExcel_Writer_IWriter
{
    ...
    public function save($pFilename = null)
    {
        ...
        if ($this->_excelCompatibility) {
            ...
            $this->setLineEnding("\r\n");
            fwrite($fileHandle,
                'sep=' . $this->getDelimiter() . $this->_lineEnding);
        } elseif ($this->_useBOM) {
        ...
        for ($row = 1; $row <= $maxRow; ++$row) {
            ...
            $cellsArray = $sheet->rangeToArray('A' . $row . ':' . $maxCol
                                               . $row, '',
    ...
    private function _writeLine($pFileHandle = null, $pValues = null)
    {
        if (is_array($pValues)) {
            ...
            foreach ($pValues as $element) {
                // Escape enclosures
                $element = str_replace($this->_enclosure,
                    $this->_enclosure . $this->_enclosure, $element);
