...
class PHPExcel_Writer_Excel5_Parser
{
    ...
    function _convert($token)
    {
        if (preg_match("/\"([^\"]|\"\"){0,255}\"/", $token)) {
            ...
            // match external references like 'Sheet1'!A1 or 'Sheet1:Sheet2'!A1 or 'Sheet1'!$A$1 or 'Sheet1:Sheet2'!$A$1
        } elseif (preg_match("/^'" . self::REGEX_SHEET_TITLE_QUOTED . "(\:"
        ...
        } elseif (preg_match('/^(\$)?[A-Ia-i]?[A-Za-z](\$)?(\d+)\:(\$)?[A-Ia-i]?[A-Za-z](\$)?(\d+)$/',
            $token)
        ) {
            ...
            // match error codes
        } elseif (preg_match("/^#[A-Z0\/]{3,5}[!?]{1}$/", $token) or $token
                                                                     == '#N/A'
        ) {
...
//		$volatile = $this->_functions[$token][3];
    ...
    function _convertRange2d($range, $class = 0)
    {
        ...
        if (preg_match('/^(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)\:(\$)?([A-Ia-i]?[A-Za-z])(\$)?(\d+)$/',
            $range)
        ) {
...
//		$class = 0; // formulas like Sheet1!$A$1:$A$2 in list type data validation need this class (0x3B)
        ...
        if (preg_match("/^(\\$)?[A-Ia-i]?[A-Za-z](\\$)?(\d+)$/", $cell1)) {
            ...
            list($row1, $col1, $row2, $col2) = $this->_rangeToPackedRange($cell1
                                                                          . ':'
                                                                          . $cell2);
        }
...
//		}
    ...
    function _rangeToPackedRange($range)
    {
        ...
        $col1 = 0;
        $col2
            = 65535; // FIXME: maximum possible value for Excel 5 (change this!!!)
        ...
        switch ($token) {
            ...
            default:
                ...
                elseif (is_numeric($token) and
                        ( ! is_numeric($token . $this->_lookahead)
                          or ($this->_lookahead == '')) and
                        ($this->_lookahead != '!') and ($this->_lookahead
                                                        != ':')
                ) {
                    return $token;
                ...
                elseif (preg_match("/\"([^\"]|\"\"){0,255}\"/", $token)
                        and $this->_lookahead != '"' and (substr_count($token,
                            '"') % 2 == 0)
                ) {
                    return $token;
                } // If it's an error code
                elseif (preg_match("/^#[A-Z0\/]{3,5}[!?]{1}$/", $token)
                        or $token == '#N/A'
                ) {
                    return $token;
                } // if it's a function call
                elseif (preg_match("/^[A-Z0-9\xc0-\xdc\.]+$/i", $token)
                        and ($this->_lookahead == "(")
                ) {
                    return $token;
                }
    ...
    function _expression()
    {
        ...
        if (preg_match("/\"([^\"]|\"\"){0,255}\"/", $this->_current_token)) {
            ...
            return $result;
            // If it's an error code
        } elseif (preg_match("/^#[A-Z0\/]{3,5}[!?]{1}$/", $this->_current_token)
                  or $this->_current_token == '#N/A'
        ) {
    ...
    function _fact()
    {
        ...
        // if it's a reference
        if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/',
            $this->_current_token)
        ) {
        ...
        elseif (is_numeric($this->_current_token)) {
            if ($this->_lookahead == '%') {
                $result = $this->_createTree('ptgPercent',
                    $this->_current_token, '');
        ...
        throw new PHPExcel_Writer_Exception("Syntax error: "
                                            . $this->_current_token .
    ...
    function toReversePolish($tree = [])
    {
        ...
        if (preg_match("/^[A-Z0-9\xc0-\xdc\.]+$/", $tree['value']) and
            ...
            return $left_tree . $this->_convertFunction($tree['value'],
                    $tree['right']);
        } else {
