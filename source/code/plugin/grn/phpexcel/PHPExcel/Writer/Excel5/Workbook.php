...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    public function __construct(
        ...
        for ($i = 0; $i < $countSheets; ++$i) {
            ...
            $this->_parser->setExtSheet($phpSheet->getTitle(),
                $i);  // Register worksheet name with parser

            $supbook_index = 0x00;
            $ref = pack('vvv', $supbook_index, $i, $i);
            $this->_parser->_references[]
                = $ref;  // Register reference with parser
    ...
    public function addXfWriter($style, $isStyleXf = false)
    {
        ...
        $xfWriter->setFgColor($this->_addColor($style->getFill()
                                                     ->getStartColor()
                                                     ->getRGB()));
        $xfWriter->setBgColor($this->_addColor($style->getFill()->getEndColor()
                                                     ->getRGB()));
        $xfWriter->setBottomColor($this->_addColor($style->getBorders()
                                                         ->getBottom()
                                                         ->getColor()
                                                         ->getRGB()));
        $xfWriter->setTopColor($this->_addColor($style->getBorders()->getTop()
                                                      ->getColor()->getRGB()));
        $xfWriter->setRightColor($this->_addColor($style->getBorders()
                                                        ->getRight()->getColor()
                                                        ->getRGB()));
        $xfWriter->setLeftColor($this->_addColor($style->getBorders()->getLeft()
                                                       ->getColor()->getRGB()));
        ...
        if ($style->getNumberFormat()->getBuiltInFormatCode() === false) {
            ...
            if (isset($this->_addedNumberFormats[$numberFormatHashCode])) {
                $numberFormatIndex
                    = $this->_addedNumberFormats[$numberFormatHashCode];
            } else {
                $numberFormatIndex = 164 + count($this->_numberFormats);
                $this->_numberFormats[$numberFormatIndex]
    ...
    public function writeWorkbook($pWorksheetSizes = null)
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            $this->_writeBoundsheet($this->_phpExcel->getSheet($i),
                $this->_worksheetOffsets[$i]);
        }
    ...
    private function _writeAllNumFormats()
    {
        foreach ($this->_numberFormats as $numberFormatIndex => $numberFormat) {
            $this->_writeNumFormat($numberFormat->getFormatCode(),
                $numberFormatIndex);
        }
    }
    ...
    private function _writeNames()
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                // Print area
                $printArea
                    = PHPExcel_Cell::splitRange($sheetSetup->getPrintArea());
                $printArea = $printArea[0];
                $printArea[0]
                    = PHPExcel_Cell::coordinateFromString($printArea[0]);
                $printArea[1]
                    = PHPExcel_Cell::coordinateFromString($printArea[1]);

                $print_rowmin = $printArea[0][1] - 1;
                $print_rowmax = $printArea[1][1] - 1;
                $print_colmin
                    = PHPExcel_Cell::columnIndexFromString($printArea[0][0])
                      - 1;
                $print_colmax
                    = PHPExcel_Cell::columnIndexFromString($printArea[1][0])
                      - 1;
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isColumnsToRepeatAtLeftSet()
                ...
                if ($sheetSetup->isColumnsToRepeatAtLeftSet()) {
                    $repeat = $sheetSetup->getColumnsToRepeatAtLeft();
                    $colmin = PHPExcel_Cell::columnIndexFromString($repeat[0])
                              - 1;
                    $colmax = PHPExcel_Cell::columnIndexFromString($repeat[1])
                              - 1;
                } else {
                    $colmin = 0;
                    $colmax = 255;
    ...
    private function _writeAllDefinedNamesBiff8()
    {
        ...
        if (count($this->_phpExcel->getNamedRanges()) > 0) {
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                for ($i = 0; $i < count($range); $i++) {
                    ...
                    if (isset($range[$i][1])) {
                        $range[$i][1]
                            = PHPExcel_Cell::absoluteCoordinate($range[$i][1]);
                    }
                }
                $range
                    = PHPExcel_Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
                ...
                try {
                    ...
                    if (isset($formulaData{0}) and ($formulaData{0} == "\x7A"
                                                    or $formulaData{0}
                                                       == "\x5A")
                    ) {
                    ...
                    if ($namedRange->getLocalOnly()) {
                        // local scope
                        $scope
                            = $this->_phpExcel->getIndex($namedRange->getScope())
                              + 1;
                    } else {
                        // global scope
                        $scope = 0;
                    }
                    $chunk .= $this->writeData($this->_writeDefinedNameBiff8($namedRange->getName(),
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isColumnsToRepeatAtLeftSet()
                ...
                $formulaData .= pack('Cvvvvv', 0x3B, $i, 0, 65535, $colmin,
                    $colmax); // tArea3d
                $formulaData .= pack('Cvvvvv', 0x3B, $i, $rowmin, $rowmax, 0,
                    255); // tArea3d
                $formulaData .= pack('C', 0x10); // tList

                // store the DEFINEDNAME record
                $chunk .= $this->writeData($this->_writeDefinedNameBiff8(pack('C',
                ...
                $formulaData = pack('Cvvvvv', 0x3B, $i, $rowmin, $rowmax,
                    $colmin, $colmax);
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                ...
                for ($j = 0; $j < $countPrintArea; ++$j) {
                    ...
                    $printAreaRect[0]
                        = PHPExcel_Cell::coordinateFromString($printAreaRect[0]);
                    $printAreaRect[1]
                        = PHPExcel_Cell::coordinateFromString($printAreaRect[1]);

                    $print_rowmin = $printAreaRect[0][1] - 1;
                    $print_rowmax = $printAreaRect[1][1] - 1;
                    $print_colmin
                        = PHPExcel_Cell::columnIndexFromString($printAreaRect[0][0])
                          - 1;
                    $print_colmax
                        = PHPExcel_Cell::columnIndexFromString($printAreaRect[1][0])
                          - 1;

                    // construct formula data manually because parser does not recognize absolute 3d cell references
                    $formulaData .= pack('Cvvvvv', 0x3B, $i, $print_rowmin,
                        $print_rowmax, $print_colmin, $print_colmax);

                    if ($j > 0) {
                        $formulaData .= pack('C',
                            0x10); // list operator token ','
                    }
                }
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ( ! empty($autoFilterRange)) {
                ...
                $chunk .= $this->writeData($this->_writeShortNameBiff8($name,
                    $i + 1, $rangeBounds, true));
            }
        }
     ...
     */
    private function _writeDefinedNameBiff8(
        $name,
        $formulaData,
        $sheetIndex = 0,
        $isBuiltIn = false
        ...
        $name = substr(PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($name),
            2);
        ...
        // combine the parts
        $data = pack('vCCvvvCCCC', $options, 0, $nlen, $sz, 0, $sheetIndex, 0,
                0, 0, 0)
                . $name . $formulaData;
        $length = strlen($data);
     ...
     * @return    string    Complete binary record data
     * */
    private function _writeShortNameBiff8(
        $name,
        $sheetIndex = 0,
        $rangeBounds,
        $isHidden = false
        ...
        // combine the parts
        $data = pack('vCCvvvCCCCC', $options, 0, 1, $sz, 0, $sheetIndex, 0, 0,
                0, 0, 0)
                . $name . $extra;
        $length = strlen($data);
    ...
    private function _writeWindow1()
    {
        ...
        $itabCur
            = $this->_phpExcel->getActiveSheetIndex();    // Active worksheet
    ...
    private function _writeExternsheetBiff8()
    {
        ...
        $length = 2 + 6
                      * $total_references;  // Number of bytes to follow

        $supbook_index
            = 0;           // FIXME: only using internal SUPBOOK record
        $header = pack("vv", $record, $length);
    ...
    private function _writeNumFormat($format, $ifmt)
    {
        ...
        $numberFormatString
            = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($format);
        $length = 2
                  + strlen($numberFormatString);      // Number of bytes to follow
     ...
     */
    private function _writeNameShort(
        $index,
        $type,
        $rowmin,
        $rowmax,
     ...
     * @param integer $colmax End column
     */
    private function _writeNameLong(
        $index,
        $type,
        $rowmin,
        $rowmax,
