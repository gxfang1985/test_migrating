...
//		if ($this->_firstColumnIndex > 255) $this->_firstColumnIndex = 255;
    ...
    function close()
    {
        ...
        if (($defaultWidth = $_phpSheet->getDefaultColumnDimension()
            ...
            $defaultWidth
                = PHPExcel_Shared_Font::getDefaultColumnWidthByFont($_phpSheet->getParent()
                                                                              ->getDefaultStyle()
                                                                              ->getFont());
        }
        ...
        for ($i = 0; $i <= $maxCol; ++$i) {
            ...
            if (isset($columnDimensions[$columnLetter])) {
                ...
                $xfIndex = $columnDimension->getXfIndex()
                           + 15; // there are 15 cell style Xfs
            }
        ...
        foreach ($_phpSheet->getRowDimensions() as $rowDimension) {
            ...
            $this->_writeRow($rowDimension->getRowIndex() - 1,
                $rowDimension->getRowHeight(), $xfIndex,
...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
            ...
            if ($cVal instanceof PHPExcel_RichText) {
                ...
                $arrcRun = [];
                $str_len
                    = PHPExcel_Shared_String::CountCharacters($cVal->getPlainText(),
                    'UTF-8');
                $str_pos = 0;
                ...
                foreach ($elements as $element) {
                    ...
                    // Position FROM
                    $str_pos += PHPExcel_Shared_String::CountCharacters($element->getText(),
                        'UTF-8');
                }
                $this->_writeRichTextString($row, $column,
                ...
                switch ($cell->getDatatype()) {
                    ...
                    case PHPExcel_Cell_DataType::TYPE_FORMULA:
                        ...
                        $this->_writeFormula($row, $column, $cVal, $xfIndex,
                            $calculatedValue);
                        break;
                    ...
                    case PHPExcel_Cell_DataType::TYPE_ERROR:
                        $this->_writeBoolErr($row, $column,
                            self::_mapErrorCode($cVal), 1, $xfIndex);
                        break;
        ...
        foreach (
            ...
            list($column, $row)
                = PHPExcel_Cell::coordinateFromString($coordinate);
            ...
            $this->_writeUrl($row - 1,
                PHPExcel_Cell::columnIndexFromString($column) - 1, $url);
        }
        ...
        if ( ! empty($arrConditionalStyles)) {
            ...
            foreach (
                $arrConditionalStyles as $cellCoordinate => $conditionalStyles
            ) {
    ...
    private function _writeBIFF8CellRangeAddressFixed($range = 'A1')
    {
        ...
        $firstCellCoordinates
            = PHPExcel_Cell::coordinateFromString($firstCell); // e.g. array(0, 1)
        $lastCellCoordinates
            = PHPExcel_Cell::coordinateFromString($lastCell);  // e.g. array(1, 6)
    ...
    private function _writeRichTextString($row, $col, $str, $xfIndex, $arrcRun)
    {
        ...
        $str = PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($str,
            $arrcRun);
     ...
     * @return integer
     */
    private function _writeFormula(
        $row,
        $col,
        $formula,
        $xfIndex,
        ...
        if (isset($calculatedValue)) {
            ...
            if (is_bool($calculatedValue)) {
                // Boolean value
                $num = pack('CCCvCv', 0x01, 0x00, (int)$calculatedValue, 0x00,
                    0x00, 0xFFFF);
                ...
                if (array_key_exists($calculatedValue,
                    ...
                    $num = pack('CCCvCv', 0x02, 0x00,
                        self::_mapErrorCode($calculatedValue), 0x00, 0x00,
                        0xFFFF);
                    ...
                    $num = pack('CCCvCv', 0x00, 0x00, 0x00, 0x00, 0x00,
                        0xFFFF);
                }
            } else {
    ...
    function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        $url = PHPExcel_Shared_String::ConvertEncoding($url, 'UTF-16LE',
            'UTF-8');
    ...
    private function _writeWindow2()
    {
        ...
        $zoom_factor_page_break = ($fPageBreakPreview
            ? $this->_phpSheet->getSheetView()->getZoomScale() : 0x0000);
        $zoom_factor_normal = $this->_phpSheet->getSheetView()
                                              ->getZoomScaleNormal();

        $data .= pack("vvvvV", $rgbHdr, 0x0000, $zoom_factor_page_break,
            $zoom_factor_normal, 0x00000000);
    ...
    private function _writeSelection()
    {
        ...
        $selectedCells
            = PHPExcel_Cell::splitRange($this->_phpSheet->getSelectedCells());
        ...
        $colFirst = PHPExcel_Cell::columnIndexFromString($colFirst)
                    - 1; // base 0 column index
        --$rwFirst; // base 0 row index

        list($colLast, $rwLast) = PHPExcel_Cell::coordinateFromString($last);
        $colLast = PHPExcel_Cell::columnIndexFromString($colLast)
                   - 1; // base 0 column index
    ...
    private function _writeMergedCells()
    {
        ...
        foreach ($mergeCells as $mergeCell) {
            ...
            list($firstColumn, $firstRow)
                = PHPExcel_Cell::coordinateFromString($first);
            list($lastColumn, $lastRow)
                = PHPExcel_Cell::coordinateFromString($last);
    ...
    private function _writeSheetProtection()
    {
        ...
        $options = (int) ! $this->_phpSheet->getProtection()->getObjects()
                   | (int) ! $this->_phpSheet->getProtection()->getScenarios()
                     << 1
                   | (int) ! $this->_phpSheet->getProtection()->getFormatCells()
                     << 2
                   | (int) ! $this->_phpSheet->getProtection()
                                             ->getFormatColumns() << 3
                   | (int) ! $this->_phpSheet->getProtection()->getFormatRows()
                     << 4
                   | (int) ! $this->_phpSheet->getProtection()
                                             ->getInsertColumns() << 5
                   | (int) ! $this->_phpSheet->getProtection()->getInsertRows()
                     << 6
                   | (int) ! $this->_phpSheet->getProtection()
                                             ->getInsertHyperlinks() << 7
                   | (int) ! $this->_phpSheet->getProtection()
                                             ->getDeleteColumns() << 8
                   | (int) ! $this->_phpSheet->getProtection()->getDeleteRows()
                     << 9
                   | (int) ! $this->_phpSheet->getProtection()
                                             ->getSelectLockedCells() << 10
                   | (int) ! $this->_phpSheet->getProtection()->getSort() << 11
                   | (int) ! $this->_phpSheet->getProtection()->getAutoFilter()
                     << 12
                   | (int) ! $this->_phpSheet->getProtection()->getPivotTables()
                     << 13
                   | (int) ! $this->_phpSheet->getProtection()
    ...
    private function _writeRangeProtection()
    {
        foreach ($this->_phpSheet->getProtectedCells() as $range => $password) {
            ...
            $recordData .= PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong('p'
                                                                          . md5($recordData));
    ...
    private function _writePanes()
    {
        ...
        if ($freezePane = $this->_phpSheet->getFreezePane()) {
            list($column, $row)
                = PHPExcel_Cell::coordinateFromString($freezePane);
            $panes[0] = $row - 1;
    ...
    private function _writeSetup()
    {
        ...
        $iScale = $this->_phpSheet->getPageSetup()->getScale() ?
            $this->_phpSheet->getPageSetup()->getScale()
    ...
    private function _writeHeader()
    {
		...
		*/
        ...
        $recordData
            = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($this->_phpSheet->getHeaderFooter()
                                                                             ->getOddHeader());
        $length = strlen($recordData);
    ...
    private function _writeHcenter()
    {
        ...
        $fHCenter = $this->_phpSheet->getPageSetup()->getHorizontalCentered()
            ? 1 : 0;     // Horizontal centering
    ...
    private function _writeVcenter()
    {
        ...
        $fVCenter = $this->_phpSheet->getPageSetup()->getVerticalCentered() ? 1
    ...
    private function _writePrintGridlines()
    {
        ...
        $fPrintGrid = $this->_phpSheet->getPrintGridlines() ? 1
            : 0;    // Boolean flag
    ...
    private function _writeAutoFilterInfo()
    {
        ...
        $rangeBounds
            = PHPExcel_Cell::rangeBoundaries($this->_phpSheet->getAutoFilter()
                                                             ->getRange());
    ...
    private function _writeGuts()
    {
        ...
        $header = pack("vv", $record, $length);
        $data = pack("vvvv", $dxRwGut, $dxColGut, $maxRowOutlineLevel,
            $col_level);
    ...
    private function _writePassword()
    {
        ...
        if ( ! $this->_phpSheet->getProtection()->getSheet()
             || ! $this->_phpSheet->getProtection()->getPassword()
        ) {
            return;
        }
    ...
    function insertBitmap(
        ...
        $bitmap_array = (is_resource($bitmap) ? $this->_processBitmapGd($bitmap)
            : $this->_processBitmap($bitmap));
        list($width, $height, $size, $data)
    ...
    function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        if ($y1 >= PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet,
                $row_start + 1)
        ) {
            $y1 = 0;
        }
        ...
        while ($height >= PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet,
                $row_end + 1)) {
            $height -= PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet,
                $row_end + 1);
            ++$row_end;
        }
        ...
        //
        if (PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet,
                PHPExcel_Cell::stringFromColumnIndex($col_start)) == 0
        ) {
            return;
        }
        if (PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet,
                PHPExcel_Cell::stringFromColumnIndex($col_end)) == 0
        ) {
            return;
        }
        if (PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)
            == 0
        ) {
            return;
        }
        if (PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)
            == 0
        ) {
            return;
        }
        ...
        $y1 = $y1 / PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet,
                $row_start + 1) * 256;
     ...
     */
    private function _writeObjPicture(
        $colL,
        $dxL,
        $rwT,
        $dyT,
    ...
    function _processBitmapGd($image)
    {
        ...
        for ($j = $height; $j--;) {
            for ($i = 0; $i < $width; ++$i) {
                $color = imagecolorsforindex($image,
                    imagecolorat($image, $i, $j));
                foreach (["red", "green", "blue"] as $key) {
                    $color[$key] = $color[$key] + round((255 - $color[$key])
                                                        * $color["alpha"]
                                                        / 127);
                }
                $data .= chr($color["blue"]) . chr($color["green"])
                         . chr($color["red"]);
            }
            if (3 * $width % 4) {
    ...
    private function _writeZoom()
    {
        ...
        $header = pack("vv", $record, $length);
        $data = pack("vv", $this->_phpSheet->getSheetView()->getZoomScale(),
            100);
        $this->_append($header . $data);
    }
    ...
    private function _writeMsoDrawing()
    {
        ...
        if (isset($this->_escher)) {
            ...
            $spOffsets[0] = 0;
            $nm = count($spOffsets)
                  - 1; // number of shapes excluding first shape
            for ($i = 1; $i <= $nm; ++$i) {
                // MSODRAWING record
    ...
    private function _writeDataValidity()
    {
        ...
        $dataValidationCollection
            = $this->_phpSheet->getDataValidationCollection();
        ...
        if ( ! empty($dataValidationCollection)) {
            ...
            $grbit
                = 0x0000;       // Prompt box at cell, no cached validity data at DV records
            $horPos
                = 0x00000000;  // Horizontal position of prompt box, if fixed position
            $verPos
                = 0x00000000;  // Vertical position of prompt box, if fixed position
            $objId
                = 0xFFFFFFFF;  // Object identifier of drop down arrow object, or -1 if not visible
            ...
            foreach (
                $dataValidationCollection as $cellCoordinate => $dataValidation
            ) {
                // initialize record data
                $data = '';
    ...
    private function _writePageLayoutView()
    {
        ...
        $header = pack("vv", $record, $length);
        $data = pack("vvVVvv", $rt, $grbitFrt, 0x00000000, 0x00000000,
            $wScalvePLV, $grbit);
        $this->_append($header . $data);
    }
    ...
    private function _writeCFRule(PHPExcel_Style_Conditional $conditional)
    {
        ...
        // $flags : Option flags
        // Alignment
        $bAlignHz = ($conditional->getStyle()->getAlignment()
                                 ->getHorizontal() == null ? 1 : 0);
        $bAlignVt = ($conditional->getStyle()->getAlignment()->getVertical()
                     == null ? 1 : 0);
        $bAlignWrapTx = ($conditional->getStyle()->getAlignment()->getWrapText()
                         == false ? 1 : 0);
        $bTxRotation = ($conditional->getStyle()->getAlignment()
                                    ->getTextRotation() == null ? 1 : 0);
        $bIndent = ($conditional->getStyle()->getAlignment()->getIndent()
                    == 0 ? 1 : 0);
        $bShrinkToFit = ($conditional->getStyle()->getAlignment()
        ...
        // Border
        $bBorderLeft = ($conditional->getStyle()->getBorders()->getLeft()
                                    ->getColor()->getARGB()
                        == PHPExcel_Style_Color::COLOR_BLACK
                        && $conditional->getStyle()->getBorders()->getLeft()
                                       ->getBorderStyle()
                           == PHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderRight = ($conditional->getStyle()->getBorders()->getRight()
                                     ->getColor()->getARGB()
                         == PHPExcel_Style_Color::COLOR_BLACK
                         && $conditional->getStyle()->getBorders()->getRight()
                                        ->getBorderStyle()
                            == PHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderTop = ($conditional->getStyle()->getBorders()->getTop()
                                   ->getColor()->getARGB()
                       == PHPExcel_Style_Color::COLOR_BLACK
                       && $conditional->getStyle()->getBorders()->getTop()
                                      ->getBorderStyle()
                          == PHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
        $bBorderBottom = ($conditional->getStyle()->getBorders()->getBottom()
                                      ->getColor()->getARGB()
                          == PHPExcel_Style_Color::COLOR_BLACK
                          && $conditional->getStyle()->getBorders()->getBottom()
                                         ->getBorderStyle()
                             == PHPExcel_Style_Border::BORDER_NONE ? 1 : 0);
        ...
        $bFillStyle = ($conditional->getStyle()->getFill()->getFillType()
                       == null ? 0 : 1);
        $bFillColor = ($conditional->getStyle()->getFill()->getStartColor()
                                   ->getARGB() == null ? 0 : 1);
        $bFillColorBg = ($conditional->getStyle()->getFill()->getEndColor()
                                     ->getARGB() == null ? 0 : 1);
        ...
        if ($bFormatFont == 1) {
            // Font Name
            if ($conditional->getStyle()->getFont()->getName() == null) {
                $dataBlockFont = pack('VVVVVVVV', 0x00000000, 0x00000000,
                ...
                $dataBlockFont
                    = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($conditional->getStyle()
                                                                                 ->getFont()
                                                                                 ->getName());
            }
            // Font Size
            if ($conditional->getStyle()->getFont()->getSize() == null) {
                ...
                $dataBlockFont .= pack('V',
                    20 * $conditional->getStyle()->getFont()->getSize());
            }
            // Font Options
            ...
            if ($conditional->getStyle()->getFont()->getSubScript() == true) {
                ...
                $fontEscapement = 0;
            } elseif ($conditional->getStyle()->getFont()->getSuperScript()
                      == true
            ) {
        ...
        if ($bFormatAlign == 1) {
            ...
            // Text rotation angle
            $blockRotation = $conditional->getStyle()->getAlignment()
                                         ->getTextRotation();

            // Indentation
            $blockIndent = $conditional->getStyle()->getAlignment()
                                       ->getIndent();
            ...
            $dataBlockAlign = pack('CCvvv', $blockAlign, $blockRotation,
                $blockIndent, $blockIndentRelative, 0x0000);
        }
        if ($bFormatBorder == 1) {
            $blockLineStyle = 0;
            switch ($conditional->getStyle()->getBorders()->getLeft()
                                ->getBorderStyle()) {
            ...
            switch ($conditional->getStyle()->getBorders()->getRight()
                                ->getBorderStyle()) {
                case PHPExcel_Style_Border::BORDER_NONE              :
            ...
            switch ($conditional->getStyle()->getBorders()->getTop()
                                ->getBorderStyle()) {
                case PHPExcel_Style_Border::BORDER_NONE              :
            ...
            switch ($conditional->getStyle()->getBorders()->getBottom()
                                ->getBorderStyle()) {
                case PHPExcel_Style_Border::BORDER_NONE              :
            ...
            switch ($conditional->getStyle()->getBorders()->getDiagonal()
                                ->getBorderStyle()) {
                case PHPExcel_Style_Border::BORDER_NONE              :
                    $blockColor |= 0x00 << 21;
                    break;
        ...
        if ($bFormatFill == 1) {
            ...
            // Color
            switch ($conditional->getStyle()->getFill()->getStartColor()
                                ->getRGB()) {
                case '000000':
                    $colorIdxBg = 0x08;
                    break;
            ...
            // Fg Color
            switch ($conditional->getStyle()->getFill()->getEndColor()
                                ->getRGB()) {
                case '000000':
                    $colorIdxFg = 0x08;
                    break;
        ...
        $data = pack('CCvvVv', $type, $operatorType, $szValue1, $szValue2,
            $flags, 0x0000);
    ...
    private function _writeCFHeader()
    {
        ...
        $arrConditional = [];
        foreach (
            $this->_phpSheet->getConditionalStylesCollection() as
            ...
            foreach ($conditionalStyles as $conditional) {
                if ($conditional->getConditionType()
                    ...
                    // Cells
                    $arrCoord
                        = PHPExcel_Cell::coordinateFromString($cellCoordinate);
                    if ( ! is_numeric($arrCoord[0])) {
                        $arrCoord[0]
                            = PHPExcel_Cell::columnIndexFromString($arrCoord[0]);
                    }
                    if (is_null($numColumnMin)
                        || ($numColumnMin > $arrCoord[0])
                    ) {
                        $numColumnMin = $arrCoord[0];
                    }
                    if (is_null($numColumnMax)
