    ...
    implements PHPExcel_Writer_IWriter
{
    ...
    public function save($pFilename = null)
    {
        ...
        PHPExcel_Calculation::getInstance($this->_phpExcel)->getDebugLog()
    ...
    public function generateSheetData()
    {
        ...
        foreach ($sheets as $sheet) {
            ...
            $dimension[0]
                = PHPExcel_Cell::coordinateFromString($dimension[0]);
            $dimension[0][0]
                = PHPExcel_Cell::columnIndexFromString($dimension[0][0])
                  - 1;
            $dimension[1]
                = PHPExcel_Cell::coordinateFromString($dimension[1]);
            $dimension[1][0]
                = PHPExcel_Cell::columnIndexFromString($dimension[1][0])
                  - 1;
            ...
            if ($this->_isPdf) {
                if (is_null($this->_sheetIndex)
                    && $sheetId + 1 < $this->_phpExcel->getSheetCount()
                ) {
    ...
    public function generateNavigation()
    {
        ...
        if (count($sheets) > 1) {
            ...
            foreach ($sheets as $sheet) {
                $html .= '  <li class="sheet' . $sheetId . '"><a href="#sheet'
                         . $sheetId . '">' . $sheet->getTitle() . '</a></li>'
                         . PHP_EOL;
                ++$sheetId;
            }
    ...
    private function _extendRowsForChartsAndImages(
        ...
        if ($this->_includeCharts) {
            foreach ($pSheet->getChartCollection() as $chart) {
                if ($chart instanceof PHPExcel_Chart) {
                    ...
                    $chartTL
                        = PHPExcel_Cell::coordinateFromString($chartCoordinates['cell']);
                    $chartCol
                        = PHPExcel_Cell::columnIndexFromString($chartTL[0]);
                    if ($chartTL[1] > $rowMax) {
                        $rowMax = $chartTL[1];
                        if ($chartCol
        ...
        foreach ($pSheet->getDrawingCollection() as $drawing) {
            if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
                $imageTL
                    = PHPExcel_Cell::coordinateFromString($drawing->getCoordinates());
                ...
                if ($drawing->getCoordinates() == $coordinates) {
                    ...
                    if (( ! $this->_embedImages) || ($this->_isPdf)) {
                        ...
                        if ($fp = fopen($filename, "rb", 0)) {
                            ...
                            $imageData = 'data:' . $imageDetails['mime']
                                         . ';base64,' . $base64;
                        } else {
                            $imageData = $filename;
                        }
        ...
        foreach ($pSheet->getChartCollection() as $chart) {
            if ($chart instanceof PHPExcel_Chart) {
                ...
                if ($chartCoordinates['cell'] == $coordinates) {
                    $chartFileName = PHPExcel_Shared_File::sys_get_temp_dir()
                                     . '/' . uniqid() . '.png';
                    ...
                    if ($fp = fopen($chartFileName, "rb", 0)) {
                        ...
                        $html .= '<div style="position: relative;">';
                        $html .= '<img style="position: absolute; z-index: 1; left: '
                                 . $chartCoordinates['xOffset'] . 'px; top: '
                                 . $chartCoordinates['yOffset'] . 'px; width: '
                                 . $imageDetails[0] . 'px; height: '
    ...
    public function generateStyles($generateSurroundingHTML = true)
    {
        ...
        foreach ($css as $styleName => $styleDefinition) {
            if ($styleName != 'html') {
                $html .= '	  ' . $styleName . ' { '
                         . $this->_assembleCSS($styleDefinition) . ' }'
                         . PHP_EOL;
            }
        }
    ...
    public function buildCSS($generateSurroundingHTML = true)
    {
        ...
        foreach ($sheets as $sheet) {
            ...
            while ($column++ < $highestColumnIndex) {
                $this->_columnWidths[$sheetIndex][$column]
                    = 42; // approximation
                $css['table.sheet' . $sheetIndex . ' col.col'
                     . $column]['width']
                    = '42pt';
            }
            ...
            foreach ($sheet->getColumnDimensions() as $columnDimension) {
                if (($width
                    ...
                    $column
                        = PHPExcel_Cell::columnIndexFromString($columnDimension->getColumnIndex())
                          - 1;
            ...
            if ($rowDimension->getRowHeight() == -1) {
                $pt_height
                    = PHPExcel_Shared_Font::getDefaultRowHeightByFont($this->_phpExcel->getDefaultStyle()
                                                                                      ->getFont());
            } else {
                $pt_height = $rowDimension->getRowHeight();
            }
            $css['table.sheet' . $sheetIndex . ' tr']['height'] = $pt_height
                                                                  . 'pt';
            if ($rowDimension->getVisible() === false) {
                $css['table.sheet' . $sheetIndex . ' tr']['display'] = 'none';
                $css['table.sheet' . $sheetIndex . ' tr']['visibility']
                    = 'hidden';
            }
            ...
            foreach ($sheet->getRowDimensions() as $rowDimension) {
                ...
                $css['table.sheet' . $sheetIndex . ' tr.row' . $row]['height']
                    = $pt_height . 'pt';
                if ($rowDimension->getVisible() === false) {
                    $css['table.sheet' . $sheetIndex . ' tr.row'
                         . $row]['display']
                        = 'none';
                    $css['table.sheet' . $sheetIndex . ' tr.row'
                         . $row]['visibility']
    ...
    private function _createCSSStyleAlignment(PHPExcel_Style_Alignment $pStyle)
    {
        ...
        if ($textAlign = $this->_mapHAlign($pStyle->getHorizontal())) {
            ...
            if (in_array($textAlign, ['left', 'right'])) {
                $css['padding-' . $textAlign]
                    = (string)((int)$pStyle->getIndent() * 9) . 'px';
            }
        }
...
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
    ...
    private function _generateTableHeader($pSheet)
    {
        ...
        if ( ! $this->_useInlineCss) {
            ...
            if ($this->_isPdf && $pSheet->getShowGridlines()) {
                $html .= '	<table border="1" cellpadding="1" id="sheet'
                         . $sheetIndex . '" cellspacing="1" style="' . $style
                         . '">' . PHP_EOL;
            } else {
                $html .= '	<table border="0" cellpadding="1" id="sheet'
                         . $sheetIndex . '" cellspacing="0" style="' . $style
                         . '">' . PHP_EOL;
            }
        }
        ...
        // Write <col> elements
        $highestColumnIndex
            = PHPExcel_Cell::columnIndexFromString($pSheet->getHighestColumn())
              - 1;
        $i = -1;
        while ($i++ < $highestColumnIndex) {
            if ( ! $this->_isPdf) {
                if ( ! $this->_useInlineCss) {
                    ...
                    $style = isset($this->_cssStyles['table.sheet' . $sheetIndex
                                                     . ' col.col' . $i]) ?
                        $this->_assembleCSS($this->_cssStyles['table.sheet'
                                                              . $sheetIndex
                                                              . ' col.col'
                                                              . $i]) : '';
    ...
    private function _generateRow(
        ...
        if (is_array($pValues)) {
            ...
            foreach ($pValues as $cellAddress) {
                ...
                $coordinate = PHPExcel_Cell::stringFromColumnIndex($colNum)
                              . ($pRow + 1);
                if ( ! $this->_useInlineCss) {
                    $cssClass = '';
                    ...
                    $cssClass = [];
                    if ($cellType == 'th') {
                        if (isset($this->_cssStyles['table.sheet' . $sheetIndex
                                                    . ' th.column' . $colNum])
                        ) {
                            $this->_cssStyles['table.sheet' . $sheetIndex
                                              . ' th.column' . $colNum];
                        }
                    } else {
                        if (isset($this->_cssStyles['table.sheet' . $sheetIndex
                ...
                if ($cell instanceof PHPExcel_Cell) {
                    ...
                    if ( ! $this->_useInlineCss) {
                        ...
                        if ($cellType == 'th') {
                            if (isset($this->_cssStyles['th.style'
                                                        . $cell->getXfIndex()])
                            ) {
                                $cssClass = array_merge($cssClass,
                                    $this->_cssStyles['th.style'
                                                      . $cell->getXfIndex()]);
                            }
                        } else {
                            if (isset($this->_cssStyles['td.style'
                                                        . $cell->getXfIndex()])
                            ) {
                                $cssClass = array_merge($cssClass,
                                    $this->_cssStyles['td.style'
                                                      . $cell->getXfIndex()]);
                            }
                        }
                        ...
                        if ($sharedStyle->getAlignment()->getHorizontal()
                            ...
                            && isset($this->_cssStyles['.'
                                                       . $cell->getDataType()]['text-align'])
                        ) {
                            $cssClass['text-align'] = $this->_cssStyles['.'
                                                                        . $cell->getDataType()]['text-align'];
                        }
                    }
                }
                ...
                if (isset($this->_isBaseCell[$pSheet->getParent()
                    ...
                    $endCellCoord = PHPExcel_Cell::stringFromColumnIndex($colNum
                                                                         + $colSpan
                                                                         - 1)
                                    . ($pRow + $rowSpan);
                    if ( ! $this->_useInlineCss) {
                ...
                if ($writeCell) {
                    ...
                    if ( ! $this->_useInlineCss) {
                        ...
                        if (isset($this->_cssStyles['table.sheet' . $sheetIndex
                            ...
                            $height = $this->_cssStyles['table.sheet'
                                                        . $sheetIndex
                                                        . ' tr.row'
                                                        . $pRow]['height'];
        ...
        foreach ($sheetIndexes as $sheetIndex) {
            ...
            foreach ($candidateSpannedRow as $rowIndex) {
                if (isset($this->_isSpannedCell[$sheetIndex][$rowIndex])) {
                    if (count($this->_isSpannedCell[$sheetIndex][$rowIndex])
                        == $countColumns
                    ) {
                        $this->_isSpannedRow[$sheetIndex][$rowIndex]
                            = $rowIndex;
                    };
                }
            }
            ...
            if (isset($this->_isSpannedRow[$sheetIndex])) {
                foreach ($this->_isSpannedRow[$sheetIndex] as $rowIndex) {
                    ...
                    while ($c++ < $e) {
                        $baseCell
                            = $this->_isSpannedCell[$sheetIndex][$rowIndex][$c]['baseCell'];
    ...
    private function _setMargins(PHPExcel_Worksheet $pSheet)
    {
        ...
        $right
            = PHPExcel_Shared_String::FormatNumber($pSheet->getPageMargins()
                                                          ->getRight())
              . 'in; ';
