    ...
    PHPExcel_Properties
{
    ...
    public function setLineColorProperties(
        $value,
        $alpha = 0,
        $type = self::EXCEL_COLOR_TYPE_STANDARD
    ) {
    ...
    public function setLineStyleProperties(
        ...
        ( ! is_null($line_width))
            ? $this->_line_properties['style']['width']
            = $this->getExcelPointsWidth((float)$line_width)
            : null;
        ( ! is_null($compound_type))
            ? $this->_line_properties['style']['compound']
        ...
        ( ! is_null($head_arrow_type))
            ? $this->_line_properties['style']['arrow']['head']['type']
            = (string)$head_arrow_type
            : null;
        ( ! is_null($head_arrow_size))
            ? $this->_line_properties['style']['arrow']['head']['size']
            = (string)$head_arrow_size
            : null;
        ( ! is_null($end_arrow_type))
            ? $this->_line_properties['style']['arrow']['end']['type']
            = (string)$end_arrow_type
            : null;
        ( ! is_null($end_arrow_size))
            ? $this->_line_properties['style']['arrow']['end']['size']
    ...
    {
        return $this->getArrayElementsValue($this->_line_properties['style'],
            $elements);
    }
    ...
    private function _setGlowColor($color, $alpha, $type)
    {
        ...
        if ( ! is_null($alpha)) {
            $this->_glow_properties['color']['alpha']
    ...
    private function _setShadowColor($color, $alpha, $type)
    {
        ...
        if ( ! is_null($alpha)) {
            $this->_shadow_properties['color']['alpha']
                = $this->getTrueAlpha((int)$alpha);
        }
        if ( ! is_null($type)) {
