...
class PHPExcel_Chart_Renderer_jpgraph
{
    ...
    private function _formatDataSetLabels(
        $groupID,
        $datasetLabels,
        $labelCount,
        $rotation = ''
        ...
        foreach ($datasetLabels as $i => $datasetLabel) {
            if (is_array($datasetLabel)) {
                ...
                if ( ! is_null($datasetLabelFormatCode)) {
                    $datasetLabels[$i]
                        = PHPExcel_Style_NumberFormat::toFormattedString($datasetLabel,
    ...
    private function _renderLegend()
    {
        ...
        if ( ! is_null($legend)) {
            ...
            switch ($legendPosition) {
                case 'r'    :
                    $this->_graph->legend->SetPos(0.01, 0.5, 'right',
                        'center');    //	right
                    $this->_graph->legend->SetColumns(1);
                    break;
                case 'l'    :
                    $this->_graph->legend->SetPos(0.01, 0.5, 'left',
                        'center');    //	left
                    $this->_graph->legend->SetColumns(1);
                    break;
                case 't'    :
                    $this->_graph->legend->SetPos(0.5, 0.01, 'center',
                        'top');    //	top
                    break;
                case 'b'    :
                    $this->_graph->legend->SetPos(0.5, 0.99, 'center',
                        'bottom');    //	bottom
                    break;
                default        :
                    $this->_graph->legend->SetPos(0.01, 0.01, 'right',
                        'top');    //	top-right
                    $this->_graph->legend->SetColumns(1);
                    break;
            }
    ...
    private function _renderCartesianPlotArea($type = 'textlin')
    {
        ...
        $rotation = $this->_chart->getPlotArea()->getPlotGroupByIndex(0)
                                 ->getPlotDirection();
    ...
    private function _renderPlotLine(
        $groupID,
        $filled = false,
        $combination = false,
        $dimensions = '2d'
    ) {
        $grouping = $this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)
                                 ->getPlotGrouping();
        ...
        if ($grouping == 'percentStacked') {
            $sumValues = $this->_percentageSumCalculation($groupID,
                $seriesCount);
        }
        ...
        for ($i = 0; $i < $seriesCount; ++$i) {
            ...
            if ($grouping == 'percentStacked') {
                $dataValues = $this->_percentageAdjustValues($dataValues,
                    $sumValues);
            }
            ...
            $dataLabel = $this->_chart->getPlotArea()
                                      ->getPlotGroupByIndex($groupID)
                                      ->getPlotLabelByIndex($i)->getDataValue();
    ...
    {
        $rotation = $this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)
                                 ->getPlotDirection();
        //	Rotate for bar rather than column chart
        ...
        for ($j = 0; $j < $seriesCount; ++$j) {
            ...
            if ( ! $this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)
    ...
    private function _renderPlotScatter($groupID, $bubble)
    {
        $grouping = $this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)
                                 ->getPlotGrouping();
        $scatterStyle
            = $bubbleSize = $this->_chart->getPlotArea()
                                         ->getPlotGroupByIndex($groupID)
                                         ->getPlotStyle();
        ...
        for ($i = 0; $i < $seriesCount; ++$i) {
            ...
            if ($scatterStyle == 'lineMarker') {
                ...
                list($splineDataY, $splineDataX)
                    = $spline->Get(count($dataValuesX) * self::$_width / 20);
    ...
    {
        require_once(PHPExcel_Settings::getChartRendererPath()
                     . 'jpgraph_line.php');
    ...
    private function _renderPieChart(
        $groupCount,
        $dimensions = '2d',
        $doughnut = false,
        $multiplePlots = false
    ...
    private function _renderCombinationChart(
        $groupCount,
        $dimensions,
        $outputDestination
    ) {
        ...
        for ($i = 0; $i < $groupCount; ++$i) {
            $dimensions = null;
            $chartType = $this->_chart->getPlotArea()->getPlotGroupByIndex($i)
                                      ->getPlotType();
            switch ($chartType) {
                case 'area3DChart' :
                    $dimensions = '3d';
    ...
    public function render($outputDestination)
    {
        ...
        $dimensions = null;
        if ($groupCount == 1) {
            $chartType = $this->_chart->getPlotArea()->getPlotGroupByIndex(0)
                                      ->getPlotType();
        } else {
            $chartTypes = [];
            for ($i = 0; $i < $groupCount; ++$i) {
            ...
            if (count($chartTypes) == 1) {
                ...
                return $this->_renderCombinationChart($groupCount, $dimensions,
                    $outputDestination);
            }
        }
