 ...
 * @copyright  Copyright (c) 2006 - 2014 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor
    implements PHPExcel_IComparable
{
    ...
    public function setBuiltInFormatCode($pValue = 0)
    {
        ...
        if ($this->_isSupervisor) {
            $styleArray
                = $this->getStyleArray(['code' => self::builtInFormatCode($pValue)]);
            $this->getActiveSheet()->getStyle($this->getSelectedCells())
                 ->applyFromArray($styleArray);
        } else {
    ...
    private static function _formatAsFraction(&$value, &$format)
    {
        ...
        $GCD = PHPExcel_Calculation_MathTrig::GCD($decimalPart,
            $decimalDivisor);
    ...
    private static function _complexNumberFormatMask($number, $mask, $level = 0)
    {
        ...
        if (strpos($mask, '.') !== false) {
            ...
            $result1 = self::_complexNumberFormatMask($numbers[0], $masks[0],
                1);
            $result2
                = strrev(self::_complexNumberFormatMask(strrev($numbers[1]),
                strrev($masks[1]), 1));
        ...
        if ($r > 1) {
            ...
            foreach ($result as $block) {
                ...
                $mask = substr_replace($mask, $blockValue, $offset,
                    $size);
            }
            if ($number > 0) {
    ...
    public static function toFormattedString(
        ...
        if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i',
            $format)
        ) { // datetime format
...
//				$format = str_replace(array('\\"', '*'), array('"', ''), $format);
                ...
                if (preg_match('/#?.*\?\/\?/', $format, $m)) {
                    ...
                    if (preg_match($number_regex, $m, $matches)) {
                        ...
                        $minWidth = strlen($left) + strlen($dec)
                                    + strlen($right);
                        if ($useThousands) {
                            $value = number_format(
                                $value
                            ...
                            $value = preg_replace($number_regex, $value,
                                $format);
                        } else {
                            if (preg_match('/[0#]E[+-]0/i', $format)) {
                                ...
                                $value = self::_complexNumberFormatMask($value,
                                    $format);
                            } else {
                                $sprintf_pattern = "%0$minWidth."
                                                   . strlen($right) . "f";
                                $value = sprintf($sprintf_pattern,
                                    $value);
                                $value = preg_replace($number_regex,
                                    $value, $format);
                            }
                        }
                    }
                ...
                if (preg_match('/\[\$(.*)\]/u', $format, $m)) {
                    ...
                    $value = preg_replace('/\[\$([^\]]*)\]/u', $currencyCode,
                        $value);
                }
            }
        }
