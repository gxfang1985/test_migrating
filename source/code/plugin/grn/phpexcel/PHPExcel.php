...
class PHPExcel
{
    ...
    {//we need some constants here...
        ...
        switch ($What) {
            ...
            case 'target':
            case 'data':
                if (is_array($this->_ribbonXMLData)
                    && array_key_exists($What, $this->_ribbonXMLData)
                ) {
     ...
     *
     */
    public function setRibbonBinObjects(
        $BinObjectsNames = null,
        $BinObjectsData = null
    ) {
    ...
    public function getRibbonBinObjects($What = 'all')
    {
        ...
        switch ($What) {
            ...
            case 'names':
            case 'data':
                if (is_array($this->_ribbonBinObjects)
                    && array_key_exists($What, $this->_ribbonBinObjects)
                ) {
            ...
            case 'types':
                if (is_array($this->_ribbonBinObjects)
                    && array_key_exists('data', $this->_ribbonBinObjects)
    ...
    public function addNamedRange(PHPExcel_NamedRange $namedRange)
    {
        if ($namedRange->getScope() == null) {
            ...
            // local scope
            $this->_namedRanges[$namedRange->getScope()->getTitle() . '!'
                                . $namedRange->getName()]
                = $namedRange;
        }
    ...
    public function getNamedRange(
        ...
        if ($namedRange != '' && ($namedRange !== null)) {
            ...
            if (($pSheet !== null)
                && isset($this->_namedRanges[$pSheet->getTitle() . '!'
                                             . $namedRange])
            ) {
     ...
     * @return PHPExcel
     */
    public function removeNamedRange(
        $namedRange,
        PHPExcel_Worksheet $pSheet = null
    ) {
        if ($pSheet === null) {
    ...
    public function garbageCollect()
    {
        ...
        foreach ($this->_cellXfCollection as $index => $cellXf) {
            if ($countReferencesCellXf[$index] > 0
                || $index == 0
