 ...
 *
 */
function smarty_function_grn_workflow_cellular_string_split($params, &$smarty)
{
    $function_name = 'grn_workflow_cellular_string_split';
    ...
    foreach ($matches as $start => $len) {
        ...
        if ($len) {
            ...
            require_once($smarty->_get_plugin_filepath('function',
                'grn_workflow_cellular_mailto'));
            $mailto_params = ['mail' => $_mailaddress];
            $_tag
                = smarty_function_grn_workflow_cellular_mailto($mailto_params,
                $smarty);
        }
    ...
    if ($page_number > 0) {
        $link_param = array_merge($G_INPUT, [
            ...
            'caption'      => cb_msg('grn.workflow.cellular', 'previous_page'),
            'fragment'     => "body",
            $page_paraname => $page_number - 1
        ]);
        $output .= smarty_function_grn_workflow_cellular_link($link_param,
            $smarty);
        $output .= "<br>..";
    }
    ...
    if ($page_number + 1 < count($out)) {
        $link_param = array_merge($G_INPUT, [
            'page'         => cb_get_pagename(),
            'caption'      => cb_msg('grn.workflow.cellular', 'next_page'),
            'fragment'     => "body",
            $page_paraname => $page_number + 1
        ]);
        $output .= "..<br>";
