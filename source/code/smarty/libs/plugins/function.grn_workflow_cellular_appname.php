...
/**
 * @name   grn_workflow_cellular_appname
 * workflowアプリケーション名を表示
 *
 ...
 *
 *  記述例
 *  <pre>{grn_workflow_cellular_appname no_icon="1"}
 */
function smarty_function_grn_workflow_cellular_appname($params, &$smarty)
{
    ...
    if (__grn_workflow_is_cellular_application()) {
        ...
        $params['app_id'] = 'workflow';

        return smarty_function_grn_cellular_appname($params, $smarty);
    }

    $function_name = "grn_workflow_cellular_appname";
    ...
    if (array_key_exists($app_id, $app_array)) {
        ...
        if ($show_icon && strlen($app_array[$app_id]['icon'])) {
            ...
            $icon
                = smarty_function_grn_workflow_cellular_pictogram(['key' => $app_array[$app_id]['icon']],
                $smarty);
        }
    ...
    if (strlen($icon_name)) {
        require_once($smarty->_get_plugin_filepath('function',
            'grn_workflow_cellular_pictogram'));
        $icon
            = smarty_function_grn_workflow_cellular_pictogram(['key' => $icon_name],
            $smarty);
    }
