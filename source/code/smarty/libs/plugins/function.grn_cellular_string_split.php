 ...
 *
 */
function smarty_function_grn_cellular_string_split($params, &$smarty)
{
    $function_name = 'grn_cellular_string_split';
    ...
    foreach ($matches as $start => $len) {
        ...
        if ($len) {
            ...
            $_tag = smarty_function_grn_cellular_mailto($mailto_params,
                $smarty);
        }
    ...
    if ($page_number > 0) {
        $link_param = array_merge($G_INPUT, [
            ...
            'caption'      => cb_msg('grn.cellular.common', 'previous_page'),
            'fragment'     => "body",
            $page_paraname => $page_number - 1
        ]);
        $output .= smarty_function_grn_cellular_link($link_param, $smarty);
        $output .= "<br>..";
    }
    ...
    $output .= str_replace("<br />\r\n", "<br />" . $indent_str,
        nl2br($out[$page_number]));

    if ($page_number + 1 < count($out)) {
        $link_param = array_merge($G_INPUT, [
            'page'         => cb_get_pagename(),
            'caption'      => cb_msg('grn.cellular.common', 'next_page'),
            'fragment'     => "body",
            $page_paraname => $page_number + 1
        ]);
        $output .= "..<br>";
