...
function smarty_function_grn_cellular_link($params, &$smarty)
{
    $function_name = "grn_cellular_link";
    ...
    require_once($smarty->_get_plugin_filepath('function',
        'grn_cellular_pageurl'));
    $page = smarty_function_grn_cellular_pageurl($pageurl_params, $smarty);
    ...
    if (is_numeric($accesskey) && $accesskey >= -1 && $accesskey < 10) {
        ...
        if ($accesskey != -1) {
            switch ($accesskey) {
                case 10:
                    $str .= " accesskey='#' directkey='#'>";
                    break;
                default:
                    $str .= " accesskey='{$accesskey}' directkey='{$accesskey}'>";
                    break;
            }

            require_once($smarty->_get_plugin_filepath('function',
                'grn_cellular_pictogram'));
            $str .= smarty_function_grn_cellular_pictogram([
                'key' => 'num' . $accesskey
            ], $smarty);
        } else {
            $str .= '>';
    ...
    if ($icon) {
        ...
        if (is_array($icon)) {
            foreach ($icon as $i) {
                $str .= smarty_function_grn_cellular_pictogram(['key' => $i],
                    $smarty);
            }
        } else {
            $str .= smarty_function_grn_cellular_pictogram(['key' => $icon],
                $smarty);
        }
    }
