...
function smarty_function_grn_report_member($params, &$smarty)
{
    $t = new GRN_Smarty();

    // 必須チェック
    if ( ! array_key_exists('members', $params)) {
        return 'grn_report_member: missing members parameter';
    }

    $addressee_ids = [];
    foreach ($params['members'] as $addressee) {
    ...
    return $t->fetch('report/_view_member.tpl');
}
