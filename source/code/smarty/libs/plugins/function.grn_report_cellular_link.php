...
function smarty_function_grn_report_cellular_link($params, &$smarty)
{
    $function_name = "grn_report_cellular_link";
    ...
    require_once($smarty->_get_plugin_filepath('function',
        'grn_report_cellular_pageurl'));
    $page = smarty_function_grn_report_cellular_pageurl($pageurl_params,
        $smarty);
    ...
    if (is_numeric($accesskey) && $accesskey >= -1 && $accesskey < 10) {
        ...
        if ($accesskey != -1) {
            ...
            $str .= smarty_function_grn_report_cellular_pictogram([
                'key' => 'num' . $accesskey
            ], $smarty);
        } else {
            $str .= '>';
//            $str .= cb_msg('grn.cellular.common','accesskey_nothing');
    ...
    if ($icon) {
        ...
        if (is_array($icon)) {
            foreach ($icon as $i) {
                $str .= smarty_function_grn_report_cellular_pictogram(['key' => $i],
                    $smarty);
            }
        } else {
            $str .= smarty_function_grn_report_cellular_pictogram(['key' => $icon],
                $smarty);
        }
    }
