...
<span class="display_options">
    ...
    <span id="display_options_dialog{$plid}" class="display_options_dialog" style="display:none;">
      <ul>
        <li class="display_options_dialog_ch">{if $is_not_manage || $navi.count < 1}{assign var='delete_disabled' value='true'}{/if}{capture name='grn_report_operation_GRN_RPRT_OP_284'}{cb_msg module='grn.report.operation' key='GRN_RPRT_OP-284' replace='true'}{/capture}{grn_link page='report/operation/filter_delete_all' caption=$smarty.capture.grn_report_operation_GRN_RPRT_OP_284 image='delete20.gif' cid=$category_id fid=$form_id disabled=$delete_disabled}
        </li>
      </ul>
    </span>
