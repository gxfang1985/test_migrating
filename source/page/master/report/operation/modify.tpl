...
<p class="explanation">{cb_msg module='grn.report.operation' key='GRN_RPRT_OP-198' replace='true'}</p>
{assign var='half_width_number' value='1'}
{include file='grn/show_validation_errors.tpl'}
{include file='grn/indispensable.tpl'}

<p></p>
{cb_msg module='grn.report.operation' key='GRN_RPRT_OP-199' replace='true'}<span class="bold">{grn_image image='report20.gif'}{$report_form_name|escape}</span>{cb_msg module='grn.report.operation' key='GRN_RPRT_OP-200' replace='true'}
<p></p>
...
<table class="std_form">
 ...
 <tr>
  <td></td>
  <td>
   {capture name='grn_report_operation_GRN_RPRT_OP_210'}{cb_msg module='grn.report.operation' key='GRN_RPRT_OP-210' replace='true'}{/capture}{grn_button_submit class='button_bold margin' caption=$smarty.capture.grn_report_operation_GRN_RPRT_OP_210 onclick='this.form.cmd.value="next";if(confirmIfExistFailedFile() && report_file_validate(false)) grn_onsubmit_common(this.form); else return false;'}
