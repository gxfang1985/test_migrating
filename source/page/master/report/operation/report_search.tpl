...
<!--
    searchReportUrl = {/literal}'{grn_pageurl page="report/operation/search_report_list"}'{literal};
...
<form name="{$form_name}" id="searchConditionForm" method="post" action="{grn_pageurl page=$page_prefix|cat:'/report_search'}">
    ...
    <input type="hidden" name="from" value="operation">
    ...
    <div class="explanation">{cb_msg module='grn.report.operation' key='GRN_RPRT_OP-262' replace='true'}</div>

    {include file="grn/show_validation_errors.tpl"}
    {include file='grn/indispensable.tpl'}
    <table class="std_form">
        <tr valign="top">
            <th nowrap>{cb_msg module='grn.report.operation' key='GRN_RPRT_OP-263' replace='true'}</th>
            <td>
                {capture name=suffix}
                    {if $form.deleted == 1}
        ...
        <tr valign="top">
            <th nowrap>{capture name='grn_report_operation_GRN_RPRT_OP_264'}{cb_msg module='grn.report.operation' key='GRN_RPRT_OP-264' replace='true'}{/capture}{grn_show_input_title title=$smarty.capture.grn_report_operation_GRN_RPRT_OP_264}</th>
    ...
    <div id="menu_part">
        <div id="smart_main_menu_part">
            <span class="menu_item">
                {capture name='grn_report_operation_r_output_CSV'}{cb_msg module='grn.report' key='r_output_CSV' replace='true'}{/capture}{grn_link page=$link caption=$smarty.capture.grn_report_operation_r_output_CSV image='export20.gif' cid=$category_id fid=$form_id disabled=true}
            </span>
        </div>
    </div>
</div>
