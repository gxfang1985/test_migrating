...
<form name="{$form_name}" method="post" action="{grn_pageurl page=$post_page}">
 ...
 <td></td>
 <td>
 <div class="sub_title">{cb_msg module='grn.system.user' key='GRN_SY_US-396' replace='true'}</div>

  <table class="admin_list_table" style="width:50%">
  <tr>
  <th style="text-align:center" nowrap>{cb_msg module='grn.system.user' key='GRN_SY_US-397' replace='true'}</th>
  </tr>
  <tr>
  <td><div align="center">{grn_checkbox name="authority_transferable" id="authority_transferable" value='1' checked=$authorities.transferable}</div></td>
  </tr>
  </table>
