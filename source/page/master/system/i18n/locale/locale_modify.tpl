{include file='grn/system_head.tpl'} {include file='grn/system_header.tpl'} {grn_title title=$page_title class=$page_info.parts[0]} {assign var='form_name' value=$smarty.template|basename}
<form name="{$form_name}" method="post" action="{grn_pageurl page='system/i18n/locale/command_locale_modify'}"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
<p></p>
...
<table class="std_form">
  <tbody>
    <tr>
      ...
      <td>{capture name='tmp'}{cb_msg module='grn.system.i18n.locale' key='Locale-14' replace='true'}{/capture}{grn_text_multilanguage element_name='locale-name' form_name=$form_name values=$locale_names error_message=$smarty.capture.tmp}</td>
    </tr>
    <tr>
      <th>{cb_msg module='grn.system.i18n.locale' key='Locale-15' replace='true'}<span class="attention">*</span></th>
      <td>{capture name='tmp'}{cb_msg module='grn.system.i18n.locale' key='Locale-15' replace='true'}{/capture}{grn_text name='locale-code' maxlength='100' size='50' title=$smarty.capture.tmp value=$locale_code disable_return_key=true}</td>
    ...
    <tr>
      ...
      <td><select name="language" id="language">
          ...
          <option value="{$language_code}" {if $language_code == $language}selected{/if}>{cb_language_name code=$language_code}</option>
          {/foreach}
      </select></td>
    </tr>
    <tr>
      ...
      <td><select name="long-date-format" id="long-date-format">
          {foreach from=$long_date_format_list.$language item=iter_long_date_format key=long_date_format_key}
          <option value="{$long_date_format_key}" {if $iter_long_date_format==$long_date_format}selected{/if}>{grn_date_format format=$iter_long_date_format language=$language}</option>
          {/foreach}
    ...
    <tr>
      ...
      <td><select name="short-date-format" id="short-date-format">
          {foreach from=$short_date_format_list.$language item=iter_short_date_format key=short_date_format_key}
          <option value="{$short_date_format_key}" {if $iter_short_date_format==$short_date_format}selected{/if}>{grn_date_format format=$iter_short_date_format language=$language}</option>
          {/foreach}
    ...
    <tr>
      ...
      <td><select name="time-format" id="time-format">
          {foreach from=$time_format_list.$language item=iter_time_format key=time_format_key}
          <option value="{$time_format_key}" {if $iter_time_format==$time_format}selected{/if}>{grn_date_format format=$iter_time_format language=$language}</option>
          {/foreach}
          {capture name='tmp'}{cb_msg module='grn.system.i18n.locale' key='Locale-23' replace='true'}{/capture}{validate form=$form_name field="time-format" criteria='notEmpty' transform='cb_trim' append='validation_errors' message=$smarty.capture.tmp}
        </select>
      </td>
    </tr>
    <tr>
      ...
      <td>
          ...
          {grn_button_cancel page="system/i18n/locale/locale_detail" lid=$lid}
    </tr>
  </tbody>
</table>
