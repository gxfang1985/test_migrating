...
<div data-role="content" data-theme="{$data_theme}" class="mobile-content-padding-grn">
    ...
    <div class="mobile_schedulelist_menu_grn mobile_attendance_userListSwitch_grn">
      <div class="ui-grid-b">
        {if $tab_id eq 1}
            ...
            <div class="ui-block-b"><a href="{grn_pageurl page=$schedule_prefix|cat:'/attendee_list' event=$event_id bdate=$bdate uid=$user_id gid=$group_id referer_key=$referer_key tab_id=2}" class="ui-link">{cb_msg module='grn.schedule.mobile' key='GRN_SCH_MOBILE-31' replace='true'}</a></div>
            <div class="ui-block-c"><a href="{grn_pageurl page=$schedule_prefix|cat:'/attendee_list' event=$event_id bdate=$bdate uid=$user_id gid=$group_id referer_key=$referer_key tab_id=3}" class="ui-link">{cb_msg module='grn.schedule.mobile' key='GRN_SCH_MOBILE-29' replace='true'}</a></div>
        {elseif $tab_id eq 2}
            <div class="ui-block-a"><a href="{grn_pageurl page=$schedule_prefix|cat:'/attendee_list' event=$event_id bdate=$bdate uid=$user_id gid=$group_id referer_key=$referer_key tab_id=1}" class="ui-link">{cb_msg module='grn.schedule.mobile' key='GRN_SCH_MOBILE-30' replace='true'}</a></div>
  ...
  {else}
    <div class="mobile-title-member-grn">
      {$smarty.capture.list_title}{$count}{cb_msg module='grn.mobile' key='GRN_MOBILE-1' replace='true'}
    </div>
  {/if}
  <!-- End GTM-1630 -->
  <div class="mobile-padding-leftright-grn">
    <ul data-role="listview" data-theme="{$data_theme}" id="member_list" class="mobile-top-listview-member-grn mobile-bottom-listview-member-grn mobile_attendance_userListUl_grn">
      {include file='grn/mobile_userinfo_list.tpl'}
    </ul>
