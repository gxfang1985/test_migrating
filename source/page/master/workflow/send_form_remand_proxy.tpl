...
<p class="explanation">{cb_msg module='grn.workflow' key='enter_application_information' replace='true'}</p>
{assign var='half_width_number' value='1'}
{include file='grn/_ajax_submit_check_error.tpl'}
{include file='grn/indispensable.tpl'}

<p></p>
{cb_msg module='grn.workflow' key='opened_bracket' replace='true'}<span class="bold">{grn_workflow_form_icon_image icon_type=$petition.icon_type icon_id=$petition.icon_id icon_url=$petition.icon_url}{$petition.form_name|escape}</span>{cb_msg module='grn.workflow' key='closed_bracket' replace='true'}
...
{foreach from=$item_data_list item=item_data_line}
     ...
     <tr valign="top">
         <td colspan="2">
             ...
             {else}
                 <table>
                     <tr>
                         {grn_workflow_include_item folder_id=$folder_id petition_id=$petition_id iid=$item_data_line.iid item=$item_data_line display="input"}
                         {foreach from=$item_data_line.sub_item_data_list item=sub_item_data}
                             {grn_workflow_include_item folder_id=$folder_id petition_id=$petition_id iid=$sub_item_data.iid item=$sub_item_data display="input"}
                         {/foreach}
                     </tr>
                 </table>
             {/if}
 ...
 <tr>
  <td>
   {if $item_data_line.description_type != 1}
    {if $item_data_line.description_editor == 1}
   <div class="adjust_link">{$item_data_line.description|grn_noescape}</div>
    {else}
     {grn_format body=$item_data_line.description}
    {/if}
   {else}
     <div class="adjust_link"><small>{capture name='grn_workflow_description'}{cb_msg module='grn.workflow' key='whats_this' replace='true'}{/capture}{grn_link image='popup14.gif' page='workflow/send_description_common' caption="[`$smarty.capture.grn_workflow_description`]" target='_blank' folder_id=$folder_id petition_id=$petition_id item_data_id=$item_data_line.iid mode=$mode}</small></div>
   {/if}
  </td>
 </tr>
  {/if}
  {if $item_data_line.type == 1}
    <tr valign="top">
     <th rowspan="1" align="left" width="30%" nowrap>{cb_msg module='grn.workflow' key='col_agent_type_0' replace='true'}</th>
     <td nowrap>{grn_user_name uid=$petition.agent_user name=$petition.agent_user_name}</td>
    </tr>
    <tr valign="top">
     <th rowspan="1" align="left" width="30%" nowrap>{cb_msg module='grn.workflow' key='mandator' replace='true'}</th>
     <td nowrap>{grn_user_name uid=$petition.user name=$petition.user_name}</td>
    </tr>
  {/if}
 {/if}
{/foreach}
 ...
 <tr>
  ...
  <td>
   ...
   <div class="mTop10">
    ...
    {capture name='grn_pageurl_command_send_form_remand_proxy'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
    {capture name='grn_workflow_set_route_onclick'}
        ...
        if((typeof(workflow_file_validate)=='function') && (!confirmIfExistFailedFile() || !workflow_file_validate(false)))
            return false;
        else grn.page.workflow.ajax_submit.workflow_submit('{$form_name}', '{$smarty.capture.grn_pageurl_command_send_form_remand_proxy}', this);
    {/capture}
    {grn_button id='workflow_button_set_route' ui='main' spacing='normal' caption=$smarty.capture.grn_workflow_set_path_2 onclick=$smarty.capture.grn_workflow_set_route_onclick}
    {grn_button id='workflow_button_cancel' class="button_submit_grn" caption=$smarty.capture.grn_workflow_do_cancel onclick="if(grn.component.button('#workflow_button_cancel').isDisabled())return false; document.forms['$form_name'].cmd.value='cancel'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_form_remand_proxy`', this);"}
   </div>
   {/strip}
  </td>
 </tr>
