   ...
   <tr valign="top">
       <td colspan="2">
           ...
           <table class="layout-border-none" width="100%">
               <tr>
                   {grn_workflow_include_item folder_id=$folder_id petition_id=$petition_id iid=$item_data_line.iid item=$item_data_line display="view-confirm"}
                   {foreach from=$item_data_line.sub_item_data_list item=sub_item_data}
                       {grn_workflow_include_item folder_id=$folder_id petition_id=$petition_id iid=$sub_item_data.iid item=$sub_item_data display="view-confirm"}
                   {/foreach}
               </tr>
           </table>
       </td>
 ...
 {else}
  ...
  {if $item_data_line.type == 1}
    <tr valign="top">
     ...
     <td nowrap>{grn_user_name uid=$login.id name=$login.name users_info=$users_info}</td>
    </tr>
  {/if}
 {/if}
{/foreach}
...
<small>
{if !$simple}
{cb_msg module='grn.workflow' key='bracket_arrow_left' replace='true'}{capture name='grn_workflow_hide_processor'}{cb_msg module='grn.workflow' key='hide_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_remand' caption=$smarty.capture.grn_workflow_hide_processor fid=$folder_id pid=$petition_id sf=1 simple=1 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}
{else}
{cb_msg module='grn.workflow' key='bracket_arrow_right' replace='true'}{capture name='grn_workflow_show_all_processor'}{cb_msg module='grn.workflow' key='show_all_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_remand' caption=$smarty.capture.grn_workflow_show_all_processor fid=$folder_id pid=$petition_id sf=1 simple=0 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}
{/if}
</small>
...
<table class="list_column">
 ...
 <tr>
  <td class="wf_route_arrow_area_grn" align="center" rowspan="{$path_step_data_count|escape}"><span class="wf_route_arrow_grn">{cb_msg module='grn.workflow' key='arrow_down' replace='true'}</span></td>
 {foreach from=$path_step_data_list key=psid item=path_step_data}
  {if GRN_WORKFLOW_CIRCULAR == $path_step_data.col_type}
  <td nowrap>
   {cb_msg module='grn.workflow' key='pass_along' replace='true'}
  {else}
   {if $path_step_data.last_acceptance}
  <td class="last_decision" nowrap>
   <span class="kind">{cb_msg module='grn.workflow' key="col_acceptance_type_`$path_step_data.col_acceptance_type`"}</span>
   {else}
  <td nowrap>
   {cb_msg module='grn.workflow' key="col_acceptance_type_`$path_step_data.col_acceptance_type`"}
   {/if}
  {/if}
  </td>
  <td>{$path_step_data.col_role|escape}</td>
  {if !$simple}
  <td>
   <table class="layout">
   {foreach from=$path_step_data.users item=user}
    <tr>
{if $user._id}
...
{/if}
  ...
  {else}
   {if $next_flag != 1}
  <td align="center" rowspan="{$path_step_data_count|escape}" nowrap>
   <small>{cb_msg module='grn.workflow' key='bracket_arrow_right' replace='true'}{capture name='grn_workflow_show_all_processor'}{cb_msg module='grn.workflow' key='show_all_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_remand' caption=$smarty.capture.grn_workflow_show_all_processor fid=$folder_id pid=$petition_id sf=1 simple=0 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}</small>
  </td>
...
{strip}
    <div class="mTop15 mBottom15">
      ...
      {capture name='grn_workflow_apply'}{cb_msg module='grn.workflow' key='apply' replace='true'}{/capture}
      {capture name='grn_workflow_do_cancel'}{cb_msg module='grn.workflow' key='do_cancel' replace='true'}{/capture}
      {capture name='grn_pageurl_command_send_confirm_remand'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
      {grn_button id='workflow_button_back' spacing='tight' caption=$smarty.capture.grn_workflow_back_to_prev_screen onclick="document.forms['$form_name'].cmd.value='back'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_remand`', this);"}
      {grn_button id='workflow_button_create_proxy_request' ui='main' spacing='loose' caption=$smarty.capture.grn_workflow_apply onclick="document.forms['$form_name'].cmd.value='send'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_remand`', this);"}
      {grn_button id='workflow_button_cancel' caption=$smarty.capture.grn_workflow_do_cancel onclick="document.forms['$form_name'].cmd.value='cancel'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_remand`', this);"}
    </div>
{/strip}
