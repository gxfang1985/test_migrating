     ...
     <tr valign="top">
         <td colspan="2">
             ...
             {else}
                 <table>
                     <tr>
                         {grn_workflow_include_item category_id=$category_id form_id=$form_id iid=$item_line.iid item=$item_line display="input"}
                         {foreach from=$item_line.sub_item_list item=sub_item}
                             {grn_workflow_include_item category_id=$category_id form_id=$form_id iid=$sub_item.iid item=$sub_item display="input"}
                         {/foreach}
                     </tr>
                 </table>
             {/if}
 ...
 <tr>
  ...
  <td>
   ...
   <div class="mTop10">
    ...
    {capture name='grn_workflow_save_as_draft'}{cb_msg module='grn.workflow' key='save_as_draft' replace='true'}{/capture}
    {capture name='grn_pageurl_command_send_form'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
    {capture name='grn_workflow_set_route_onclick'}
        if(grn.component.button("#{$page_info.last}_submit_set_path").isDisabled())
            return false;
        ...
        if((typeof(workflow_file_validate)=='function') && (!confirmIfExistFailedFile() || !workflow_file_validate(false)))
            return false;
        else grn.page.workflow.ajax_submit.workflow_submit('{$form_name}', '{$smarty.capture.grn_pageurl_command_send_form}', this);
    {/capture}
    {capture name='grn_workflow_save_as_draft_onclick'}
        if(grn.component.button("#{$page_info.last}_submit_save_as_draft").isDisabled())
            return false;
        document.forms['{$form_name}'].cmd.value='save';
        if((typeof(workflow_file_validate)=='function') && !confirmIfExistFailedFile() && (!confirmIfExistFailedFile() || !workflow_file_validate(true)))
            return false;
        else grn.page.workflow.ajax_submit.workflow_submit('{$form_name}', '{$smarty.capture.grn_pageurl_command_send_form}', this);
    {/capture}
     {grn_button id=`$page_info.last`_submit_set_path ui='main' spacing='loose' caption=$smarty.capture.grn_workflow_set_path_2 onclick=$smarty.capture.grn_workflow_set_route_onclick}
     {grn_button id=`$page_info.last`_submit_save_as_draft class="button_submit_grn" spacing='tight' caption=$smarty.capture.grn_workflow_save_as_draft onclick=$smarty.capture.grn_workflow_save_as_draft_onclick}
     {grn_button id='workflow_button_cancel' action='cancel' class="button_submit_grn" onclick="if(grn.component.button('#workflow_button_cancel').isDisabled())return false; document.forms['$form_name'].cmd.value='cancel'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_form`', this);"}
   </div>
   {/strip}
  </td>
 </tr>
