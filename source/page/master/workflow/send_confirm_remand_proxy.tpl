...
<small>
{if !$simple}
{cb_msg module='grn.workflow' key='bracket_arrow_left' replace='true'}{capture name='grn_workflow_hide_processor'}{cb_msg module='grn.workflow' key='hide_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_remand_proxy' caption=$smarty.capture.grn_workflow_hide_processor fid=$folder_id pid=$petition_id sf=1 simple=1 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}
{else}
{cb_msg module='grn.workflow' key='bracket_arrow_right' replace='true'}{capture name='grn_workflow_show_all_processor'}{cb_msg module='grn.workflow' key='show_all_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_remand_proxy' caption=$smarty.capture.grn_workflow_show_all_processor fid=$folder_id pid=$petition_id sf=1 simple=0 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}
{/if}
</small>
...
{/if}
  ...
  <td align="center" rowspan="{$path_step_data_count|escape}" nowrap>
   <small>{cb_msg module='grn.workflow' key='bracket_arrow_right' replace='true'}{capture name='grn_workflow_show_all_processor'}{cb_msg module='grn.workflow' key='show_all_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_remand_proxy' caption=$smarty.capture.grn_workflow_show_all_processor fid=$folder_id pid=$petition_id sf=1 simple=0 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}</small>
  </td>
...
{strip}
   <div class="mTop15 mBottom15">
       ...
       {grn_button id='workflow_button_create_proxy_request' ui='main' spacing='loose' caption=$smarty.capture.grn_workflow_apply_per_pro onclick="document.forms['$form_name'].cmd.value='send'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_path_remand_proxy`', this);"}
