...
<table class="view_table">
 ...
 <tr valign="top">
   <th>{cb_msg module='grn.workflow' key='date_type' replace='true'}</th>
   <td>{if $item.settings.date_type == 0}{cb_msg module='grn.workflow' key='date' replace='true'}{else}{cb_msg module='grn.workflow' key='date_time' replace='true'}{/if}</td>
 </tr>
 <tr valign="top">
  <th>{cb_msg module='grn.workflow' key='default_value' replace='true'}</th>
  <td>
      {if $item.settings.initial_type == 0}
        {cb_msg module='grn.workflow' key='date_application_created' replace='true'}
      {elseif $item.settings.initial_type == 1}
        {if $item.settings.initial_year}{$item.settings.initial_year|escape}{cb_msg module='grn.workflow' key='GRN_WKF_13' replace='true'}{/if}
        {$item.settings.initial_month|escape}{cb_msg module='grn.workflow' key='GRN_WKF_14' replace='true'}
        {$item.settings.initial_day|escape}{cb_msg module='grn.workflow' key='GRN_WKF_15' replace='true'}
        {if $item.settings.date_type neq 0}
          {if $item.settings.initial_hour != ''}{$item.settings.initial_hour|escape}{else}--{/if}{cb_msg module='grn.workflow' key='GRN_WKF_18' replace='true'}
          {if $item.settings.initial_minute != ''}{$item.settings.initial_minute|escape}{else}--{/if}{cb_msg module='grn.workflow' key='GRN_WKF_19' replace='true'}
        {/if}
      {else}
        {cb_msg module='grn.workflow' key='blank' replace='true'}
      {/if}
  </td>
 </tr>
