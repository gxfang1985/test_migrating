...
{assign var='access_page' value='workflow/system/change_access_list'}
{elseif $display == 'detail-operation'}
{assign var='access_page' value='workflow/operation/change_access_list'}
{else}
{assign var='access_page' value='workflow/change_access_list'}
{/if}
...
<table class="view_table">
 ...
 <tr valign="top">
  <th>{cb_msg module='grn.workflow' key='maximum_file_number' replace='true'}</th>
...
{include file='workflow/item/_detail_item_condition.tpl' required_item=true}
 <tr valign="top">
  <th>{cb_msg module='grn.workflow' key='required_field_2' replace='true'}</th>
  <td>{if $item.required == 1}{cb_msg module='grn.workflow' key='yes' replace='true'}{else}{cb_msg module='grn.workflow' key='no' replace='true'}{/if}</td>
 </tr>
 <tr valign="top">
  <th>{cb_msg module='grn.workflow' key='display' replace='true'}</th>
  <td>
      {if $item.settings.inline == 1}{cb_msg module='grn.workflow' key='show_image_with_text' replace='true'} {/if}
      {if $item.settings.inline == 1 && $item.settings.thumbnail == 1}<br>{/if}
      {if $item.settings.thumbnail == 1}{cb_msg module='grn.workflow' key='key_26_1' replace='true'}{$item.settings.thumbnail_xsize}{cb_msg module='grn.workflow' key='key_26_2' replace='true'}{$item.settings.thumbnail_ysize}{cb_msg module='grn.workflow' key='key_26_3' replace='true'}{/if}
  </td>
 </tr>
