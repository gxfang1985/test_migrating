...
{if $display == 'add-system' || $display == 'modify-system'}
<form name="{$form_name}" method="post" action="{grn_pageurl page='workflow/system/item/command_set_date'}" enctype="multipart/form-data">
    {else}
<form name="{$form_name}" method="post" action="{grn_pageurl page='workflow/item/command_set_date'}" enctype="multipart/form-data">
{/if}
...
<table class="std_form">
 ...
 <tr>
    <th nowrap>{capture name='grn_workflow_date_type'}{cb_msg module='grn.workflow' key='date_type' replace='true'}{/capture}{grn_show_input_title title=$smarty.capture.grn_workflow_date_type}</th>
        <td>
        {if $settings.date_type == 0}
        ...
        {/if}
        {capture name='grn_workflow_date'}{cb_msg module='grn.workflow' key='date' replace='true'}{/capture}{grn_radio name='date_type' id='date' value='0' caption=$smarty.capture.grn_workflow_date checked=$allow}&nbsp;
        {capture name='grn_workflow_date_time'}{cb_msg module='grn.workflow' key='date_time' replace='true'}{/capture}{grn_radio name='date_type' id='date_time' value='1' caption=$smarty.capture.grn_workflow_date_time checked=$deny}<br>
        </td>
    </tr>
 <tr>
  ...
  <td>
     ...
     {/if}
     <div id="div_date" style="{if $settings.date_type == 0}{else}display:none;{/if}">
         {capture name='grn_workflow_date_application_created'}{cb_msg module='grn.workflow' key='date_application_created' replace='true'}{/capture}
         {grn_radio name='initial_type' id='0' value='0' caption=$smarty.capture.grn_workflow_date_application_created checked=$initial_type_0}<br>
         {capture name='grn_workflow_specified_date'}{cb_msg module='grn.workflow' key='specified_date' replace='true'}{/capture}
         {grn_radio name='initial_type' id='1' value='1' caption=$smarty.capture.grn_workflow_specified_date checked=$initial_type_1}
         {grn_select_date prefix='initial_' date=$initial_date form_name=$form_name}<br>
         {capture name='grn_workflow_blank'}{cb_msg module='grn.workflow' key='blank' replace='true'}{/capture}
         {grn_radio name='initial_type' id='2' value='2' caption=$smarty.capture.grn_workflow_blank checked=$initial_type_2}
     </div>
     <div id="div_date_time" style="{if $settings.date_type == 0}display:none;{else}{/if}">
         {capture name='grn_workflow_date_application_created1'}{cb_msg module='grn.workflow' key='date_application_created1' replace='true'}{/capture}
         {grn_radio name='initial_type1' id='3' value='0' caption=$smarty.capture.grn_workflow_date_application_created1 checked=$initial_type_0}<br>
         {capture name='grn_workflow_specified_date1'}{cb_msg module='grn.workflow' key='specified_date1' replace='true'}{/capture}
         {grn_radio name='initial_type1' id='4' value='1' caption=$smarty.capture.grn_workflow_specified_date1 checked=$initial_type_1}
         {grn_select_date prefix='initial_1_' date=$initial_date form_name=$form_name}&nbsp;{grn_select_time prefix='initial_1_' time=$initial_time}<br>
         {capture name='grn_workflow_blank'}{cb_msg module='grn.workflow' key='blank' replace='true'}{/capture}
         {grn_radio name='initial_type1' id='5' value='2' caption=$smarty.capture.grn_workflow_blank checked=$initial_type_2}
     </div>
  </td>
 </tr>
 <tr valign="top">
 ...
 <!--<tr>
  <th nowrap>{capture name='grn_workflow_required_field_2'}{cb_msg module='grn.workflow' key='required_field_2' replace='true'}{/capture}{grn_show_input_title title=$smarty.capture.grn_workflow_required_field_2}</th>
  <td>
  </td>
 </tr>-->
 <tr valign="top">
...
<script>
    ...
    jQuery(document).ready(function(){
        var div_date = jQuery("#div_date");
        var div_date_time = jQuery("#div_date_time");
        jQuery("#date").click(function(){
            div_date.show();
            div_date_time.hide();
        });
        jQuery("#date_time").click(function(){
            div_date_time.show();
            div_date.hide();
        });
        var initial_hour = jQuery("#initial_1_hour");
        var initial_minute = jQuery("#initial_1_minute");
        initial_hour.change(function(){
            if (initial_hour.val() != '' && initial_minute.val() == '')
            {
                initial_minute.val(0);
            }
        });
    });
    {/literal}
