       ...
       <td colspan="2">
           ...
           {else}
               <table class="layout-border-none" width="100%">
                   <tr>
                       {grn_workflow_include_item category_id=$category_id form_id=$form_id iid=$item_line.iid item=$item_line display="view-confirm"}
                       {foreach from=$item_line.sub_item_list item=sub_item}
                           {grn_workflow_include_item category_id=$category_id form_id=$form_id iid=$sub_item.iid item=$sub_item display="view-confirm"}
                       {/foreach}
                   </tr>
               </table>
           {/if}
 ...
 {else}
  ...
  {if $item_line.type == 1}
    ...
    <tr valign="top">
     ...
     <td nowrap>{grn_user_name uid=$mandator.id name=$mandator.name users_info=$users_info}</td>
    </tr>
  {/if}
 {/if}
{/foreach}
...
<small>
{if !$simple}
{cb_msg module='grn.workflow' key='bracket_arrow_left' replace='true'}{capture name='grn_workflow_hide_processor'}{cb_msg module='grn.workflow' key='hide_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_proxy' caption=$smarty.capture.grn_workflow_hide_processor cid=$category_id fid=$form_id sf=1 simple=1 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}
{else}
{cb_msg module='grn.workflow' key='bracket_arrow_right' replace='true'}{capture name='grn_workflow_show_all_processor'}{cb_msg module='grn.workflow' key='show_all_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_proxy' caption=$smarty.capture.grn_workflow_show_all_processor cid=$category_id fid=$form_id sf=1 simple=0 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}
{/if}
</small>
...
{/if}
  ...
  <td align="center" rowspan="{$path_step_count|escape}" nowrap>
   <small>{cb_msg module='grn.workflow' key='bracket_arrow_right' replace='true'}{capture name='grn_workflow_show_all_processor'}{cb_msg module='grn.workflow' key='show_all_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_proxy' caption=$smarty.capture.grn_workflow_show_all_processor cid=$category_id fid=$form_id sf=1 simple=0 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}</small>
  </td>
...
{strip}
   <div class="mTop15 mBottom15">
       ...
       {capture name='grn_pageurl_command_send_confirm_proxy'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
       {grn_button id='workflow_button_back' spacing='tight' caption=$smarty.capture.grn_workflow_back_to_prev_screen onclick="document.forms['$form_name'].cmd.value='back'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_proxy`', this);"}
       {grn_button id='workflow_button_create_proxy_request' ui='main' spacing='loose' caption=$smarty.capture.grn_workflow_apply_per_pro onclick="document.forms['$form_name'].cmd.value='send'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_proxy`', this);"}
       {grn_button id='workflow_button_cancel' caption=$smarty.capture.grn_workflow_do_cancel onclick="document.forms['$form_name'].cmd.value='cancel'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_proxy`', this);"}
   </div>
{/strip}
...
<input type="hidden" name="mid" value="{$mandator.id|escape}">
