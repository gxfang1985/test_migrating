...
<form name="{$form_name}" method="post" action="{grn_pageurl page='workflow/operation/'|cat:$page_info.last}"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
...
<form name="{$form_name}/petition_delete_multi" method="post" action="{grn_pageurl page='workflow/operation/petition_delete_multi'}"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
...
{foreach from=$petition_list item=petition}
 ...
 {/if}
   ...
   <td nowrap>{grn_user_name uid=$petition.user name=$petition.user_name truncated=$name_width}</td>
   <td nowrap>
 {if $petition.status == GRN_WORKFLOW_STATUS_REJECTION}
  {grn_user_name uid=$petition.transactor_id name=$petition.transactor_name truncated=$name_width}
 {else}
  {foreach from=$petition.transactor item=user}
   {grn_user_name uid=$user._id name=$user.col_display_name truncated=$name_width valid=$user.col_valid}
  {/foreach}
