...
<form name="{$form_name}" method="post" action="{grn_pageurl page='workflow/operation/command_'|cat:$page_info.last}" enctype="multipart/form-data"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
<input type="hidden" name="cid" value="{$cid|escape}">
<input type="hidden" name="fid" value="{$fid|escape}">
<p class="explanation">{cb_msg module='grn.workflow.system' key='w_enter_private_route_info_y' replace='true'}</p>
...
<table class="std_form">
 ...
 <tr>
  <td></td>
  <td>
   <input class="margin" type="submit" name="add" value="{cb_msg module='grn.workflow.lang' key='form_path_add_w_do_set_y' replace='true'}">
