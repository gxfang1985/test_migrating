...
{if $file.modifiable}
 ...
 {else}
  {if $enable_lock}
   <input type="button" value="{cb_msg module='grn.workflow.system' key='w_update_files' replace='true'}" onclick="javascript:location.href='{grn_pageurl page=workflow/operation/file_lock cid=$cid rid=$rid mid=$mid rfid=$rfid frm=$frm}';">
  {else}
   <input type="button" value="{cb_msg module='grn.workflow.system' key='w_update_files' replace='true'}" onclick="javascript:location.href='{grn_pageurl page=workflow/operation/file_upload cid=$cid rid=$rid mid=$mid rfid=$rfid frm=$frm}';">
  {/if}
 {/if}
  <span class="menu_item">{capture name='grn_workflow_system_w_update_file_information'}{cb_msg module='grn.workflow.system' key='w_update_file_information' replace='true'}{/capture}{grn_link page='workflow/operation/file_modify' caption=$smarty.capture.grn_workflow_system_w_update_file_information image='modify20.gif' cid=$cid rid=$rid mid=$mid rfid=$rfid frm=$frm}</span>
{/if}
{if $file.deletable}
 {if $file.locked}
  <span class="menu_item">{capture name='grn_workflow_system_w_do_delete'}{cb_msg module='grn.workflow.system' key='w_do_delete' replace='true'}{/capture}{grn_link page='workflow/operation/file_delete' caption=$smarty.capture.grn_workflow_system_w_do_delete image='delete20.gif' cid=$cid rid=$rid mid=$mid rfid=$rfid disabled=true frm=$frm}</span>
 {else}
  <span class="menu_item">{capture name='grn_workflow_system_w_do_delete'}{cb_msg module='grn.workflow.system' key='w_do_delete' replace='true'}{/capture}{grn_link page='workflow/operation/file_delete' caption=$smarty.capture.grn_workflow_system_w_do_delete image='delete20.gif' cid=$cid rid=$rid mid=$mid rfid=$rfid frm=$frm}</span>
 {/if}
{/if}
...
<form name="unlock" method="post" action='{grn_pageurl page="workflow/operation/command_file_unlock"}'><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
...
<p><font color="green">{cb_msg module='grn.workflow.system' key='w_file_in_use' replace='true'}</font><input type="submit" value="{cb_msg module='grn.workflow.system' key='w_release_edit_lock_y' replace='true'}"></p>
 ...
 {/if}
{/if}
{include file="grn/file_view.tpl" download_page="workflow/operation/file_download" linkparams=$linkparams}
