...
<div id="main_menu_part">
    ...
    {if $is_nonparty || ! $is_manage}{assign var='disabled_flag' value=1}{/if}
    {if $disabled_flag || ! $is_move}{assign var='disabled_move_flag' value=1}{/if}
    <span class="menu_item">{capture name='grn_workflow_system_w_do_change_y'}{cb_msg module='grn.workflow.system' key='w_do_change_with_no_cancel_y' replace='true'}{/capture}{grn_link page='workflow/operation/category_modify' caption=$smarty.capture.grn_workflow_system_w_do_change_y image='modify20.gif' cid=$category.cid disabled=$disabled_flag}</span>
    <span class="menu_item">{capture name='grn_workflow_system_w_move'}{cb_msg module='grn.workflow.system' key='w_move' replace='true'}{/capture}{grn_link page='workflow/operation/category_move' caption=$smarty.capture.grn_workflow_system_w_move image='move20.gif' cid=$category.cid s_cid=$parent_category.cid disabled=$disabled_move_flag}</span>
    <span class="menu_item">{capture name='grn_workflow_system_w_do_delete'}{cb_msg module='grn.workflow.system' key='w_do_delete' replace='true'}{/capture}{grn_link page='workflow/operation/category_delete' caption=$smarty.capture.grn_workflow_system_w_do_delete image='delete20.gif' cid=$category.cid disabled=$disabled_flag}</span>
    <div id="rare_menu_part_under">
        {if $num_children <= 1 || ! $is_manage}{assign var='category_order_disabled' value=1}{/if}
        {if $num_form <= 1 || ! $is_manage}{assign var='form_order_disabled' value=1}{/if}
        <span class="menu_item">{capture name='grn_workflow_operation_w_changing_sub_category_order'}{cb_msg module='grn.workflow.operation' key='w_changing_sub_category_order' replace='true'}{/capture}{grn_link page='workflow/operation/category_order' caption=$smarty.capture.grn_workflow_operation_w_changing_sub_category_order image='order20.gif' pcid=$category.cid disabled=$category_order_disabled}</span>
        <span class="menu_item">{capture name='grn_workflow_operation_w_change_order_app_form_divider_y'}{cb_msg module='grn.workflow.operation' key='w_change_order_app_form_divider_y' replace='true'}{/capture}{grn_link page='workflow/operation/form_order' caption=$smarty.capture.grn_workflow_operation_w_change_order_app_form_divider_y image='order20.gif' cid=$category.cid disabled=$form_order_disabled}</span>
    </div>
</div>
<p>
