...
{cb_msg module='grn.workflow.system' key='w_select_category_for_access_authority' replace='true'}
</div>
...
{if $node_id}
  ...
  {if $is_manage}
  <span class="menu_item">{capture name='grn_workflow_operation_view_access_list'}{cb_msg module='grn.workflow.operation' key='view_access_list' replace='true'}{/capture}{grn_link image='modify20.gif' page="`$page_prefix`access_list" nid=$node_id reset=1 caption=$smarty.capture.grn_workflow_operation_view_access_list}</span>
  {else}
  <span class="menu_item">{capture name='grn_workflow_operation_view_access_list'}{cb_msg module='grn.workflow.operation' key='view_access_list' replace='true'}{/capture}{grn_link image='modify20.gif' page="`$page_prefix`access_list" nid=$node_id reset=1 caption=$smarty.capture.grn_workflow_operation_view_access_list disabled=true}</span>
  {/if}
  </div>

  <p>
  <div>
  <span class="sub_title">{cb_msg module='grn.workflow.operation' key='w_status_for_access_authority_setting' replace='true'}</span>
  </div>
  <table class="view_table">
   ...
   <tr>
    ...
    <td nowrap>{grn_image image='access20.gif'}{if $access.security_model == 'revoke'}{cb_msg module='grn.workflow.operation' key='c_limit' replace='true'}{else}{cb_msg module='grn.workflow.operation' key='c_accept' replace='true'}{/if}</td>
   </tr>
   <tr>
    <th nowrap>{cb_msg module='grn.workflow.operation' key='w_number_for_access_authority_setting' replace='true'}</th>
