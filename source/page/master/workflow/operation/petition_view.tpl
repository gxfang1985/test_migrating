...
<div id="main_menu_part">
    <span class="menu_item">{capture name='grn_workflow_operation_petition_print'}{cb_msg module='grn.workflow.operation' key='petition_print' replace='true'}{/capture}{grn_link caption=$smarty.capture.grn_workflow_operation_petition_print page='workflow/operation/petition_print' image='print20.gif' cid=$category_id fid=$form_id pid=$petition_id simple=$simple}</span>
    <span class="menu_item">{capture name='grn_workflow_system_w_do_delete'}{cb_msg module='grn.workflow.system' key='w_do_delete' replace='true'}{/capture}{grn_link caption=$smarty.capture.grn_workflow_system_w_do_delete page='workflow/operation/petition_delete' image='delete20.gif' cid=$category_id fid=$form_id pid=$petition_id}</span>
</div>
<!--menubar end-->
    ...
    <div class="workflow_attention">
        ...
        <table>
            ...
            <tr>
                ...
                <td colspan="3">{$petition.comment|escape}</td>
            </tr>
        </table>
    </div>
    <p></p>
...
<div class="marginHalf">
    ...
    <font size="+1"><span class="bold">{$petition.form_name}{cb_msg module='grn.workflow.system' key='w_bracket_left' replace='true'}{$petition.name|escape}{cb_msg module='grn.workflow.system' key='w_bracket_right' replace='true'}</span></font>
</div>
        ...
        {foreach from=$item_data_list item=item_data_line}
            ...
            {else}
                <tr valign="top">
                    ...
                    {/if}
                    {if $item_data_line.sub_item_data_count == 0}
                        {grn_workflow_include_item category_id=$category_id folder_id=$foler_id petition_id=$petition_id iid=$item_data_line.iid item=$item_data_line display="view-operation"}
                    {else}
                        <td>
                        ...
                        {/if}
                                <tr>
                                    {grn_workflow_include_item category_id=$category_id folder_id=$foler_id petition_id=$petition_id iid=$item_data_line.iid item=$item_data_line display="view-operation"}
                                    {foreach from=$item_data_line.sub_item_data_list item=sub_item_data}
                                        {grn_workflow_include_item category_id=$category_id folder_id=$foler_id petition_id=$petition_id iid=$sub_item_data.iid item=$sub_item_data display="view-operation"}
                                    {/foreach}
                                </tr>
                            </table>
                        </td>
...
<!-- ここまで　項目セクション -->
    ...
    {if $simple}
        ...
        <span class="menu_item">
            <small>{cb_msg module='grn.workflow.operation' key='w_bracket_arrow_right' replace='true'}{capture name='grn_workflow_system_w_show_all_progress_state_y'}{cb_msg module='grn.workflow.system' key='w_show_all_progress_state_y' replace='true'}{/capture}{grn_link page='workflow/operation/petition_view' caption=$smarty.capture.grn_workflow_system_w_show_all_progress_state_y cid=$category_id fid=$form_id pid=$petition_id simple=0}{cb_msg module='grn.workflow.system' key='w_bracket_right' replace='true'}</small>
        </span>
        {if GRN_WORKFLOW_FOLDER_TYPE_TEMP_ID != $folder.col_folder_type}
            <div id="main_menu_part">
                <span class="menu_item">
                    {capture name='grn_workflow_operation_petition_change_path_history_list'}{cb_msg module='grn.workflow.operation' key='petition_change_path_history_list' replace='true'}{/capture}{grn_link caption=$smarty.capture.grn_workflow_operation_petition_change_path_history_list page='workflow/operation/petition_change_path_history_list' image='view20.gif' cid=$category_id fid=$form_id pid=$petition_id disabled=$no_change_path_history}
                ...
                {foreach name=path_step from=$path_steps item=path_step}
                    {if $path_step.col_activate}
                        <tr>
                            ...
                            {/if}
                            {* 経路ステップ名 *}
                            <td>{grn_link caption=$path_step.col_role page='workflow/operation/petition_path_step_view' cid=$category_id fid=$form_id pid=$petition_id psid=$path_step._id}</td>
                            {* 処理者/結果/コメント/日時 *}
                            {if $not_first}
                    ...
                    {if $path_step.col_activate}
                            ...
                            {foreach name=user from=$path_step.users item=user}
                                <tr>
                                    {if $not_first}
                                        ...
                                        {/if}
                                        {* 経路ステップ名 *}
                                        <td rowspan="{$path_step.user_count|escape}">{grn_link caption=$path_step.col_role page='workflow/operation/petition_path_step_view' cid=$category_id fid=$form_id pid=$petition_id psid=$path_step._id br=true}</td>
                                    {/if}
                                    {* 結果 *}
                        ...
                        {else}
                            ...
                            <tr>
                                ...
                                {* 経路ステップ名 *}
                                <td>{grn_link caption=$path_step.col_role page='workflow/operation/petition_path_step_view' cid=$category_id fid=$form_id pid=$petition_id psid=$path_step._id disabled=$path_step.not_modify br=true}</td>
                                {* 結果 *}
