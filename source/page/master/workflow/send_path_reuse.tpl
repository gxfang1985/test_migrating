...
{foreach from=$path_step_list key=psid item=path_step}
 {capture name=validate_require}{$path_step.col_role|grn_noescape}{cb_msg module='grn.workflow' key='required_field' replace='true'}{/capture}
 {validate form=$form_name field="path_step_`$path_step._id`_sUID" criteria="notEmpty" message=$smarty.capture.validate_require append="validation_errors"}
{/foreach}
...
access_plugin=$plugin}
 ...
 <div class="mTop15 mBottom15">
  ...
  {capture name='grn_pageurl_command_send_path_reuse'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
  {strip}
   {grn_button id='workflow_button_back' auto_disable=false spacing='tight' caption=$smarty.capture.grn_workflow_back_to_prev_screen
   onclick="document.forms['$form_name'].cmd.value='back'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_path_reuse`', this);"}
   {grn_button id='workflow_button_confirm' auto_disable=false ui='main' spacing='loose' caption=$smarty.capture.grn_workflow_confirm_contents_2
   onclick="document.forms['$form_name'].cmd.value='send'; grn.component.member_add.get_instance('route_member_add').prepareSubmit(); grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_path_reuse`', this);"}
   {grn_button id='workflow_button_cancel' action='cancel' onclick="document.forms['$form_name'].cmd.value='cancel'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_path_reuse`', this);"}
  {/strip}
 </div>
