...
{foreach from=$path_step_list key=psid item=path_step}
 ...
 {validate form=$form_name field="path_step_`$path_step.col_path_step`_sUID" criteria="notEmpty" message=$smarty.capture.validate_require append="validation_errors"}
{/foreach}
...
{grn_workflow_member_add
    ...
    include_org=$show_organize access_plugin=$plugin}
...
{strip}
 <div class="mTop15 mBottom15">
  ...
  {capture name='grn_pageurl_command_send_path_draft'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
   {grn_button id='workflow_button_back' spacing='tight' caption=$smarty.capture.grn_workflow_back_to_prev_screen onclick="document.forms['$form_name'].cmd.value='back'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_path_draft`', this);"}
   {grn_button id='workflow_button_confirm' ui='main' spacing='loose' caption=$smarty.capture.grn_workflow_confirm_contents_2 onclick="document.forms['$form_name'].cmd.value='send'; grn.component.member_add.get_instance('route_member_add').prepareSubmit(); grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_path_draft`', this);"}
   {grn_button id='workflow_button_cancel' caption=$smarty.capture.grn_workflow_do_cancel onclick="document.forms['$form_name'].cmd.value='cancel'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_path_draft`', this);"}
  </div>
{/strip}
