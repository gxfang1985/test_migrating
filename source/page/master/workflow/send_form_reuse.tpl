 ...
 <tr>
  ...
  <td>
   ...
   <div class="mTop10">
    ...
    {capture name='grn_pageurl_command_send_form_reuse'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
    {capture name='grn_workflow_set_route_onclick'}
        ...
        if((typeof(workflow_file_validate)=='function') && (!confirmIfExistFailedFile() || !workflow_file_validate(false)))
            return false;
        else grn.page.workflow.ajax_submit.workflow_submit('{$form_name}', '{$smarty.capture.grn_pageurl_command_send_form_reuse}', this);
    {/capture}
    ...
    {grn_button id='workflow_button_cancel' class="button_submit_grn" action='cancel' onclick="if(grn.component.button('#workflow_button_cancel').isDisabled())return false; document.forms['$form_name'].cmd.value='cancel'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_form_reuse`', this);"}
   </div>
   {/strip}
  </td>
 </tr>
