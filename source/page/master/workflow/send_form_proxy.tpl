...
{foreach from=$item_list item=item_line}
 ...
 <tr valign="top">
  ...
  <td nowrap>{grn_user_name uid=$mandator.id name=$mandator.name}</td>
 </tr>
  {/if}
 {/if}
{/foreach}
<!-- ここまで　項目セクション -->
 <tr>
  ...
  <td>
   {strip}
   <div class="mTop10">
    {capture name='grn_pageurl_command_send_form_proxy'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
    ...
    {capture name='grn_workflow_set_route_onclick'}
        ...
        if((typeof(workflow_file_validate)=='function') && (!confirmIfExistFailedFile() || !workflow_file_validate(false)))
            return false;
        else grn.page.workflow.ajax_submit.workflow_submit('{$form_name}', '{$smarty.capture.grn_pageurl_command_send_form_proxy}', this);
    {/capture}
