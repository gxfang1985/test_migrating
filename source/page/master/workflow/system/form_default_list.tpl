...
<td width="60%">
 ...
 <form name="{$form_name}" method="post" action="{grn_pageurl page="workflow/system/form_default_delete_multi"}"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
...
{/if}
  {capture name='grn_workflow_system_form_default_skip_set'}{cb_msg module='grn.workflow.system' key='default_skip_set_link' replace='true'}{/capture}{grn_link image='nouser20.gif' page="workflow/system/form_default_skip_set" params=$ours_params poid=$poid cid=$cid fid=$fid psid=$psid caption=$smarty.capture.grn_workflow_system_form_default_skip_set disabled=$col_skip}</span>
  <span class="m_small">{capture name='grn_workflow_system_GRN_WKF_SYS_91'}{cb_msg module='grn.workflow.system' key='GRN_WKF_SYS-91' replace='true'}{/capture}{grn_link image='modify20.gif' page="workflow/system/form_default_add" params=$ours_params reset=1 caption=$smarty.capture.grn_workflow_system_GRN_WKF_SYS_91 }</span>
  <span class="m_small">{capture name='grn_workflow_system_w_delete_all'}{cb_msg module='grn.workflow.system' key='w_delete_all' replace='true'}{/capture}{grn_link image='delete20.gif' page="workflow/system/form_default_delete_all" params=$ours_params poid=$poid cid=$cid fid=$fid psid=$psid caption=$smarty.capture.grn_workflow_system_w_delete_all disabled=$disabled}</span>
 </div>
 <div>
