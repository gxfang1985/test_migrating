...
<p class="explanation">{cb_msg module='grn.workflow.system' key='w_select_processor_y' replace='true'}<br />{cb_msg module='grn.workflow.system' key='w_comment_displayed_in_pathway_change_history' replace='true'}</p>
...
{if GRN_WORKFLOW_RESULT_UNPROCESSING < $user.col_result}
   <span class="voice">{grn_user_name uid=$user.col_user name=$user.col_display_name  valid=$user.col_valid}</span><br>
{/if}
{/foreach}
 ...
 <tr>
  ...
  <td colspan="3">{grn_user_name uid=$login.id name=$login.name}</td>
 </tr>
 <tr valign="top">
 ...
 <tr>
  ...
  <td>
   ...
   {grn_button_cancel page='workflow/system/petition_path_step_view' cid=$category_id fid=$form_id pid=$petition_id psid=$path_step_id}
  </td>
 </tr>
</table>
