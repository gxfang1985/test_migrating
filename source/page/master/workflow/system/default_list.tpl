...
{cb_msg module='grn.workflow.system' key='GRN_WKF_SYS-93' replace='true'}
<br />
...
<br />
{cb_msg module='grn.workflow.system' key='GRN_WKF_SYS-94' replace='true'}
<br />
</p>
<table width="100%">
<tr valign="top">
<td width="60%">
 <form name="{$form_name}" method="post" action="{grn_pageurl page="workflow/system/default_delete_multi"}"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
...
<span class="sub_title">{cb_msg module='grn.workflow.system' key='form_default_change_path' replace='true'}</span> 
<div id="main_menu_part">
 <span class="menu_item">{capture name='grn_workflow_system_w_do_change_y'}{cb_msg module='grn.workflow.system' key='w_do_change_with_no_cancel_y' replace='true'}{/capture}{grn_link image='modify20.gif' page="workflow/system/default_change_path" params=$ours_params pid=$pid psid=$psid poid=$poid caption=$smarty.capture.grn_workflow_system_w_do_change_y}</span>
</div>
...
{/if}
  {capture name='grn_workflow_system_form_default_skip_set'}{cb_msg module='grn.workflow.system' key='default_skip_set_link' replace='true'}{/capture}{grn_link image='nouser20.gif' page="workflow/system/default_skip_set" params=$ours_params pid=$pid psid=$psid poid=$poid caption=$smarty.capture.grn_workflow_system_form_default_skip_set disabled=$col_skip}</span>
  <span class="m_small">{capture name='grn_workflow_system_GRN_WKF_SYS_91'}{cb_msg module='grn.workflow.system' key='GRN_WKF_SYS-91' replace='true'}{/capture}{grn_link image='modify20.gif' page="workflow/system/default_add" params=$ours_params reset=1 caption=$smarty.capture.grn_workflow_system_GRN_WKF_SYS_91 }</span>
  <span class="m_small">{capture name='grn_workflow_system_w_delete_all'}{cb_msg module='grn.workflow.system' key='w_delete_all' replace='true'}{/capture}{grn_link image='delete20.gif' page="workflow/system/default_delete_all" params=$ours_params pid=$pid psid=$psid poid=$poid caption=$smarty.capture.grn_workflow_system_w_delete_all disabled=$disabled}</span>
 </div>
 <div>
...
{if 'static_role' == $member.type}
 ...
 <td  class="m_small">{capture name='grn_workflow_system_w_change_y'}{cb_msg module='grn.workflow.system' key='w_change_y' replace='true'}{/capture}{grn_link image='modify20.gif' page="workflow/system/default_chief_set" params=$ours_params pid=$pid psid=$psid poid=$poid mid=$member._id caption=$smarty.capture.grn_workflow_system_w_change_y}</td>
{else}
