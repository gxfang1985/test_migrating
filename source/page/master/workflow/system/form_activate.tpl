...
<form name="{$form_name}" method="post" action="{grn_pageurl page='workflow/system/command_'|cat:$page_info.last}"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
 <p>{cb_msg module='grn.workflow.system' key='key_137_1' replace='true'}<span class="bold">{grn_workflow_form_icon_image icon_type=$form.icon_type icon_id=$form.icon_id icon_url=$form.icon_url}{grn_sentence caption=$form.name}</span>{cb_msg module='grn.workflow.system' key='key_137_2' replace='true'}{if $form.active == 1}{cb_msg module='grn.workflow.system' key='key_137_inactive' replace='true'}{else}{cb_msg module='grn.workflow.system' key='key_137_active' replace='true'}{/if}{cb_msg module='grn.workflow.system' key='key_137_3' replace='true'}</p>
 <p>
  {capture name='grn_workflow_system_w_yes'}{cb_msg module='grn.workflow.system' key='w_yes' replace='true'}{/capture}{grn_button_submit class="margin" caption=$smarty.capture.grn_workflow_system_w_yes id="`$page_info.last`_submit"}
