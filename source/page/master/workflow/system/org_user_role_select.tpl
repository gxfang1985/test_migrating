...
<table width="100%" cellpadding="5" cellspacing="0" style="border-collapse: collapse;" bgcolor="#ffffff">
 ...
 <div class="sub_title">{cb_msg module='grn.grn' key='GRN_GRN-997' replace='true'} {include file="grn/smart_data_count.tpl" start_num=$navi_info.start_count end_num=$navi_info.end_count all_num=$navi_info.count}</div>
    ...
    <table style="width:50%">
     ...
     <td></td>
     <td>
     <div class="sub_title">{cb_msg module='grn.workflow.system' key='GRN_WKF_SYS-55' replace='true'}</div>

     <table class="admin_list_table" style="width:60%">
     <tr>
     <th style="text-align:center" nowrap>{cb_msg module='grn.workflow.system' key='GRN_WKF_SYS-56' replace='true'}</th>
     <th style="text-align:center" nowrap>{cb_msg module='grn.workflow.system' key='GRN_WKF_SYS-57' replace='true'}</th>
     </tr>
     <tr>
     <td align="center">{grn_checkbox name="col_authority_workflow" id="workflow" value='1' checked=$col_authority_workflow onclick="change_status(this, 'cellular');" disabled=$is_over_user}</td>
