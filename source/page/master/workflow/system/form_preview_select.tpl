...
{if $org_id}
{capture name='grn_workflow_system_w_top'}{cb_msg module='grn.workflow.system' key='w_top' replace='true'}{/capture}{grn_link page='workflow/system/form_preview_select' top=1 caption=$smarty.capture.grn_workflow_system_w_top cid=$category_id fid=$form_id}
{else}
{capture name='grn_workflow_system_w_top'}{cb_msg module='grn.workflow.system' key='w_top' replace='true'}{/capture}{grn_link page='workflow/system/form_preview_select' top=1 caption=$smarty.capture.grn_workflow_system_w_top class='tree-select-current' cid=$category_id fid=$form_id}
{/if}
  <div id="org_tree"></div>
  </div>
...
{if ! $is_root}
 ...
 {foreach from=$user_list key=key item=item}
   <tr>
    <td>{grn_link image='blank20.gif' page="workflow/system/form_preview" cid=$category_id fid=$form_id oid=$org_id uid=$key caption=$item.display_name target='_blank'}</td>
   </tr>
 {/foreach}
  </table>
  <div class="contents_navi">
...
var linkURL = '{grn_pageurl page='workflow/system/form_preview_select' cid=$category_id fid=$form_id}';
