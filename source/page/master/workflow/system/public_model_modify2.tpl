...
{cb_msg module='grn.workflow.system' key='key_163_1' replace='true'}<span class="bold">{grn_image image='category20.gif'}{$node.name|escape}</span>{cb_msg module='grn.workflow.system' key='key_163_2' replace='true'}
{cb_msg module='grn.workflow.system' key='key_163_3' replace='true'}<span class="bold">{if $security_model eq 'grant'}{$smarty.capture.grant_caption|grn_noescape}{elseif  $security_model eq 'revoke'}{$smarty.capture.revoke_caption|grn_noescape}{/if}</span>{cb_msg module='grn.workflow.system' key='key_163_4' replace='true'}

<div class="attention">{cb_msg module='grn.workflow.system' key='key_164_1' replace='true'}<span class="bold">{grn_sentence image='category20.gif' caption=$node.name}</span>{cb_msg module='grn.workflow.system' key='key_164_2' replace='true'}</div>
