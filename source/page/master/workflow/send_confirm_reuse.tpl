...
<small>
{if !$simple}
{cb_msg module='grn.workflow' key='bracket_arrow_left' replace='true'}{capture name='grn_workflow_hide_processor'}{cb_msg module='grn.workflow' key='hide_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_reuse' caption=$smarty.capture.grn_workflow_hide_processor fid=$folder_id pid=$petition_id sf=1 simple=1 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}
{else}
{cb_msg module='grn.workflow' key='bracket_arrow_right' replace='true'}{capture name='grn_workflow_show_all_processor'}{cb_msg module='grn.workflow' key='show_all_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_reuse' caption=$smarty.capture.grn_workflow_show_all_processor fid=$folder_id pid=$petition_id sf=1 simple=0 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}
{/if}
</small>
...
{/if}
  ...
  <td align="center" rowspan="{$path_step_count|escape}" nowrap>
   <small>{cb_msg module='grn.workflow' key='bracket_arrow_right' replace='true'}{capture name='grn_workflow_show_all_processor'}{cb_msg module='grn.workflow' key='show_all_processor' replace='true'}{/capture}{grn_link page='workflow/send_confirm_reuse' caption=$smarty.capture.grn_workflow_show_all_processor fid=$folder_id pid=$petition_id sf=1 simple=0 tmp_key=$tmp_key}{cb_msg module='grn.workflow' key='closed_parenthesis' replace='true'}</small>
  </td>
...
{strip}
  <div class="mTop15  mBottom15">
      ...
      {capture name='grn_pageurl_command_send_confirm_reuse'}{grn_pageurl page=workflow/command_`$page_info.last`}{/capture}
          {grn_button id='workflow_button_back' spacing='tight' caption=$smarty.capture.grn_workflow_back_to_prev_screen onclick="document.forms['$form_name'].cmd.value='back'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_reuse`', this);"}
          {grn_button id='workflow_button_create' ui='main' spacing='loose' caption=$smarty.capture.grn_workflow_apply onclick="document.forms['$form_name'].cmd.value='send'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_reuse`', this);"}
          {grn_button id='workflow_button_cancel' action='cancel' onclick="document.forms['$form_name'].cmd.value='cancel'; grn.page.workflow.ajax_submit.workflow_submit('$form_name', '`$smarty.capture.grn_pageurl_command_send_confirm_reuse`', this);"}
  </div>
{/strip}
