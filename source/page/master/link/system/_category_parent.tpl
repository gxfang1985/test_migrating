...
{if $move_link}
 <tr>
  <th nowrap>{cb_msg module='grn.link.system' key='GRN_LNK_SY-185' replace='true'}</th>
  <td>{grn_image image='category20.gif'}{$parent.path|escape}</td>
 </tr>
 <tr valign="top">
  <th nowrap>{cb_msg module='grn.link.system' key='GRN_LNK_SY-186' replace='true'}</th>
{elseif $target}
 <tr>
  <th nowrap>{cb_msg module='grn.link.system' key='GRN_LNK_SY-187' replace='true'}</th>
  <td>{grn_image image='category20.gif'}{$target.parent.path|escape}</td>
 </tr>
 <tr valign="top">
  <th nowrap>{cb_msg module='grn.link.system' key='GRN_LNK_SY-188' replace='true'}</th>
{/if}
  <td width="80%">
...
<input type="text" name="text" value="{$text|escape}" size="20"><input type="submit" name="search" value="{cb_msg module='grn.link.system' key='GRN_LNK_SY-189' replace='true'}">
</div>
...
{/foreach}
<p>
<a href="{grn_pageurl page=$page_info.all cid=$category_id s_cid=$set_category_id ss=1 params=$params}">[{cb_msg module='grn.link.system' key='GRN_LNK_SY-190' replace='true'}]</a>
</div>
...
<small>
{capture name='grn_link_system_GRN_LNK_SY_191'}{cb_msg module='grn.link.system' key='GRN_LNK_SY-191' replace='true'}{/capture}{grn_link image='category20.gif' page=$page_info.all cid=$category_id s_cid=$category.parent.cid ss=1 params=$params caption=$smarty.capture.grn_link_system_GRN_LNK_SY_191}
</small>
</div>
{/if}
...
{else}
<div class="m_small">
{grn_image image='category20.gif'}{cb_msg module='grn.link.system' key='GRN_LNK_SY-192' replace='true'}
</div>
<hr size="1">
{/if}
...
<div class="sub_title">{cb_msg module='grn.link.system' key='GRN_LNK_SY-193' replace='true'}</div>
<small>
...
{capture name='grn_link_system_GRN_LNK_SY_194'}{cb_msg module='grn.link.system' key='GRN_LNK_SY-194' replace='true'}{/capture}{grn_button_submit class='margin' caption=$smarty.capture.grn_link_system_GRN_LNK_SY_194 disabled=1}
{else}
{capture name='grn_link_system_GRN_LNK_SY_195'}{cb_msg module='grn.link.system' key='GRN_LNK_SY-195' replace='true'}{/capture}{grn_button_submit class='margin' caption=$smarty.capture.grn_link_system_GRN_LNK_SY_195}
{/if}
{if $move_link}
{grn_button_cancel page=$page_prefix|cat:'/view' cid=$category_id lid=$link_id}
{else}
