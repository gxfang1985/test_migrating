    ...
    <table class="std_form">
        <tr>
            ...
            <td>
                ...
                {else}
                    {cb_msg module='grn.address' key='GRN_ADDR-101' replace='true'}{$resources.application_name|escape}
                {/if}
            </td>
        </tr>
        <!--bulitin_items-->
            ...
            {if $item_id != 'route_time' && $item_id != 'route_fare' && $item_id != 'route_detail'}
                {if $item.use}
                    ...
                    {if $item_id == 'personal_name'}
                        {capture name='grn_address_GRN_ADDR_102'}{cb_msg module='grn.address' key='GRN_ADDR-102' replace='true'}{/capture}{assign var='display_name' value=$smarty.capture.grn_address_GRN_ADDR_102}
                        {if ! ( $not_modify && $not_get )}
                            {capture name='front'}{capture name='grn_address_GRN_ADDR_103'}{cb_msg module='grn.address' key='GRN_ADDR-254' replace='true'}{/capture}{capture name='grn_address_GRN_ADDR_104'}{cb_msg module='grn.address' key='GRN_ADDR-255' replace='true'}{/capture}{include file='address/_action_item_contents_name.tpl' family_id='address_family_name' family_key='family_name' family_validate=$smarty.capture.grn_address_GRN_ADDR_103 given_id='address_given_name' given_key='given_name' given_validate=$smarty.capture.grn_address_GRN_ADDR_104}{/capture}
                        {/if}
                    {elseif $item_id == 'personal_sort_key'}
                        {capture name='grn_address_GRN_ADDR_105'}{cb_msg module='grn.address' key='GRN_ADDR-105' replace='true'}{/capture}{assign var='display_name' value=$smarty.capture.grn_address_GRN_ADDR_105}
                        {if ! ( $not_modify && $not_get )}
                            {capture name='front'}{capture name='grn_address_GRN_ADDR_106'}{cb_msg module='grn.address' key='GRN_ADDR-256' replace='true'}{/capture}{capture name='grn_address_GRN_ADDR_107'}{cb_msg module='grn.address' key='GRN_ADDR-257' replace='true'}{/capture}{include file='address/_action_item_contents_name.tpl' family_id='address_family_sort_key' family_key='family_sort_key' family_validate=$smarty.capture.grn_address_GRN_ADDR_106 given_id='address_given_sort_key' given_key='given_sort_key' given_validate=$smarty.capture.grn_address_GRN_ADDR_107}{/capture}
                        {/if}
                    {else}
                        {if ! ( $not_modify && $not_get )}
        ...
        <tr>
            ...
            <td>
                <div class="mTop10">
                    {strip}
                        {capture name="grn_address_GRN_ADDR_108"}{cb_msg module='grn.address' key='GRN_ADDR-108' replace='true'}{/capture}
                        {grn_button id='address_book_button_add' ui='main' action='submit' spacing='normal' caption=$smarty.capture.grn_address_GRN_ADDR_108}
                        {if $page}
                            {grn_button id='address_book_button_cancel' action='cancel' page=$page page_param_mid=$url_params.mid}
                        {else}
                            {grn_button id='address_book_button_cancel' action='cancel' page='address/index' page_param_bid=$book_id}
                        {/if}
                    {/strip}
                </div>
            </td>
