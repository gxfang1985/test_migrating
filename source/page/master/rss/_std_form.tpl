...
{foreach from=$form.items key=key item=item}
 ...
 {/if}
  ...
  <td>
 {if $item.necessary}
  {capture name='grn_rss_GRN_RSS_1'}{cb_msg module='grn.rss' key='GRN_RSS-1' replace='true'}{/capture}{validate form=$form_name field=$key criteria="notEmpty" message=$item.display_name|cat:$smarty.capture.grn_rss_GRN_RSS_1 transform="cb_trim" append="validation_errors"}
 {/if}
 {if $item.type == 'string'}
 ...
 {/if}
 {if $code_explanation}
  {if $item.display_name eq $code_title}
   <div class="margin_top">
    <span class="sub_explanation">{$code_explanation|escape}</span>
   </div>
  {/if}
 {/if}
  </td>
...
{foreach from=$form.buttons key=key item=item}
 ...
 {elseif $item.type == 'cancel'}
  ...
  {elseif $item.key}
   {grn_button_cancel class='margin' page=$item.page key=$item.key}
  {else}
