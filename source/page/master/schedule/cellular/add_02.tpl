...
{capture name=validate_schedule_title_emoji}
    {cb_msg module='grn.schedule.cellular' key="txt_sche_title"}:
    {cb_msg module='grn.cellular.common' key="validate_message_emoji"}<BR>
{/capture}

{capture name=validate_schedule_memo_emoji}
    {cb_msg module='grn.schedule.cellular' key="txt_sche_info"}:
    {cb_msg module='grn.cellular.common' key="validate_message_emoji"}<BR>
{/capture}
...
{cb_msg module='grn.schedule.cellular' key="title_add_02"}<br>
<hr>
...
{grn_cellular_form name=$form_name method=post page="$pagepath/command_add_02" mid="$mid"}
{if $schedata.banner_flag}<input type=hidden name="BN" value=banner>{/if}

{validate form=$form_name field="title" criteria="grn_cellular_notEmoji" message=$smarty.capture.validate_schedule_title_emoji}
{validate form=$form_name field="info" criteria="grn_cellular_notEmoji" message=$smarty.capture.validate_schedule_memo_emoji}
...
<textarea name=info cols=16 rows=3 istyle="1" mode="hiragana">{$schedata.info|escape}</textarea><br>
...
{foreach from=$extended_items key=item_id item=item}
    {if $item.use}
        {cb_msg module='grn.schedule.cellular' key='GRN_SCH_CE-17' replace='true'}{grn_show_input_title title=$item.display_name}<br>
...
{if $schedata.joint_edit}
    ...
    <br>
{else}
    <BR>{cb_msg module='grn.schedule.cellular' key="no_editable_joint"}<BR>
{/if}
...
{grn_cellular_submit accesskey=1 caption_module='grn.schedule.cellular' caption_key="link_add_02_submit"}
</form>

<br>
{grn_cellular_link page="$pagepath/add_01" re=1 day=$day mid=$mid accesskey=2 caption_module='grn.schedule.cellular' caption_key="link_to_back"}<br>
