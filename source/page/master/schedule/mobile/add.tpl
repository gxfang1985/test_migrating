...
<script type="text/javascript" language="javascript">
    var __thisPage = grn.page.schedule.mobile.schedule_add;
    __thisPage.msgRepeat = '<p style="word-break: break-word;">{cb_msg module='grn.schedule' key='GRN_SCH-862' replace='true'}<br><br>{cb_msg module='grn.schedule' key='GRN_SCH-863' replace='true'}{cb_msg module='grn.schedule' key='GRN_SCH-864' replace='true'}';
    __thisPage.titleRepeat = '{$before_delete.title}';
    __thisPage.yes = '{cb_msg module='grn.common' key='yes' replace='true'}';
    __thisPage.no = '{cb_msg module='grn.common' key='no' replace='true'}';
</script>

{assign var="form_name" value=$smarty.template|basename}
<form id="schedule_mobile_add" name="{$form_name}" method="post" enctype="multipart/form-data" action="{grn_pageurl page='schedule/mobile/command_add'}" data-ajax="false">
...
<div data-role="content" class="mobile-content-grn" data-theme="{$data_theme}">
  ...
  {/if}
  {include file="schedule/mobile/_add_button.tpl" form_id='schedule_mobile_add'}
...
{capture name='default_public_select_page'}{cb_msg module='grn.schedule.mobile' key='GRN_SCH_MOBILE-43' replace='true'}{/capture}
{grn_mobile_user_add
  form_name=$form_name
  sUID='p_sUID'
