...
<script type="text/javascript" language="javascript">
    ...
    handleAdd.ajaxURL = '{grn_pageurl page='schedule/mobile/command_modify'}';
    handleAdd.defaultBackURL = '{grn_pageurl page='schedule/mobile/index'}';
    handleAdd.formId = '{$form_id}';
    {if $page_info.last == 'repeat_modify'}
        handleAdd.conflict = '{cb_msg module='grn.schedule.personal' key='GRN_SCH_PE-71' replace='true'}';
        handleAdd.onlyNoConflict = '{cb_msg module='grn.schedule.personal' key='GRN_SCH_PE-83' replace='true'}';
    ...
    {/if}
    handleAdd.typeSchedule = '{$page_info.last}';
    {if $use_attendance_check && $schedule_event.attendance_check}
        handleAdd.needCheckShowRefreshDialog = true;
    {/if}
</script>
...
<div data-theme="{$data_theme}" class="mobile-buttonArea-grn">
    <div class="mobile_ok_grn">
      <input id="addBtn" type="button" value="{cb_msg module='grn.schedule' key='GRN_SCH-174' replace='true'}" data-inline="true" data-theme="{$data_theme}"/>
    </div>
    <div class="mobile_cancel_grn mobile_show_overlay_js">
      <input id="cancelBtn" class="cancel_button" type="reset" value="{cb_msg module='grn.schedule.mobile' key='GRN_SCH_MOBILE-17' replace='true'}" data-inline="true" data-theme="{$data_theme}" onclick="javascript:location.href='{grn_pageurl page=$breadcrumb_upper_page.page params=$breadcrumb_upper_page.params}'"/>
    </div>
</div>