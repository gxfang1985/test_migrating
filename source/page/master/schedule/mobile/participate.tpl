...
<div data-role="content" data-theme="{$data_theme}" class="mobile-content-grn">
    ...
    <!--end breadcrumb-->
    <form method="post" name="{$form_name}" action="{grn_pageurl page='schedule/mobile/command_'|cat:$page_info.last}"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
        <div class="mobile_delete_confirm_grn mobile_schedule_delete_confirm_grn">
            <span class="mobile_icon_attention_grn"></span>
            <span class="mobile_text_grn">{cb_msg module='grn.schedule' key='GRN_SCH-175' replace='true'}</span>
