...
(function()
{
    ...
    G.ERRMSG_PARTICIPANT_SELECT_IS_REQUIRED = '{/literal}{cb_msg module='grn.schedule' key='GRN_SCH-157' replace='true'}{literal}';
    G.ERRMSG_SCOPE_SELECT_IS_REQUIRED = '{/literal}{cb_msg module='grn.schedule' key='GRN_SCH-161' replace='true'}{literal}';
    G.ERRMSG_DATETIME_CANDIDATE_IS_REQUIRED = '{/literal}{cb_msg module='grn.schedule' key='GRN_SCH-167' replace='true'}{literal}';
})();
</script>
{/literal}
<div data-role="content" data-theme="{$data_theme}" class="mobile-content-grn">
    ...
    <form method="post" name="{$form_name}" action="{cb_pageurl page='schedule/mobile/command_'|cat:$page_info.last}"><input type="hidden" name="csrf_ticket" value="{$csrf_ticket}">
        <div class="mobile_delete_confirm_grn mobile_schedule_delete_confirm_grn">
            ...
            <span class="mobile_text_grn">{cb_msg module='grn.schedule' key='GRN_SCH-150' replace='true'}</span>
        ...
        <div class="mobile_schedule_grn">
            <div class="mobile_list_grn">
                ...
                {if $schedule_event.netmeeting}
                    <p>{cb_msg module='grn.schedule.netmeeting' key='GRN_VCB-35' replace='true'}{if $schedule_event.netmeeting_msg}<br>{if ($schedule_event.type == "share") and ($schedule_event.user_id == $login.id)}{cb_msg module='grn.schedule.netmeeting' key='GRN_VCB-36' replace='true'}{else}{cb_msg module='grn.schedule.netmeeting' key='GRN_VCB-37' replace='true'}{/if}{/if}</p>
                {/if}
                {** -------------- VCBSCH0010 END   ----------------- **}
                ...
                {else}
                    ...
                    {if (($schedule_event.type == "share_repeat") or ($schedule_event.type == "share") or ($schedule_event.type == "share_banner")) and ($schedule_event.user_id == $login.id)}
                        <div class="mobile_title_grn">{cb_msg module='grn.schedule' key='GRN_SCH-156' replace='true'}<span class="mobile_important_grn">*</span></div>
                        <ul id="member">
                            <li>
                                <span class="mobile_icon_radiobutton_grn mobile_icon_radiobuttonoff_grn"></span>
                                <span class="mobile_radiobutton_grn"><input name="member" type="radio" value="1"></span>
                                <span class="mobile_text_grn ">{cb_msg module='grn.schedule' key='GRN_SCH-159' replace='true'}</span>
                            </li>
                            <li>
                                <span class="mobile_icon_radiobutton_grn mobile_icon_radiobuttonoff_grn"></span>
                                <span class="mobile_radiobutton_grn"><input name="member" type="radio" value="0"></span>
                                <span class="mobile_text_grn mobile_important_grn">{cb_msg module='grn.schedule' key='GRN_SCH-158' replace='true'}</span>
                            </li>
                        </ul>
                    {/if}
                    {if ($schedule_event.type == "share_repeat") or ($schedule_event.type == "repeat")}
                        <div class="mobile_title_grn">{cb_msg module='grn.schedule' key='GRN_SCH-160' replace='true'}<span class="mobile_important_grn">*</span></div>
                        <ul id="apply">
                            <li>
                            ...
                            <li>
                                ...
                                <span class="mobile_text_grn mobile_important_grn">{cb_msg module='grn.schedule' key='GRN_SCH-165' replace='true'}</span>
                            </li>
                        </ul>
                    {/if}
                    {if ( $schedule_event.type == 'share_temporary' ) or ( $schedule_event.type == 'temporary' )}
                        <div class="mobile_title_grn">{cb_msg module='grn.schedule' key='GRN_SCH-166' replace='true'}<span class="mobile_important_grn">*</span></div>
                        <ul id="temporary">
                                    ...
                                    <li>
                                        ...
                                        <ul>
                                            {foreach from=$facility_data.dates item=temporary}
                                                <li>
                                                    <span class="mobile_icon_radiobutton_grn mobile_icon_radiobuttonoff_grn"></span><span class="mobile_radiobutton_grn"><input name="did" id="{$temporary.temporary_id}" type="radio" value="{$temporary.temporary_id}"></span>
                                                    <span class="mobile_text_grn">{$temporary.str_date}<br />{$temporary.str_date_sub}</span>
                                                </li>
                                            {/foreach}
                                        </ul>
                                    </li>
                            ...
                            {else}
                                {foreach from=$schedule_event.temporary item=temporary}
                                    <li>
                                        <span class="mobile_icon_radiobutton_grn mobile_icon_radiobuttonoff_grn"></span>
                                        <span class="mobile_radiobutton_grn"><input name="did" id="{$temporary.temporary_id}" type="radio" value="{$temporary.temporary_id}"></span>
                                        <span class="mobile_text_grn">{$temporary.str_date}<br />{$temporary.str_date_sub}</span>
                                    </li>
                                {/foreach}
                            {/if}
