...
<p>
 <input type=button value="{cb_msg module='grn.schedule' key='GRN_SCH-672' replace='true'}" onClick="popup_apply( this.form )"{if $is_candidate_organization} disabled{/if}>
 <input type="button" value="{cb_msg module='grn.schedule' key='GRN_SCH-673' replace='true'}" onClick="window.close();">
</p>
<input type="hidden" name="gid" value="{$set_organization_id|escape}">
