...
<p>
    {cb_msg module='grn.schedule' key='GRN_SCH-832' replace='true'}<br>
    {cb_msg module='grn.schedule' key='GRN_SCH-176' replace='true'}
    <span class="bold">{grn_sentence caption=$schedule_event.data class='schedule20'}</span>
</p>
{** -------------- VCBSCH0010 START ----------------- **}
{if $netmeeting}
 <p>
 ...
 <table class="std_form">
  ...
  <tr>
   ...
   <td>
    <div class="mTop10">
     {strip}
      {capture name='grn_schedule_GRN_SCH_183'}{cb_msg module='grn.schedule' key='GRN_SCH-183' replace='true'}{/capture}
      {grn_button ui="main" spacing="normal" action="submit" caption=$smarty.capture.grn_schedule_GRN_SCH_183 id="schedule_button_save"}

      {capture name='grn_schedule_GRN_SCH_184'}{cb_msg module='grn.schedule' key='GRN_SCH-184' replace='true'}{/capture}
      {grn_button action="cancel" caption=$smarty.capture.grn_schedule_GRN_SCH_184 id="schedule_button_cancel" page='schedule/view' page_param_event=$schedule_event.event_id page_param_bdate=$schedule_event.date page_param_uid=$user_id page_param_gid=$group_id page_param_referer_key=$referer_key}
     {/strip}
     </div>
   </td>
  </tr>
