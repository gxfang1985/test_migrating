...
public class CmdUserIdListCSVExport {
    ...
    {
        System.err.println("Usage: java CmdUserIdListCSVExport [OPTIONS]");
        System.err.println();
    ...
    public static void main(String[] args) {
        ...
        for ( int i = 0; i < args.length; i++ )    {
            if ( args[i].equals("-h") ) {                    // Print this message and exit.
                ...
                title =  new String[]{"ユーザーID","ユーザーログイン名"};
            }
        ...
        try {
            ...
            // Garoonへデータ送信/取得
            TreeMap maps = grnLink.getUserIdList();
            
            // TreeMapをCSVで書き出す
            // Writerの準備
            userCSV = new CSVWriter(new OutputStreamWriter(new FileOutputStream(filePath), encode));
            
            // タイトル行の書出
            if(title !=null){
                userCSV.println(title);
            }
            ...
            while (it.hasNext()) {
                ...
                line[0] = ((Long) entry.getKey()).toString();  // ユーザーID
                line[1] = (String) entry.getValue();           // ユーザーログイン名
                
                userCSV.println(line);
            }
            userCSV.close();
