/* GASLink - Link module to the Garoon Administration System. */
...
/**
 * システム管理にログインし、ユーザー組織所属情報の書き込みを行う。
 *
 * @author サイボウズ
 * @version	1.0.0
 */
public class belongGrnLink2 extends HttpClient
{
    private static final String GASERROR_HEADER = "X-Cybozu-Error";
  
    /**
     * システム管理に接続、ログインし、セッションを開始します。（ユーザー所属組織情報）
     *
     * @param uri ガルーン２のURL
     ...
     */
    protected belongGrnLink2(URI uri, String userName, String password)
     ...
     * @throws RemoteException システム管理でエラーが発生
     */
    protected belongGrnLink2(URI uri)
	throws IOException, RemoteException
    {
    	super(uri);
    }
     ...
     * @throws RemoteException システム管理でエラーが発生
     */
    public belongGrnLink2(String uri, String userName, String password)
	throws IOException, RemoteException
    {
    	this(URI.create(uri + "/ldap/system/ldapUserOrganization_import1"), userName, password);
    }
     ...
     * @throws RemoteException システム管理でエラーが発生
     */
    public belongGrnLink2(String uri)
	throws IOException, RemoteException
    {
    	this(URI.create(uri + "/ldap/system/ldapUserOrganization_import2"));
    }
     ...
     * インスタンス生成時に、{@link #belongGrnLink2}内で呼ばれている。
     *
     * @param userName ログインするユーザーアカウント名
     * @param password ログインするためのパスワード 
     * @throws IOException 通信エラーが発生
    ...
    // API
    /**
     * ガルーン２にユーザー所属組織情報データを送信
     * 
     * @param data ユーザー所属組織情報
     * @param skipFirst 先頭行に項目名を書き出す。trueの場合は先頭行に項目名を書き出し、falseの場合は先頭行に項目名を書き出さない。
     * @return ガルーン２のエラーコード
     * @throws IOException 通信エラーが発生
     * @throws RemoteException システム管理でエラーが発生
     */
    public String uploadBelongCSV(byte[] data, boolean skipFirst, String encode)
	throws IOException, RemoteException
    {
    	...
    	ps.addProperty("fn", "system/user/user_organization_import1");
    	ps.close();

    	String s = processHeader2(hc,"failed to upload an organization CSV file.");
	...
	throws IOException, RemoteException
	{
    	...
    	processResult(hc, "failed to upload a organization CSV file.");
    }
}
